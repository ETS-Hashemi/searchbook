...
- [x] 2.17 Ch 17 RRT* and Informed RRT* (rewiring, asymptotic optimality, informed ellipsoid sampling) — written and compiled (20 pp); round-1 review applied
...
- [x] 6.4 Accuracy spot-check of all theorems, formulas and complexity tables by a second reviewer (independent Opus reviewer, `docs/accuracy-spot-check.md`: 144 statements + 8 tables checked, 1 error, 1 imprecise, 4 minor; all six applied)
...
- [~] 6.6 Apply book-level revisions and re-review until Accept — five fix agents running (see loop log 38); mechanical book-wide pass follows
...
- [x] 8.7 Overleaf compiles within its limits: pre-rendered figure cache (`Overleaf/figcache/`, 178 figures via TikZ externalization in "graphics if exists" mode, `tools/render_figures.py`, shipped in the zip). Measured: one pass over the full book 51 s (was 117 s), complete run 115 s (was 8-12 min); one chapter 5 s per pass, 13 s complete (was 41 s). Overleaf free plan (20 s): one chapter at a time; premium (4 min): whole book. Verified: 736 pages, 0 errors, index and bibliography intact, pixel comparison with the previous PDF shows only line-level reflow near a few figures.
...
| 42 | 09-11 02:35 | user reported that Overleaf cannot compile the book: cause is Overleaf's compile timeout (free 20 s, premium 240 s) against 117 s per pass; fixed with a pre-rendered figure cache (TikZ external, graphics-if-exists), renderer script, index guarded in export jobs, package rebuilt (902 files, 17 MB zip) | done; the zip now compiles chapter by chapter on the free plan and whole on premium |
