...
\section*{The project behind this book}

This book grew out of a doctoral research project in the Department of Systems
Engineering at École de technologie supérieure (ÉTS) in Montréal, on the automated
planning of trajectories for a swarm of drones. The swarm has to reach its goals while
keeping a formation, avoiding collisions among its own members, and avoiding obstacles,
both fixed ones and moving ones such as drones that are not part of the swarm and do
not communicate with it. Solving that problem meant collecting the algorithms that each
part of it requires, from graph search and multi-agent path finding to reactive
avoidance, tracking, prediction and control, and then designing a system that combines
them.

Two things were missing when that work began. The first was a single place to learn
the algorithms. They are scattered over the conferences and journals of several
communities (artificial intelligence, robotics, control and computer graphics), so a
researcher entering the subject had to find the papers one by one and could easily
miss a related method; no textbook gathered them in a form that can be taught in one
semester. The second was a worked-out system design showing how the pieces fit
together. This book supplies both. Every algorithm is presented with its idea, its
definitions, its pseudocode, a worked example and a \python{} implementation, and
\cref{ch:ch24} presents the hybrid architecture that was designed for the research
project and used in its implementation, as a framework that other researchers and
students can adopt, with modifications of their own, for their own systems.
...
\emph{implement} the algorithms rather than merely recognize their names. You should be
...
\begin{description}[style=nextline,leftmargin=1.5em]
  \item[Part I, Foundations.] \Cref{ch:ch01} states the problem and previews the
        hybrid architecture that the whole book builds toward. \Cref{ch:ch02} is a
        toolbox: graphs, grids, configuration space, time-indexed paths, simple motion
        models, the geometry of moving disks, Gaussians and priority queues.
...
order is a good fast path; \cref{sec:ch01-howto} draws it and this is its list:
\cref{ch:ch04} (\astar), \cref{ch:ch05} (\dstarlite), \cref{ch:ch09} (\cbs),
\cref{ch:ch07} (the MAPF problem), \cref{ch:ch12,ch:ch13} (velocity obstacles and
\orca), \cref{ch:ch14} (DWA), \cref{ch:ch16,ch:ch17} (\rrt and \rrtstar),
\cref{ch:ch02} (the toolbox, which stands in for the plan's ``planning reference''),
\cref{ch:ch21} (MPC) and \cref{ch:ch20} (trajectory prediction), then \cref{ch:ch24}.

Three remarks come with that order. First, the plan lists \cbs before the MAPF
overview; if you have never seen a multi-agent path finding instance, read
\cref{ch:ch07} first---it is short, and \cref{ch:ch09} assumes it. Second, the order
reaches trajectory prediction without the Kalman filter, although \cref{ch:ch20}
extrapolates with one and takes its input from the tracker of \cref{ch:ch18}; read
\cref{ch:ch18} first, or at least its section on predicting ahead. Third, the toolbox
of \cref{ch:ch02} and the mathematical refresher of \cref{ch:appB} are reference
material, to be opened whenever a chapter assumes something you do not have, rather
than a stop to be postponed until the tenth item of the list.

Every chapter ends with a box that says where its algorithm sits in the hybrid system,
so you can also read by \emph{layer}: the global planner (Parts II and III), the local
safety layer (Part IV), the prediction layer (Part VI) and the constraints (Part VII).

\section*{Conventions}

Every chapter opens with a box of \emph{learning objectives} and closes with a
\emph{summary} box and exercises. Three kinds of colored box appear inside every
chapter: \emph{key idea} (green) states the one thing to remember; \emph{in the drone
system} (purple) says how the algorithm is used in the hybrid architecture; and
\emph{pitfall} (orange) warns about a mistake that is easy to make. Two more appear
occasionally: a \emph{historical note} (gray) gives context, and a plain note carries
its own title. Definitions, examples, theorems and remarks share one counter per
chapter, so ``Example~4.3'' comes after ``Definition~4.2''. Exercises are marked with
one to three stars (\difficulty{1} routine, \difficulty{2} needs thought,
\difficulty{3} a small project), and hints or solutions for a selection of them are in
\cref{ch:appC}. The priority the training plan assigns to an algorithm is shown as a
tag such as \priority{Essential}; the tags are collected in \cref{tab:ch01-map} and
\cref{tab:appA-traceability}, and appear in running text only in \cref{ch:ch01} and
\cref{ch:appA}, where the plan itself is being reproduced. Pseudocode is written in a
uniform style with numbered lines;
...
preface. The book uses American spelling throughout (\emph{optimize}, \emph{neighbor},
\emph{center}); titles in the bibliography keep the spelling of the original publication.
...
The chapters were drafted with the assistance of large-language-model writing tools
under the author's direction, and the manuscript was reviewed in several rounds for
technical accuracy, completeness against the training plan, and clarity; the author is
responsible for the content. Mistakes will nevertheless remain; if you find
one, the \LaTeX{} sources and code are distributed with the book precisely so that it
can be corrected.

\section*{Acknowledgments}

I thank my dear wife, my parents and my family for the support they gave me throughout
my education. I thank Professor Julio Montecinos for his support, for the time he
devoted to training me, for reviewing my work, and for his precious input, which truly
brought out my potential as a researcher.

\bigskip
\noindent Seyed Masoud Hashemi Ahmadi\\
Montréal, September 2026
