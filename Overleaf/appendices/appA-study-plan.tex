...
\begin{itemize}
  ...
  \item design the capstone experiment matrix, name the metrics to report, and say what each
    metric hides;
...
\cref{sec:appA-timeline} refuses to let you compress them.}
...
strategy, and the whole chart is the hybrid.}
...
\paragraph{A six-question self-check.}
The chapters carry the graded exercises; the questions below are the plan's own ``Done
when'' tests turned into prompts, and you should be able to answer all six from your own
lab notebook without opening a chapter.
\begin{enumerate}
  \item State the optimality theorem of \astar with its conditions, and say what breaks when
    each condition is dropped (Week~1).
  \item Show a log in which the path cost after every \dstarlite repair equals the cost of a
    fresh \astar from the same position (Week~2).
  \item Give an instance on which one priority order fails and another succeeds, and name
    the conflict type that appears (Weeks~3).
  \item Draw the constraint tree of a two-agent instance by hand and match it against your
    \cbs log (Week~4).
  \item Show your cost-against-runtime plot as $w$ varies and say in numbers what
    suboptimality bought (Week~5).
  \item Name one run of your capstone in which the local layer was not enough, and say which
    trigger fired and what the replanning count was (Week~12).
\end{enumerate}
\index{self-check}
\FloatBarrier
...
\begin{itemize}
  ...
  \item If time runs short, compress Week~7 (DWA \emph{or} APF), Week~8 (use a library for
    \rrtstar) and the MILP part of Week~11. Do not compress Weeks~1, 4, 6, 9 or 12: each
