...
\begin{itemize}
  \item choose a twelve-week route through this book and a weekly rhythm that fits the
    plan's 6--8 hours of study;
  \item say, for every week, what to build and how to tell that it works, using the week's
    ``Done when'' test rather than a feeling of understanding;
  \item name the four layers of the capstone architecture, the interface of each layer, and
    the decision logic that switches between them at run time;
  \item design the capstone experiment matrix, name the metrics to report, and say what the
    headline metrics hide;
  \item use the traceability table in both directions: from an algorithm of the plan to its
    chapter and week, and from a chapter to the depth the plan expects of you there.
\end{itemize}
\end{objectives}
...
bounded-suboptimal variant enhanced \cbs (\ecbs), \cref{ch:ch09,ch:ch10}), a local safety layer
...
moving on. Over twelve weeks that is $12 \times 6$ to $12 \times 8$, or 72--96 hours in total, which is what you
should budget before you start.
\index{study load}
...
capstone of Week~12, and any paper you write afterward, will draw on the numbers, failure
...
\begin{itemize}
  \item \textbf{Python 3.10+ and NumPy.} Every reference implementation in this book is
    plain Python with NumPy (SciPy only for the quadratic program (QP) and mixed-integer
    linear program (MILP) solvers of \cref{ch:ch21,ch:ch22}). Write your own code the same way; you will read and debug it
    faster.
  ...
  \item \textbf{PyBullet or gym-pybullet-drones} \cite{panerati2021learning} for a 3D
    simulator with quadrotor dynamics. You need it from Week~8 onward and for the
...
\Cref{tab:appA-schedule} is the twelve-week schedule of the training plan, with two columns
added: \emph{Read} names the chapters to read and \emph{Exercise} the exercise of this book
that is the week's deliverable. The \emph{Learn}, \emph{Coding exercise} and
...
\begin{longtable}{@{}L{0.5cm}L{1.75cm}L{1.5cm}L{2.6cm}L{3.0cm}L{1.45cm}L{3.0cm}@{}}
\caption{The twelve-week schedule, mapped to the chapters of this book and to the
exercise that is the week's deliverable.}
\label{tab:appA-schedule}\\
\toprule
\textbf{Wk} & \textbf{Topic} & \textbf{Read} & \textbf{Learn} & \textbf{Coding exercise} & \textbf{Exercise} & \textbf{Milestone}\\
\midrule
\endfirsthead
\multicolumn{7}{@{}l}{\textit{\Cref{tab:appA-schedule} (continued)}}\\
\toprule
\textbf{Wk} & \textbf{Topic} & \textbf{Read} & \textbf{Learn} & \textbf{Coding exercise} & \textbf{Exercise} & \textbf{Milestone}\\
\midrule
\endhead
...
1 & Foundations: Dijkstra and \astar
  ...
  & \cref{exr:ch03-coding,exr:ch04-coding}
...
3 & MAPF basics and conflict types
  ...
  & \cref{exr:ch07-prioritized,exr:ch08-coding}
...
6 & Velocity Obstacles, RVO, and \orca
  ...
  & \cref{exr:ch12-coding,exr:ch13-coding}
...
7 & DWA and Artificial Potential Fields
  ...
  & \cref{exr:ch14-coding} or \cref{exr:ch15-coding}
...
8 & \rrt, \rrtstar, and continuous 3D planning
  ...
  & \cref{exr:ch16-coding,exr:ch17-coding}
...
11 & MPC and formation/communication constraints
  ...
  & \cref{exr:ch21-coding,exr:ch23-coding}
...
12 & Hybrid system integration
  ...
  & Build the capstone architecture (Section~\ref{sec:appA-capstone}) and run controlled experiments.
  & \cref{exr:ch24-coding,exr:ch25-coding}
...
the families. Notice that seven of the twelve weeks -- more than half -- go to the three
``learn deeply'' families, and that the capstone gets a full week of its own. If you are short of time, the
places to compress are Week~5 (benchmark two values of $w$ instead of three), Week~8 (use a
library for \rrtstar) and the MILP part of Week~11 (read \cref{ch:ch22}, do not implement it). Do not
...
asks you to learn deeply, take seven of the twelve weeks (2, 3 and 2); the capstone has a week of its
own.}
...
\cref{ch:ch02}; plan lighter weeks around heavier ones. When these notes say ``the coding
exercise of'' a chapter, the exercise meant is the one named in the \emph{Exercise} column
of \cref{tab:appA-schedule}; several chapters carry more than one exercise marked
\emph{Coding}, and that column says which of them is the week's deliverable.
...
look like a new bug. \Cref{fig:appA-week-dependencies} draws the artifacts that later weeks
reuse, so that you can see how far a red test travels before it is found. If the milestone sentence is not true on the last day of the week,
spend the first hours of the next week on it and shorten that week's exercise instead;
Section~\ref{sec:appA-timeline} names the parts that can be compressed.
\end{pitfall}

\begin{figure}[htb]
\centering
\inputfigure{appA/week-dependencies}
\caption{What each week hands to the weeks after it. Only the reuse that the week notes
assert is drawn: the space-time \astar of Week~1 is the low level of Weeks~3, 4 and of the
replanning layer, the Week-3 checker is the conflict detector of \cbs and of the capstone's
re-check step, and the Week-6 simulator produces both the failure cases of Week~7 and the
trajectories that Week~10 learns from. Notice how much hangs on Weeks~1 and 3: four arrows leave Week~1 and
two leave Week~3, so a bug carried out of either reappears in every week it feeds, which is why
Section~\ref{sec:appA-timeline} refuses to let you compress them.}
\label{fig:appA-week-dependencies}
\end{figure}
...
\textbf{Focus.} Dijkstra's settled-node invariant on graphs with non-negative edge costs (a
node popped from \Open with the smallest $\gcost$ has its final cost), and what the heuristic buys in \astar: an admissible
...
at a given time; with the zero heuristic both searches expand the same nodes as Dijkstra, up to tie-breaking.
...
conflict types (vertex, edge, swapping, following, cycle) and the two objectives (sum of
costs, makespan). Keep the two edge-like types apart: an edge conflict is the same edge in
the same direction, a swap is the two agents exchanging ends. A swap is invisible if you
compare positions only. Then \cref{ch:ch08}:
...
\textbf{Traps.} Off-by-one in time (the position at time~0 is the start; a swap is
agent~$i$ moving $u \to v$ while agent~$j$ moves $v \to u$ over the same step
$t \to t+1$); an agent that has arrived stays at its goal and must be
...
allows; benchmark both for $w \in \set{1.1, 1.5, 2}$ with $w = 1$ (plain \cbs) as the
reference, as \cref{exr:ch10-coding}(a) asks: success rate under a time
...
\cbs's optimal cost, and a paragraph that says with numbers what suboptimality buys.
...
disk whose radius is the sum of the two radii. The velocity obstacle (VO) written $\VO^{\ttc}_{A|B}$ without an
argument (\cref{ch:ch13}, \cref{def:ch13-vo}) is the set of \emph{relative} velocities
$\vel_{\mathrm{rel}} = \vel_A - \vel_B$ that bring the two disks into contact within the
horizon $\ttc$; its apex is at the origin, and \orca reasons in exactly this form.
\emph{Adding} $\vel_B$ gives the absolute set, the velocities of $A$ that collide if $B$
holds its current velocity, which \cref{ch:ch12} writes with the argument,
$\VO^{\ttc}_{A|B}(\vel_B)$ (\cref{def:ch12-truncated}), and whose relative form is
recovered by subtracting $\vel_B$ again (\cref{thm:ch12-truncated}). Getting the
direction of that translation the wrong way round is the commonest reading error of the
week. The reciprocal velocity
...
\textbf{Done when.} You can draw the cone and the \orca half-plane for one pair and say
which velocities each of them forbids. Across the three modes you should see no avoidance
collide, VO avoid but oscillate or detour widely, and \orca stay smooth and collision-free
at the same speeds; if your radii and speeds put VO closer to \orca than that, say so and
report the separation numbers rather than doubting a correct implementation.

\textbf{Traps.} The orientation of the half-plane (it must exclude the velocity obstacle);
the truncated cone for finite $\ttc$ is a disk plus two tangent legs, not a triangle; when
...
\noindent\textbf{Load}\ \difficulty{1}\quad\textbf{Read}\ \cref{ch:ch14,ch:ch15}.
...
\textbf{Exercises.} The coding exercises of \cref{ch:ch16,ch:ch17}, which halve the week
between them: \cref{exr:ch16-coding} is the first half, the \rrt and the 3D obstacle
field, and \cref{exr:ch17-coding} completes it with rewiring, the convergence plot and
the comparison against grid \astar. The 3D field you build here is the map of the
capstone.
...
\Cref{ch:ch22} is the book's addition to the plan's Week~11: the plan gives MILP a
Medium priority in its algorithm list but no week of its own, so it is read here, as
background for the cost of exactness, and its coding exercise is optional.
...
\textbf{Focus.} The architecture and decision logic of Section~\ref{sec:appA-capstone}: the safety
...
planner, a prediction layer, a local safety layer and a replanning layer.
...
\Cref{tab:appA-layers} lists the four layers in the plan's words and
\cref{fig:appA-architecture} draws them with the data that crosses each interface. The
numbering is the plan's; it is neither the order in which the layers act at run time (that
is the decision logic below) nor the order in which you build them (that is the schedule).

\begin{figure}[htb]
\centering
\inputfigure{appA/architecture}
\caption{The four layers of the capstone and the data that crosses each interface. Notice
that every arrow carries one named object -- time-indexed paths, a predicted trajectory with
its covariance, a velocity, a repaired path -- which is what makes the layers separable and
the ``avoidance strategy'' axis of the experiments a switch rather than a rewrite. The
dashed edge is the one feedback path: a replan invalidates the swarm plan, so Layer~1's
conflict check runs again.}
\label{fig:appA-architecture}
\end{figure}
...
\midrule
1\ Global planner (\cbs/\ecbs)
  & Plan nominal, time-indexed paths for the drones under your control. Resolve conflicts among connected swarm members before execution. (Week~3 builds the conflict checker it needs.)
  & \cref{ch:ch09,ch:ch10} & 4--5\\
\addlinespace
2\ Prediction layer (Kalman filter / LSTM)
  ...
  & \cref{ch:ch18,ch:ch19,ch:ch20} & 9--10\\
\addlinespace
...
4\ Replanning layer (\dstarlite or \astar)
  & If the local maneuver cannot safely return the drone to its nominal route, replan from the current state. Then re-check swarm conflicts and formation/communication constraints. (Week~11 adds the formation and communication constraints this re-check must test.)
...
\Cref{fig:appA-decision-logic} draws the five steps as a flowchart.
\begin{enumerate}
...
\begin{figure}[tbh]
\centering
\inputfigure{appA/decision-logic}
\caption{The decision logic of the capstone, one pass per control cycle. Notice the three
triggers on the left: the two diamonds are the only places where a drone leaves or rejoins
its nominal \cbs/\ecbs path, so they are also the only places to instrument when you count
replans. The two dashed cuts are the ``avoidance strategy'' axis of the experiment matrix:
cutting layer~4 leaves the local-only strategy, cutting layer~3 leaves the replan-only
strategy, and the whole chart is the hybrid. In replan-only the \emph{yes} branch goes
straight to step~4 (dotted); in local-only step~4 is never reached and the drone stays on
the local layer until it can reconnect.}
\label{fig:appA-decision-logic}
\end{figure}
...
five factors -- three swarm sizes, four intruder counts, four prediction qualities, three
avoidance strategies and four constraint settings (two formation options crossed with two
communication options) -- have $3 \times 4 \times 4 \times 3 \times 4 = 576$ cells before
seeds, and $11\,520$ runs at the $20$ seeds per cell that \cref{ch:ch25} asks for; you
will not run all of them. This is the count of \cref{ch:ch25}
(\cref{sec:ch25-design} and \cref{tab:ch25-matrix}): the swarm size is read as the three
levels $2$, $4$, $8$, with
``and more if computationally feasible'' an invitation rather than a fourth level, while
the intruder count keeps four levels because ``multiple crossing trajectories'' is a
level of its own. Fix every factor at a default, vary one or two at a time, and let the
...
\begin{pitfall}[Benchmarking on one instance]
One instance, one seed and one trajectory split are the three cheapest ways to reach a
conclusion that does not survive contact with a second one. Week~5 warns against concluding
from one instance and Week~10 against tuning on the test set; the capstone inherits both
traps at once, because a hybrid strategy that wins on a single crossing geometry can lose on
the next. Fix the factors of \cref{tab:appA-experiments} at a default, run every cell you
report over several seeds, give the spread and not only the mean, and keep the instances you
tune on apart from the instances you report on.
\end{pitfall}
\FloatBarrier
...
\paragraph{An eight-question self-check.}
The chapters carry the graded exercises; the questions below are the plan's own ``Done
when'' tests turned into prompts, graded like those exercises from \difficulty{1} to
\difficulty{3}, and you should be able to answer all eight from your own lab notebook
without opening a chapter.
\begin{enumerate}
  \item \difficulty{1} State the optimality theorem of \astar with its conditions, and say
    what breaks when each condition is dropped (Week~1).
  \item \difficulty{2} Show a log in which the path cost after every \dstarlite repair
    equals the cost of a fresh \astar from the same position (Week~2).
  \item \difficulty{1} Give an instance on which one priority order fails and another
    succeeds, and name the conflict type that appears (Week~3).
  \item \difficulty{2} Draw the constraint tree of a two-agent instance by hand and match it
    against your \cbs log (Week~4).
  \item \difficulty{2} Show your cost-against-runtime plot as $w$ varies and say in numbers
    what suboptimality bought (Week~5).
  \item \difficulty{2} Draw the VO cone and the \orca half-plane for one crossing pair and
    say which velocities each of them forbids (Week~6).
  \item \difficulty{2} Show the filtered estimate beating the raw measurements on one run,
    and the ADE/FDE table per horizon for constant velocity and the LSTM (Weeks~9--10).
  \item \difficulty{3} Name one run of your capstone in which the local layer was not
    enough, and say which trigger fired and what the replanning count was (Week~12).
\end{enumerate}
\index{self-check}
\FloatBarrier
...
source to read next to it. The first six items are the plan's highest priority: they are three of
the four capstone layers -- the global planner (item~3), the local safety layer
(items~5--6) and the replanning layer (items~1--2) -- plus the MAPF definitions (item~4)
that make the conflict re-check precise; the prediction layer's sources come later, at
item~12. To read a paper next to its chapter: read the chapter up to the end of its worked
...
\begin{summary}[Appendix summary]
\begin{itemize}
  \item The plan builds three abilities and the perception that feeds them: a global planner
    for planned routes, a local method for surprises, and a replanner for when the local
    response is not enough.
  \item The rhythm is 6--8 hours a week -- at most two reading, four to six coding -- and the
    milestone rule: if the week's sentence is not true on the last day, close the gap in the
    first hours of the next week instead of moving on.
  \item If time runs short, compress Week~5 (two values of $w$ instead of three), Week~8
    (use a library for \rrtstar) and the MILP part of Week~11. Do not compress Weeks~1, 4, 6, 9 or 12: each
    builds a layer that the later weeks assume, as \cref{fig:appA-week-dependencies} shows.
  \item The capstone has four layers -- global planner, prediction, local safety,
    replanning (\cref{fig:appA-architecture}) -- joined by the five-step decision logic of
    \cref{fig:appA-decision-logic}, and is measured by the factor matrix of
    \cref{tab:appA-experiments} and its ten metrics.
  \item The ten statements of \cref{tab:appA-checklist} are the exit test. Tick a line only
    when you can do what it says without opening the book.
\end{itemize}
\end{summary}
