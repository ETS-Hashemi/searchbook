...
\begin{itemize}
  \item manipulate vectors, matrices and block matrices with the identities that the filtering, control and consensus chapters rely on, and recognise a positive (semi)definite matrix when you meet one;
  \item linearise a nonlinear map with its Jacobian, judge the error of the linearisation, and step a double integrator forward exactly;
  \item write down the density of a multivariate Gaussian, transform it affinely, condition it on some of its variables, and gate a measurement with the Mahalanobis distance;
  \item tell a convex problem from a non-convex one, read the KKT conditions, and say what a linear, a quadratic and a mixed-integer program is and how each is solved;
  \item form the Laplacian of a graph and explain why its second eigenvalue measures how well the graph is connected;
...
it. It is a refresher, not a course: every result is stated exactly, the short
proofs are given in full, the long ones are sketched with a pointer to a
textbook, and each section names the chapters that depend on it
(\cref{tab:appB-map}). Read a section when a chapter sends you here or when a
symbol in a derivation looks unfamiliar; nothing here has to be read in order.
The notation is that of the notation table at the front of the book: vectors
are bold lower case, matrices bold upper case, $\mat{A}\T$ is the transpose,
$\mat{A}\inv$ the inverse, $\mat{I}$ the identity, $\vect{1}$ the vector of
ones and $\norm{\cdot}$ the Euclidean norm. Every number quoted in the
examples is produced by the script \code{code/figures/gen\_appB\_numbers.py};
change its inputs to check your own hand calculations against it.

\begin{table}[tb]
  \centering\small
  \caption{Where the book uses each part of this appendix.}
  \label{tab:appB-map}
  \begin{tabular}{@{}lL{7.2cm}l@{}}
  \toprule
  Section & Contents & Used mainly in\\
  \midrule
  \ref{sec:appB-linalg} & norms, products, eigenvalues, definiteness, block matrices, Schur complement, least squares & \cref{ch:ch02,ch:ch18,ch:ch21,ch:ch23}\\
  \ref{sec:appB-calculus} & gradient, Jacobian, Hessian, Taylor expansion, double-integrator step & \cref{ch:ch02,ch:ch15,ch:ch19,ch:ch21}\\
  \ref{sec:appB-probability} & covariance, Gaussians, conditioning, Bayes, Monte Carlo, chi-square gating & \cref{ch:ch18,ch:ch19,ch:ch20}\\
  \ref{sec:appB-optimization} & convexity, KKT, linear, quadratic and mixed-integer programs, gradient descent & \cref{ch:ch13,ch:ch15,ch:ch20,ch:ch21,ch:ch22}\\
  \ref{sec:appB-graphs} & adjacency and Laplacian matrices, algebraic connectivity, shortest-path notation & \cref{ch:ch03,ch:ch04,ch:ch23}\\
  \ref{sec:appB-complexity} & asymptotic notation, binary heaps, hashing, \code{heapq} and \code{dict} & \cref{ch:ch03,ch:ch04,ch:ch08,ch:ch09}\\
  \bottomrule
  \end{tabular}
\end{table}
...
\norm{\vect{y}} \cos\theta$, where $\theta$ is the angle between the vectors;
hence the \textbf{Cauchy--Schwarz inequality}\index{Cauchy--Schwarz inequality}
$\abs{\vect{x} \cdot \vect{y}} \le \norm{\vect{x}} \norm{\vect{y}}$, and two
vectors are orthogonal exactly when their dot product is zero. Second, the
\textbf{projection} of $\vect{x}$ onto the direction of a unit vector
$\vect{u}$ is $(\vect{x} \cdot \vect{u})\, \vect{u}$; the distance from a point
to a line or a segment in \cref{ch:ch02} is the length of what remains after
the projection is removed. Third, $\vect{a} \times \vect{b}$ in $\R^3$ is
orthogonal to both factors and has length $\norm{\vect{a}} \norm{\vect{b}}
\sin\theta$, while the planar scalar $\vect{a} \times \vect{b}$ is positive when
$\vect{b}$ lies counter-clockwise from $\vect{a}$ and negative when it lies
clockwise: this sign is the left-or-right test behind every half-plane and
cone membership test of \cref{ch:ch12,ch:ch13}. The grid heuristics of
\cref{ch:ch04} are norms of the displacement $\vect{d}$ to the goal: the
Manhattan distance is $\norm{\vect{d}}_1$, the Euclidean distance
$\norm{\vect{d}}$ and the Chebyshev distance $\norm{\vect{d}}_\infty$.

\subsection{Matrices, transpose, inverse and rank}

A matrix $\mat{A} \in \R^{m \times n}$ maps $\R^n$ to $\R^m$ by $\vect{x}
\mapsto \mat{A}\vect{x}$; the product $\mat{A}\mat{B}$ is the composition
``first $\mat{B}$, then $\mat{A}$'', with entries $(\mat{A}\mat{B})_{ij} =
\sum_k a_{ik} b_{kj}$, and it is not commutative in general. The
\textbf{transpose}\index{matrix!transpose} $\mat{A}\T$ swaps rows and
columns; a square matrix with $\mat{A}\T = \mat{A}$ is \textbf{symmetric}. The
\textbf{inverse}\index{matrix!inverse} $\mat{A}\inv$ of a square matrix is the
matrix with $\mat{A}\mat{A}\inv = \mat{A}\inv\mat{A} = \mat{I}$, when it
exists. The \textbf{rank}\index{matrix!rank} of $\mat{A}$ is the number of
linearly independent columns, which equals the number of linearly independent
rows. The \textbf{trace} $\tr \mat{A} = \sum_i a_{ii}$ and the
\textbf{determinant} $\det \mat{A}$ are defined for square matrices; a matrix
$\mat{Q}$ with $\mat{Q}\T\mat{Q} = \mat{I}$ is \textbf{orthogonal} (rotations
and reflections are the examples that matter here) and preserves norms and dot
products. The following equivalences are the working definition of
invertibility.
...
$\lambda_i$ along each of $n$ orthogonal axes $\vect{q}_i$. Powers and inverses
follow at once: $\mat{A}^k = \mat{Q}\mat{\Lambda}^k\mat{Q}\T$ and, if no
eigenvalue vanishes, $\mat{A}\inv = \mat{Q}\mat{\Lambda}\inv\mat{Q}\T$. The
\textbf{quadratic form}\index{quadratic form} $\vect{x}\T\mat{A}\vect{x} =
\sum_i \lambda_i (\vect{q}_i\T\vect{x})^2$ is a weighted sum of squared
coordinates along the axes; its sign is the notion that decides whether a
covariance is valid, a cost is convex and a consensus protocol converges.
...
\end{proposition}
\begin{proof}
(i) follows from $\vect{x}\T\mat{A}\vect{x} = \sum_i \lambda_i (\vect{q}_i\T\vect{x})^2$: take $\vect{x} = \vect{q}_i$ for one direction and note that all terms are non-negative for the other; the eigenvalues of $\mat{A}\inv$ are $1/\lambda_i$. (ii) $\vect{x}\T\mat{B}\T\mat{B}\vect{x} = \norm{\mat{B}\vect{x}}^2 \ge 0$; conversely $\mat{B} = \mat{\Lambda}^{1/2}\mat{Q}\T$ works, and Gaussian elimination without pivoting produces the triangular factor \cite[Appendix~C]{boyd2004convex}. (iii) is immediate from the definition, since $\vect{x}\T\mat{C}\T\mat{A}\mat{C}\vect{x} = (\mat{C}\vect{x})\T\mat{A}(\mat{C}\vect{x})$. \qedhere
\end{proof}
...
Large matrices in this book come in blocks: a state $(\pos, \vel)$ splits its
covariance into position, velocity and cross blocks; the joint distribution of
a state and a measurement splits in the same way; the KKT system of a
quadratic program stacks a Hessian and a constraint matrix. Blocks multiply
like scalars as long as their order is kept,
\begin{equation}
...
\end{theorem}
\begin{proof}
Multiply out the right-hand side of \cref{eq:appB-block-fact} with \cref{eq:appB-block-product}: the product of the last two factors is $\begin{bsmallmatrix}\mat{S} & \mat{0}\\ \mat{C} & \mat{D}\end{bsmallmatrix}$, and multiplying by the first factor gives $\begin{bsmallmatrix}\mat{S} + \mat{B}\mat{D}\inv\mat{C} & \mat{B}\\ \mat{C} & \mat{D}\end{bsmallmatrix} = \mat{M}$. The outer factors are block-triangular with identity diagonal blocks, so their determinant is $1$ and their inverses are obtained by negating the off-diagonal block; the determinant of the middle factor is $\det\mat{S}\det\mat{D}$. Inverting the three factors in reverse order and multiplying out gives \cref{eq:appB-block-inverse}. The second statement is the first one applied to $\mat{M}$ with its block rows and block columns swapped. \qedhere
\end{proof}
...
says. The same theorem, read in both directions, gives the identity that
\cref{ch:ch18} uses to pass between the covariance form and the information
form of the Kalman update, and that turns the inversion of a large matrix into
the inversion of a small one whenever the update has low rank.
...
\end{proposition}
\begin{proof}
Apply \cref{thm:appB-block} to $\mat{M} = \begin{bsmallmatrix}\mat{A} & \mat{U}\\ \mat{V} & -\mat{C}\inv\end{bsmallmatrix}$. Eliminating the $(2,2)$ block gives $\mat{S} = \mat{A} + \mat{U}\mat{C}\mat{V}$ and $(\mat{M}\inv)_{11} = \mat{S}\inv$; eliminating the $(1,1)$ block gives $\mat{S}' = -\mat{C}\inv - \mat{V}\mat{A}\inv\mat{U}$ and $(\mat{M}\inv)_{11} = \mat{A}\inv + \mat{A}\inv\mat{U}\mat{S}'^{-1}\mat{V}\mat{A}\inv$. Equating the two expressions for the same block gives \cref{eq:appB-woodbury}. \qedhere
\end{proof}
...
\end{theorem}
\begin{proof}
Write $\vect{r} = \mat{A}\vect{x}^* - \vect{b}$, let $\vect{x}$ be arbitrary and $\vect{d} = \vect{x} - \vect{x}^*$. Then $\norm{\mat{A}\vect{x} - \vect{b}}^2 = \norm{\vect{r} + \mat{A}\vect{d}}^2 = \norm{\vect{r}}^2 + 2\vect{d}\T\mat{A}\T\vect{r} + \norm{\mat{A}\vect{d}}^2$. If \cref{eq:appB-normal} holds, the middle term vanishes for every $\vect{d}$ and $\vect{x}^*$ is a minimiser; if it does not hold, choosing $\vect{d} = -t\mat{A}\T\vect{r}$ with a small $t > 0$ makes the sum of the last two terms negative, so $\vect{x}^*$ is not a minimiser. With full column rank, $\mat{A}\vect{d} = \vect{0}$ forces $\vect{d} = \vect{0}$, so $\vect{d}\T\mat{A}\T\mat{A}\vect{d} = \norm{\mat{A}\vect{d}}^2 > 0$ for $\vect{d} \ne \vect{0}$ and the minimiser is unique. The weighted case is the unweighted one for $\mat{W}^{1/2}\mat{A}$ and $\mat{W}^{1/2}\vect{b}$, with $\mat{W}^{1/2} = \mat{Q}\mat{\Lambda}^{1/2}\mat{Q}\T$ from \cref{thm:appB-spectral}. \qedhere
\end{proof}

\begin{example}[Fitting a constant-velocity line]
\label{ex:appB-lsq}
A drone is seen at the positions $0.0$, $1.1$ and $1.9$\,m at the times $t = 0, 1, 2$\,s along one axis. The model $p(t) = p_0 + v t$ is linear in the unknowns $\vect{x} = (p_0, v)\T$, so
\begin{equation*}
  \mat{A} = \begin{bmatrix} 1 & 0\\ 1 & 1\\ 1 & 2\end{bmatrix},\quad
  \vect{b} = \begin{bmatrix} 0.0\\ 1.1\\ 1.9\end{bmatrix},\quad
  \mat{A}\T\mat{A} = \begin{bmatrix} 3 & 3\\ 3 & 5\end{bmatrix},\quad
  \mat{A}\T\vect{b} = \begin{bmatrix} 3.0\\ 4.9\end{bmatrix},
\end{equation*}
and \cref{eq:appB-normal} gives $p_0 = 0.05$\,m and $v = 0.95$\,m/s. The residuals $\mat{A}\vect{x}^* - \vect{b} = (0.05, -0.10, 0.05)\T$ have squared norm $0.015$ and are orthogonal to both columns of $\mat{A}$, which is what the normal equations say.
\end{example}
...
and a measurement, weighted by the inverses of their covariances; an MPC
problem without inequality constraints (\cref{ch:ch21}) is a least-squares
problem in the stacked control inputs, and with constraints it becomes the
quadratic program of \cref{sec:appB-qp}; a least-squares line through the last
few observed positions, as in \cref{ex:appB-lsq}, is one way to build the
constant-velocity baseline of \cref{ch:ch20}.

\begin{pitfall}[Do not form the inverse]
The formulas above are written with $\mat{A}\inv$ because that is how they are proved, not how they should be computed. Solving $\mat{A}\vect{x} = \vect{b}$ with \code{numpy.linalg.solve}, or with a Cholesky factor when $\mat{A} \succ \mat{0}$, is faster and more accurate than forming \code{numpy.linalg.inv(A)} and multiplying. Two more habits pay off in \cref{ch:ch18,ch:ch19,ch:ch21}: keep a matrix that must be symmetric symmetric by replacing $\mat{P}$ with $\tfrac12(\mat{P} + \mat{P}\T)$ after each update, and check the smallest eigenvalue of a covariance before trusting a Cholesky factorisation or a Mahalanobis distance built from it, because rounding can turn a PSD matrix into one with a slightly negative eigenvalue.
\end{pitfall}
...
book are
\begin{equation}
  \nabla(\vect{b}\T\vect{x}) = \vect{b},\quad
  \nabla\left(\tfrac12\vect{x}\T\mat{A}\vect{x}\right) = \mat{A}\vect{x}\ \ (\mat{A} \text{ symmetric}),\quad
  \nabla\left(\tfrac12\norm{\vect{x} - \vect{c}}^2\right) = \vect{x} - \vect{c},\quad
...
so the force $-\nabla U$ that pulls the drone is proportional to $\vect{c} -
\vect{x}$; the repulsive potential is a function of the clearance
$\rho(\vect{x})$ to an obstacle, and by the chain rule its gradient is the
derivative of that function times the unit vector of the fourth expression,
...
\textbf{linearisation}\index{linearisation}: near $\vect{x}$ the nonlinear map
behaves like the linear map $\mat{J}_{\vect{g}}(\vect{x})$. The extended
Kalman filter of \cref{ch:ch19} replaces its motion and measurement models by
their Jacobians $\mat{F}$ and $\mat{H}$ at the current estimate, and the size
of $\vect{r}$ over the spread of the estimate is exactly what decides whether
that approximation is good enough or whether the unscented or particle
filters of the same chapter are needed. The second-order expansion is the
basis of Newton's method and of the convexity test in
...
\begin{equation}
  ...
  \label{eq:appB-AB}
\end{equation}
where $\mat{I}$ is the $d \times d$ identity.\index{double integrator} Three
chapters reuse this pair of matrices. \Cref{ch:ch18} takes $\mat{A}$ as the
transition matrix $\mat{F}$ of a constant-velocity target and turns the
unknown acceleration into process noise through $\mat{B}$, which is where its
white-noise-acceleration covariance $\mat{Q}$ comes from. \Cref{ch:ch21}
stacks $N$ copies of \cref{eq:appB-AB} into the prediction matrices of its
quadratic program. \Cref{ch:ch14} bounds the velocities reachable in one step
by $\norm{\vel_{k+1} - \vel_k} \le a_{\max}\dt$, its dynamic window, using the
first half of \cref{eq:appB-double-integrator}. The replacement $\pos_{k+1} =
\pos_k + \vel_k\dt$ (explicit Euler) drops the term $\tfrac12\acc\dt^2$: with
$\dt = 0.1$\,s, $v = 2$\,m/s and $a = 1$\,m/s$^2$ the exact step moves the
drone $0.205$\,m and leaves it at $2.1$\,m/s, and the Euler step moves it
$0.200$\,m. The difference is small per step but it accumulates, which is the
reason to use the exact form whenever the input is piecewise constant.
...
\mat{0}$ has no density, but the mean, the covariance and the two theorems
that follow still make sense for it.
...
\end{theorem}
\begin{proof}
The mean and the covariance follow from \cref{thm:appB-cov-rules}. For the shape, take first $m = n$ and $\mat{A}$ invertible: the change of variables $\state = \mat{A}\inv(\vect{y} - \vect{b})$ gives $p(\vect{y}) = \Normal(\mat{A}\inv(\vect{y} - \vect{b}); \vect{\mu}, \mat{\Sigma})\,\abs{\det\mat{A}\inv}$, and substituting shows that the exponent equals $-\tfrac12(\vect{y} - \mat{A}\vect{\mu} - \vect{b})\T(\mat{A}\mat{\Sigma}\mat{A}\T)\inv(\vect{y} - \mat{A}\vect{\mu} - \vect{b})$ while the constant becomes $((2\pi)^{n}\det(\mat{A}\mat{\Sigma}\mat{A}\T))^{-1/2}$. A general $\mat{A}$ of full row rank is extended by further rows to an invertible square matrix; the square case then applies, and the extra coordinates are integrated out with the marginalisation half of \cref{thm:appB-conditioning}, whose proof does not use this theorem. The remaining cases are handled in one line with characteristic functions \cite[Chapter~4]{sarkka2013bayesian}. \qedhere
\end{proof}

Two consequences are used constantly. The joint vector of two independent
Gaussians is Gaussian with block-diagonal covariance, because the product of
the two densities has the form \cref{eq:appB-gaussian}; applying the theorem
with $\mat{A} = [\mat{I}\ \ \mat{I}]$ shows that the sum of independent
Gaussians is Gaussian with the sums of the means and of the covariances, which
is how process noise enters the prediction step of \cref{ch:ch18}. And a
Gaussian stays Gaussian under any linear model, so a linear filter never has to
represent anything but a mean and a covariance.
...
$\Kgain$ the Kalman gain; \cref{ch:ch18} derives it from this corollary, with
the predicted mean and covariance in the roles of $\vect{\mu}$ and $\mat{P}$,
and \cref{ch:ch19} keeps the same formula while replacing $\mat{H}$ by a
Jacobian or the covariances by sample estimates. Applying
\cref{thm:appB-woodbury} to $\mat{P} - \Kgain\mat{H}\mat{P}$ gives the
equivalent information form $(\mat{P}\inv + \mat{H}\T\mat{R}\inv\mat{H})\inv$,
in which the precisions of the prior and of the measurement simply add.
...
only on the current state, the belief about $\state_k$ given the measurements
$\meas_{1:k}$ is obtained in two steps,
\begin{align}
...
Many quantities in this book are expectations without a closed form: the
probability that a predicted trajectory hits an obstacle, the mean of a
nonlinear function of a Gaussian state, the collision rate of a planner over
random scenarios. The \textbf{Monte Carlo}\index{Monte Carlo estimation}
estimate of $\E[g(\state)]$ from $N$ independent samples $\state^{(1)},
\dots, \state^{(N)}$ drawn from $p$ is
\begin{equation}
...
whatever the dimension of $\state$: four times as many samples halve the
error. For a probability $\Prob(\state \in \mathcal{R})$ take $g$ the indicator
of $\mathcal{R}$; then $\Var(g) = p(1-p)$ and the standard error is
$\sqrt{\hat{p}(1-\hat{p})/N}$, the error bar to attach to a collision rate
estimated from $N$ random scenarios (\cref{ch:ch25}). To draw from
$\Normal(\vect{\mu}, \mat{\Sigma})$, draw $\vect{\xi} \sim \Normal(\vect{0},
\mat{I})$ entry by entry and set $\state = \vect{\mu} + \mat{L}\vect{\xi}$ with
the Cholesky factor $\mat{L}\mat{L}\T = \mat{\Sigma}$; this is
\cref{thm:appB-affine} in reverse. When samples can only be drawn from a
different density $q$, \textbf{importance sampling}\index{importance sampling}
corrects the estimate with weights $w^{(i)} \propto
p(\state^{(i)})/q(\state^{(i)})$, normalised to sum to one; the particle
filter of \cref{ch:ch19} is a sequential version of this idea in which $q$ is
the motion model and the weights are measurement likelihoods
\cite{doucet2001sequential}.

\begin{example}[Mass inside the $2\sigma$ ellipse]
\label{ex:appB-montecarlo}
Drawing $N = 10\,000$ samples from the Gaussian of \cref{ex:appB-conditioning} with the Cholesky factor of \cref{ex:appB-eigen} and counting those with $d^2 \le 4$ gives $\hat{p} = 0.8695$, with standard error $\sqrt{\hat{p}(1-\hat{p})/N} = 0.0034$. The exact value $1 - e^{-2} = 0.8647$ lies within $1.5$ standard errors of the estimate.
\end{example}
...
\gamma$, where $\gamma$ is the quantile of $\chi^2_n$ at the chosen
confidence $p$; a true measurement is then wrongly rejected with probability
$1 - p$, and a detection that belongs to another target or to clutter is
rejected whenever it lands outside the ellipsoid
\cite{barshalom2001estimation}. \Cref{tab:appB-chi2} gives the thresholds
for position measurements in the plane ($n = 2$) and in space ($n = 3$), the
gate of \cref{ch:ch18,ch:ch19}; the same quantiles size the
uncertainty-inflated constraints of \cref{ch:ch24}, where an obstacle whose
position is known only as a Gaussian is enlarged to the ellipse that holds a
required probability mass.

\begin{table}[tb]
  \centering\small
  \caption{Gating thresholds $\gamma$ with $\Prob(\chi^2_n \le \gamma) = p$. A measurement whose squared Mahalanobis distance exceeds $\gamma$ is rejected; for $n = 2$ the values are $-2\ln(1-p)$.}
  \label{tab:appB-chi2}
  \begin{tabular}{@{}lccc@{}}
  \toprule
  $p$ & $n = 1$ & $n = 2$ & $n = 3$\\
...
Half-spaces $\set{\vect{x} : \vect{a}\T\vect{x} \le b}$, balls, ellipsoids and
affine subspaces are convex, and the intersection of any family of convex
sets is convex; so the set of velocities allowed by all \orca half-planes in
\cref{ch:ch13} is a convex polygon, whereas the set of collision-free
velocities outside a velocity obstacle of \cref{ch:ch12} is not convex
(\cref{fig:appB-convex}). Linear functions, norms, $\norm{\vect{x} -
\vect{c}}^2$ and $\vect{x}\T\mat{A}\vect{x}$ with $\mat{A} \succeq \mat{0}$
are convex; sums with non-negative weights and pointwise maxima of convex
functions are convex; and the sublevel sets $\set{\vect{x} : f(\vect{x}) \le
\alpha}$ of a convex function are convex sets. What makes convexity the
dividing line of optimization is the following fact.
...
iff $\nabla^2 f(\vect{x}) \succeq \mat{0}$ for all $\vect{x}$, and $\nabla^2 f
\succ \mat{0}$ everywhere implies strict convexity
...
\cref{ch:ch15}, which add non-convex repulsive terms to a convex attractive
term, can trap a drone in a local minimum: \cref{thm:appB-local-global} simply
...
at the optimum the negative gradient of the cost, the direction in which one
would like to move, is balanced by the normals of the \emph{active}
constraints, those with $g_i(\vect{x}^*) = 0$, and by \cref{eq:appB-kkt-cs}
only those can carry a positive multiplier. The multiplier $\lambda_i^*$
measures how much the optimal cost would drop if constraint $i$ were relaxed
by one unit. This is the theory behind the soft constraints of \cref{ch:ch21}:
replacing a hard constraint by a slack variable with a linear penalty
reproduces the hard constraint exactly, whenever it is feasible, as soon as the
penalty weight exceeds the multiplier \cite[Chapter~17]{nocedal2006numerical}.
With equality constraints only, the conditions reduce to the classical
...
\begin{equation}
  ...
  \label{eq:appB-lp}
\end{equation}
Equality constraints, sign constraints $\vect{z} \ge \vect{0}$ and maximisation are all reduced to this form by writing $\vect{a}\T\vect{z} = b$ as two inequalities, $z_j \ge 0$ as $-z_j \le 0$ and $\max \vect{c}\T\vect{z}$ as $\min (-\vect{c})\T\vect{z}$.
\end{definition}

An LP is convex (a linear objective is convex and $P$ is an intersection of
half-spaces), so \cref{thm:appB-kkt} applies with $g_i(\vect{z}) =
\vect{a}_i\T\vect{z} - b_i$ and $\nabla g_i = \vect{a}_i$: at an optimum,
$-\vect{c}$ is a non-negative combination of the normals of the active
constraints. In two dimensions this is a picture (\cref{fig:appB-lp}): $P$ is
a polygon, the level lines $\vect{c}\T\vect{z} = \text{const}$ are parallel
lines, and sliding a level line in the direction $-\vect{c}$ as far as $P$
allows ends at a vertex, or along a whole edge when the level lines are
parallel to it.
...
A proof is in \textcite[Chapter~13]{nocedal2006numerical}. The theorem is
what the simplex method exploits: it walks from vertex to adjacent vertex,
improving the objective at every step, and stops when no neighbour is better,
which by convexity is a global optimum. Interior-point methods approach the
optimum through the inside of $P$ instead and are polynomial in the worst
case; \cref{ch:ch22} calls both kinds of solver through \code{scipy.optimize},
and the branch-and-bound method of \cref{sec:appB-milp} solves a sequence of
LPs of this form. The velocity choice of \orca in \cref{ch:ch13} is the same
geometry: the velocity closest to the preferred one inside an intersection of
half-planes, which that chapter finds with a small linear-programming
algorithm that adds one half-plane at a time.
...
\cite{nocedal2006numerical}, and by operator-splitting methods such as ADMM,
which alternate cheap projections onto the constraints with linear solves.
...
\end{enumerate}
The method terminates because every branch removes a slice of the relaxed
region that contains no integer point, and it is exact because a subproblem
is pruned only when it provably contains nothing better than the incumbent.
Its running time is exponential in the worst case but often small in
practice, which is the trade-off that \cref{ch:ch22} explores.

\begin{example}[Branch and bound by hand]
\label{ex:appB-bnb}
Maximise $5z_1 + 4z_2$ subject to $6z_1 + 4z_2 \le 24$, $z_1 + 2z_2 \le 6$, $z_1, z_2 \ge 0$ and integer. The relaxation (node~1 of \cref{fig:appB-bnb}) has the optimum $(3, 1.5)$ with value $21$, so no integer solution can exceed $21$. Branching on $z_2$ gives node~2 ($z_2 \le 1$), whose relaxation is $(3.33, 1)$ with value $20.67$, and node~3 ($z_2 \ge 2$), whose relaxation is $(2, 2)$ with value $18$. Branching node~2 on $z_1$ gives node~4 ($z_1 \le 3$) with the integral optimum $(3, 1)$ of value $19$, which becomes the incumbent, and node~5 ($z_1 \ge 4$), whose relaxation $(4, 0)$ is integral with value $20$ and replaces it. Node~3 is now pruned: its bound $18$ cannot beat $20$, so none of its integer points needs to be examined. The optimum is $(4, 0)$ with value $20$, which an enumeration of all feasible integer points confirms; rounding the relaxed optimum to $(3, 2)$ would have been infeasible and rounding it to $(3, 1)$ suboptimal.
\end{example}

\begin{figure}[tb]
  \centering
  \inputfigure{appB/branch-and-bound}
  \caption{The branch-and-bound tree of \cref{ex:appB-bnb}. Each node shows the optimum of its LP relaxation and the bound it provides; integral solutions are green, the pruned node is grey. Node~3 is never branched because its bound $18$ is below the incumbent $20$ found at node~5.}
  \label{fig:appB-bnb}
\end{figure}
...
one face. The value of $M$ matters: too small and feasible positions are cut
off; too large and the LP relaxation becomes so loose that branch and bound
has almost nothing to prune, on top of the numerical trouble of mixing
coefficients of very different sizes.
...
controller of \cref{ch:ch15} is gradient descent on the potential $U$, with
the motion of the drone playing the role of the iteration; its oscillations in
narrow corridors are the zigzag of \cref{fig:appB-gd}, and its local minima
are the non-convexity of \cref{fig:appB-convex}. The training of the
prediction networks of \cref{ch:ch20} is gradient descent on a loss averaged
over data, with the gradient computed by the chain rule \cref{eq:appB-chain}
and estimated on random mini-batches (\emph{stochastic} gradient descent);
the learning rate is the $\alpha$ above, and the momentum and adaptive step
sizes of optimisers such as Adam are devices against exactly the
...
predecessors $\Pred(u)$. A path $\pi = (v_0, v_1, \dots, v_m)$ follows edges
from $v_0$ to $v_m$ and costs $\cost(\pi) = \sum_{i} c(v_{i}, v_{i+1})$; the
\textbf{shortest-path distance}\index{shortest-path distance} $\dist(u, v)$
is the least cost of a path from $u$ to $v$, or $\infty$ if there is none,
and it satisfies the triangle inequality $\dist(u, w) \le \dist(u, v) +
\dist(v, w)$. \Cref{ch:ch03} computes $\dist(s, \cdot)$ from a start $s$ and
writes it $\delta(s, v)$; \cref{ch:ch04} keeps for every node the cost
$\gcost(n)$ of the best path found so far, a heuristic estimate $\hcost(n)$
of $\dist(n, \gamma)$ to the goal $\gamma$, and the priority $\fcost = \gcost
+ \hcost$, and stores the frontier in \Open and the expanded nodes in
\Closed. A 4- or 8-connected grid is such a graph with $\abs{E} \le
4\abs{V}$ or $8\abs{V}$ edges, and a space-time graph
(\cref{ch:ch04,ch:ch08}) has the states $(v, t)$ for $t = 0, \dots, T$, so
$(T + 1)\abs{V}$ of them, with a wait edge from $(v, t)$ to $(v, t+1)$
alongside the move edges. The undirected Laplacian graph above and this
directed cost graph are the same object seen by different chapters: the
consensus chapter cares about who can talk to whom, the search chapters about
...
Big-O is an upper bound up to a constant factor and for large inputs;
$\Theta$ says the bound is tight. Constants and lower-order terms are dropped
($3n^2 + 50n = \Theta(n^2)$), the base of a logarithm is irrelevant, and the
usual ladder is $1 \prec \log n \prec n \prec n\log n \prec n^2 \prec 2^n
\prec n!$. A bound describes the algorithm's worst case unless it says
\emph{expected} (average over the random choices of a hash function or of the
algorithm) or \emph{amortised} (average over a sequence of operations). Read
a bound with the right variables: Dijkstra's algorithm with a binary heap runs
in $\bigO{(\abs{V} + \abs{E})\log\abs{V}}$ (\cref{ch:ch03}), which on a grid
with $\abs{E} = \Theta(\abs{V})$ is $\bigO{\abs{V}\log\abs{V}}$; a space-time
search over $T$ steps has $(T+1)\abs{V}$ states in place of $\abs{V}$; and the
exponential worst case of the multi-agent solvers of \cref{ch:ch09,ch:ch10}
is in the number of agents, not in the size of the map, which is what makes
them practical for a swarm on a large map and hopeless for a large swarm in a
small room.

\subsection{Binary heaps}

A \textbf{binary heap}\index{binary heap} stores $n$ keys in an array
$a[0], \dots, a[n-1]$ that is read as a complete binary tree: the children of
position $i$ are $2i+1$ and $2i+2$, its parent is $\lfloor (i-1)/2 \rfloor$,
and the \emph{heap property} says that every key is no larger than its
children's, so $a[0]$ is the minimum. The tree has height $\lfloor \log_2 n
\rfloor$. Inserting appends the key at the end and swaps it upwards while it
is smaller than its parent (\emph{sift-up}); extracting the minimum moves the
last key to the root and swaps it downwards with its smaller child while the
property is violated (\emph{sift-down}); each is a walk along one root-to-leaf
path, hence $\bigO{\log n}$. Building a heap from an unsorted array by
sifting down from the last internal node upwards costs $\bigO{n}$, not
$\bigO{n\log n}$ \cite[Chapter~6]{cormen2009clrs}. \Cref{tab:appB-heap}
collects the costs of the operations a search needs, together with those of
the \python containers that this book's code uses.
...
\begin{table}[tb]
  ...
  binary heap & decrease-key & $\bigO{\log n}$ & push a duplicate, skip stale entries\\
  hash table & insert, lookup, delete & $\bigO{1}$ expected & \code{dict}, \code{set}\\
  dynamic array & append; index & $\bigO{1}$ amortised; $\bigO{1}$ & \code{list.append}, \code{a[i]}\\
  dynamic array & pop from the front; membership & $\bigO{n}$ & \code{list.pop(0)}, \code{x in a}\\
...
The keys must be \emph{hashable}, which in \python means immutable: tuples of
...
and keeps the geometry in separate arrays.

The \code{heapq} module implements a binary heap on a plain list, but no
decrease-key. The standard idiom, used throughout \cref{ch:ch03,ch:ch04}
and inherited by every planner built on them, is \emph{lazy deletion}: when a
better cost is found for a node, a new entry is pushed and the old one is left
in the heap, to be recognised as stale and skipped when it surfaces
(\cref{lst:appB-heapq}). The heap then holds at most one entry per relaxation,
$\bigO{\abs{E}}$ in total, which does not change the asymptotic bound of
Dijkstra's algorithm or \astar.

\begin{lstlisting}[caption={Best-first search with \code{heapq} and lazy deletion.},label={lst:appB-heapq}]
import heapq
import itertools

def best_first(start, goal, succ, h):
    """A* skeleton; succ(n) yields (neighbour, edge cost) pairs."""
    tie = itertools.count()       # breaks ties without comparing nodes
    g = {start: 0.0}              # best cost found so far, a dict
    open_heap = [(h(start), 0.0, next(tie), start)]
    closed = set()
    while open_heap:
        f, neg_g, _, n = heapq.heappop(open_heap)
        if n in closed:           # a stale duplicate: skip it
            continue
        closed.add(n)
        if n == goal:
            return -neg_g
        for m, c in succ(n):
            ...
            if g_new < g.get(m, float("inf")):
                g[m] = g_new      # "decrease-key" = push a fresh entry
                heapq.heappush(open_heap, (g_new + h(m), -g_new, next(tie), m))
    return None
\end{lstlisting}

\begin{pitfall}[Heap entries must be comparable]
\code{heapq} orders tuples lexicographically, so when two entries have the same $\fcost$ and the same $\gcost$ it compares the next field. If that field is the node itself and nodes are dictionaries or objects without an ordering, the search crashes at the first tie; if it is a NumPy array, it crashes at the first comparison. The counter in \cref{lst:appB-heapq} guarantees that the comparison never reaches the node and, as a bonus, makes the tie-breaking deterministic and first-in-first-out. The entry $-\gcost$ before it implements the preference for larger $\gcost$ among equal $\fcost$ that \cref{ch:ch04} recommends.
\end{pitfall}
...
\begin{summary}
\begin{itemize}
  \item A symmetric matrix is a scaling along orthogonal axes, $\mat{A} = \mat{Q}\mat{\Lambda}\mat{Q}\T$; it is PSD iff all $\lambda_i \ge 0$, and every covariance matrix is PSD. Block matrices are handled with the Schur complement, which gives the block inverse, the block determinant and the Woodbury identity.
  \item Least squares is solved by the normal equations $\mat{A}\T\mat{A}\vect{x} = \mat{A}\T\vect{b}$; never form an inverse to do it.
  \item Linearisation replaces a nonlinear map by its Jacobian with an error of second order in the displacement; the double-integrator step $\pos' = \pos + \vel\dt + \tfrac12\acc\dt^2$, $\vel' = \vel + \acc\dt$ is exact for a piecewise-constant input.
  \item Covariances propagate as $\mat{A}\mat{\Sigma}\mat{A}\T$ under any linear map; Gaussians stay Gaussian under affine maps, marginalisation and conditioning, with $\vect{\mu}_{a|b} = \vect{\mu}_a + \mat{\Sigma}_{ab}\mat{\Sigma}_{bb}\inv(\state_b - \vect{\mu}_b)$ and $\mat{\Sigma}_{a|b} = \mat{\Sigma}_{aa} - \mat{\Sigma}_{ab}\mat{\Sigma}_{bb}\inv\mat{\Sigma}_{ba}$, which is the Kalman update in disguise.
  \item A filter is total probability (predict) followed by Bayes' rule (update); Monte Carlo estimates have error $\bigO{1/\sqrt{N}}$; the squared Mahalanobis distance of a Gaussian vector is $\chi^2_n$, which sets gating thresholds such as $5.991$ for $n = 2$ at $95\,\%$.
  \item Convexity makes local minima global and the KKT conditions sufficient. An LP attains its optimum at a vertex; a convex QP is an LP with a quadratic cost; a MILP is solved exactly by branch and bound on LP relaxations, with big-$M$ binaries encoding disjunctions; gradient descent converges for $\alpha < 2/\lambda_{\max}$ and crawls when the problem is ill-conditioned.
  \item The Laplacian $\mat{L} = \mat{D} - \mat{A}$ is PSD with $\mat{L}\vect{1} = \vect{0}$; its second eigenvalue is positive iff the graph is connected and sets the speed of consensus; $\lambda_n \le 2d_{\max}$ bounds the safe step size.
...
of \cref{sec:appB-linalg}; the algorithms of optimization, from gradient
descent to interior-point and active-set methods, are in
\textcite{nocedal2006numerical}; integer programming and branch and bound
are in \textcite{wolsey1998integer}. The probabilistic material follows
\textcite{thrun2005probabilistic} and \textcite{sarkka2013bayesian}, which
derive the Kalman filter and its nonlinear relatives from the Gaussian
identities of \cref{tab:appB-gaussian}; gating and data association are
...
\textcite{fiedler1973algebraic}. Heaps, hashing and asymptotic notation are
in \textcite{cormen2009clrs}.
...
\begin{exercise}[\difficulty{1}]
\label{exr:appB-gate}
Compute the gating threshold for $n = 2$ at the confidence $p = 0.9$, and explain what happens to the number of wrongly rejected true measurements when a filter underestimates its innovation covariance $\mat{S}$.
\end{exercise}

\begin{exercise}[\difficulty{2}]
\label{exr:appB-lp}
Replace the objective of \cref{ex:appB-lp} by $2z_1 + 2z_2$. Find all optimal solutions, explain why there is more than one, and give the KKT multipliers at the vertex $(3, 1)$.
\end{exercise}

\begin{exercise}[\difficulty{2}]
\label{exr:appB-cycle}
Write down the Laplacian of the 4-cycle (edges $\set{1,2}$, $\set{2,3}$, $\set{3,4}$, $\set{4,1}$), show that its eigenvalues are $0, 2, 2, 4$, and give the largest safe step size $\eps$ of the discrete consensus iteration.
\end{exercise}

\begin{exercise}[\difficulty{3} Coding]
\label{exr:appB-coding}
Starting from \code{code/figures/gen\_appB\_numbers.py}, draw $N = 10\,000$ samples from the Gaussian of \cref{ex:appB-conditioning}, estimate the mean and the covariance from the samples, and count the fraction of samples whose squared Mahalanobis distance exceeds the $95\,\%$ gate of \cref{tab:appB-chi2}. Then perturb the covariance used in the gate by $\pm 30\,\%$ and report how the rejection rate changes.
\end{exercise}
