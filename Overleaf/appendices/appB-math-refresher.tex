...
this appendix: the global planner (\cref{ch:ch09,ch:ch10}) on the graphs,
heaps and hash tables of \cref{sec:appB-graphs,sec:appB-complexity}; the
prediction layer (\cref{ch:ch18,ch:ch19,ch:ch20}) on the Gaussian algebra and
gating of \cref{sec:appB-probability} and the gradient descent of
\cref{sec:appB-gd}; the local layer (\cref{ch:ch13,ch:ch14}) on the convex
geometry of half-planes and the double-integrator step; the control and
formation layers (\cref{ch:ch21,ch:ch22,ch:ch23}) on quadratic and
mixed-integer programs, KKT multipliers and the Laplacian spectrum. When a
chapter sends you here, read only the section it names and return.
\end{dronebox}
