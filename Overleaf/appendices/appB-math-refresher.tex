...
\begin{itemize}
  ...
  \item linearize a nonlinear map with its Jacobian, bound the error, and step a double integrator forward exactly;
...
\begin{keyidea}
Four moves carry almost every derivation in this book: diagonalize a symmetric
matrix ($\mat{A} = \mat{Q}\mat{\Lambda}\mat{Q}\T$), linearize a nonlinear map
...
is positive exactly when $\vect{b}$ lies counterclockwise from $\vect{a}$:
...
\begin{table}[tb]
  ...
  $\mat{A} = \mat{L}\mat{L}\T$ \ ($\mat{A} \succ \mat{0}$, $\mat{L}$ lower triangular) & Cholesky factorization\\
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item $\mat{A} \succeq \mat{0}$ iff $\mat{A} = \mat{B}\T\mat{B}$ for some $\mat{B}$. If $\mat{A} \succ \mat{0}$, $\mat{B}\T$ can be chosen lower triangular with positive diagonal, $\mat{A} = \mat{L}\mat{L}\T$ (the \textbf{Cholesky factorization}\index{Cholesky factorization}), and this $\mat{L}$ is unique.
...
form is an ellipsoid with axes $\vect{q}_i$ and semi-axes $1/\sqrt{\lambda_i}$;
the covariance ellipse of a Gaussian in \cref{sec:appB-probability} is the
level set of the form built from $\mat{\Sigma}\inv$, so its semi-axes are
$\sqrt{\lambda_i(\mat{\Sigma})}$.
...
\begin{theorem}[Block factorization, determinant and inverse]
...
Let $\mat{A} \in \R^{m \times n}$ and $\vect{b} \in \R^m$. A vector $\vect{x}^*$ minimizes $\norm{\mat{A}\vect{x} - \vect{b}}^2$ iff it satisfies the \textbf{normal equations}\index{least squares}\index{normal equations}
\begin{equation}
  \mat{A}\T\mat{A}\,\vect{x}^* = \mat{A}\T\vect{b}.
  \label{eq:appB-normal}
\end{equation}
If $\mat{A}$ has full column rank $n$, then $\mat{A}\T\mat{A} \succ \mat{0}$ and the minimizer is unique, $\vect{x}^* = (\mat{A}\T\mat{A})\inv\mat{A}\T\vect{b}$. For a weight matrix $\mat{W} \succ \mat{0}$ the minimizer of $(\mat{A}\vect{x} - \vect{b})\T\mat{W}(\mat{A}\vect{x} - \vect{b})$ solves $\mat{A}\T\mat{W}\mat{A}\,\vect{x}^* = \mat{A}\T\mat{W}\vect{b}$.
\end{theorem}
\begin{proof}
With $\vect{r} = \mat{A}\vect{x}^* - \vect{b}$ and $\vect{d} = \vect{x} - \vect{x}^*$, $\norm{\mat{A}\vect{x} - \vect{b}}^2 = \norm{\vect{r}}^2 + 2\vect{d}\T\mat{A}\T\vect{r} + \norm{\mat{A}\vect{d}}^2$. If \cref{eq:appB-normal} holds, the middle term vanishes for every $\vect{d}$ and $\vect{x}^*$ is a minimizer; if not, $\vect{d} = -t\mat{A}\T\vect{r}$ with a small $t > 0$ makes the last two terms sum to a negative number. Full column rank gives $\norm{\mat{A}\vect{d}}^2 > 0$ for $\vect{d} \ne \vect{0}$, hence uniqueness; the weighted case is the unweighted one for $\mat{W}^{1/2}\mat{A}$ and $\mat{W}^{1/2}\vect{b}$, with $\mat{W}^{1/2} = \mat{Q}\mat{\Lambda}^{1/2}\mat{Q}\T$ from \cref{thm:appB-spectral}. \qedhere
\end{proof}
...
The formulas above are written with $\mat{A}\inv$ because that is how they are proved, not how they should be computed. Solving $\mat{A}\vect{x} = \vect{b}$ with \code{numpy.linalg.solve}, or with a Cholesky factor when $\mat{A} \succ \mat{0}$, is faster and more accurate than forming \code{numpy.linalg.inv(A)} and multiplying. Two more habits pay off in \cref{ch:ch18,ch:ch19}: re-symmetrize a covariance as $\tfrac12(\mat{P} + \mat{P}\T)$ after each update, and check its smallest eigenvalue before a Cholesky factorization, because rounding can turn a PSD matrix into one with a slightly negative eigenvalue.
\end{pitfall}
...
propagated backward through its layers. The derivatives that recur in this
book are
\begin{equation}
  \begin{aligned}
...
$\nabla^2(\tfrac12\vect{x}\T\mat{A}\vect{x}) = \mat{A}$. The fourth expression
holds only for $\vect{x} \ne \vect{c}$, because $\norm{\vect{x} - \vect{c}}$ is
not differentiable at its own center. The attractive
...
from the obstacle---a formula that the code must guard at zero clearance,
...
\textbf{linearization}\index{linearization}. The extended Kalman filter of
...
a_{\max}\dt$, its dynamic window. The explicit Euler step $\pos_{k+1} = \pos_k + \vel_k\dt$ drops the term $\tfrac12\acc\dt^2$ (with $\dt = 0.1$\,s, $v = 2$\,m/s and $a = 1$\,m/s$^2$: $0.200$\,m instead of the exact $0.205$\,m, at $2.1$\,m/s afterward), an error that accumulates over a trajectory.
...
is centered at $\vect{\mu}$, its axes are the eigenvectors $\vect{q}_i$ and its
...
\mat{0}$ has no density; \cref{thm:appB-affine} still holds for it, because a
degenerate Gaussian is defined by its mean and covariance through its
characteristic function, while the conditioning formula of
\cref{thm:appB-conditioning} needs $\mat{\Sigma}_{bb} \succ \mat{0}$ (or the
Moore--Penrose pseudo-inverse $\mat{\Sigma}_{bb}^{+}$ in place of
$\mat{\Sigma}_{bb}\inv$).
...
Let $\state = (\state_a, \state_b)$ be jointly Gaussian with a non-singular covariance,
\begin{equation}
...
\end{equation*}
and the matrix in the second term is the Schur complement of $\mat{\Lambda}_{aa}$ in $\mat{\Lambda}$, which by \cref{thm:appB-block} applied to $\mat{\Lambda}$ (whose inverse is $\mat{\Sigma}$) equals $\mat{\Sigma}_{bb}\inv$. Splitting the normalizing constant with $\det\mat{\Sigma} = \det\mat{S}\det\mat{\Sigma}_{bb}$ and $(2\pi)^{n} = (2\pi)^{n_a}(2\pi)^{n_b}$ gives the exact factorization
\begin{equation}
...
Let $\state \sim \Normal(\vect{\mu}, \mat{P})$ with $\mat{P} \succ \mat{0}$, and let $\meas = \mat{H}\state + \vect{v}$ with $\vect{v} \sim \Normal(\vect{0}, \mat{R})$, $\mat{R} \succ \mat{0}$, independent of $\state$ (so the joint covariance below is non-singular, as \cref{thm:appB-conditioning} requires). Then $(\state, \meas)$ is jointly Gaussian with $\Cov(\meas) = \mat{H}\mat{P}\mat{H}\T + \mat{R} =: \mat{S}$ and $\Cov(\state, \meas) = \mat{P}\mat{H}\T$, and
\begin{equation}
...
\Cref{tab:appB-gaussian} collects these identities in the form in which
\cref{ch:ch18,ch:ch19} use them.
...
\end{equation*}
Before the observation $x \sim \Normal(2, 2.0)$, with standard deviation $1.414$; afterward $x \mid y \sim \Normal(3.2, 0.56)$, with standard deviation $0.748$. The positive correlation $\rho = 1.2/\sqrt{2.0 \cdot 1.0} = 0.85$ means that a $y$ above its mean shifts the belief about $x$ upward, and the variance shrinks by the factor $1 - \rho^2 = 0.28$. \Cref{fig:appB-conditioning} shows the ellipses, the slice $y = 2$ and both densities.
\end{example}

\begin{figure}[tb]
  \centering
  \inputfigure{appB/gaussian-conditioning}
  \caption{Conditioning the Gaussian of \cref{ex:appB-conditioning}. The $1\sigma$ and $2\sigma$ ellipses of the joint density are tilted along the eigenvector $\vect{q}_1$ of $\mat{\Sigma}$. Observing $y = 2$ restricts the density to the dashed slice; along it, the conditional density of $x$ (orange) is centered at $3.2$, to the right of the prior mean $2$, and is narrower than the marginal density of $x$ (blue, drawn along the bottom) because knowing $y$ removes the part of the uncertainty of $x$ that is correlated with $y$.}
...
over $\state$ gives the law of \textbf{total probability}\index{total probability}, $p(\meas) = \int p(\meas \mid \state)p(\state)\,\mathrm{d}\state$.
...
discrete events the integrals are sums. For example, a detector that flags an intruder with probability $0.9$ when one is present and $0.2$ when none is, on a day with intruders present $10\,\%$ of the time, raises the probability of an intruder on a flag only to $0.09/(0.09 + 0.18) = 1/3$: false alarms on the common case outnumber true detections on the rare one.
...
approximates the two integrals by linearization, by sigma points or by
...
factor $\mat{L}\mat{L}\T = \mat{\Sigma}$ (\cref{thm:appB-affine} with
$\mat{A} = \mat{L}$ and $\vect{b} = \vect{\mu}$). Drawing $N = 10\,000$ samples this way from the Gaussian of \cref{ex:appB-conditioning} and counting those with $d^2 \le 4$ gives $\hat{p} = 0.8695$ with standard error $0.0034$, within $1.5$ standard errors of the exact value $1 - e^{-2} = 0.8647$ of \cref{thm:appB-chi2} below.
...
p(\state^{(i)})/q(\state^{(i)})$, normalized to sum to one; the particle
...
rejected whenever it lands outside the ellipsoid \cite{barshalom2001estimation}.
This is the gate of \cref{ch:ch18,ch:ch19}; the same quantiles size the
uncertainty-inflated constraints of \cref{ch:ch24}, where an obstacle known
only as a Gaussian is enlarged to the ellipse that holds a required
probability mass.
...
Throughout this section $\vect{z}$ is the vector of decision variables of a
mathematical program, not the measurement vector of
\cref{sec:appB-probability}, which is written with the same bold letter.
...
Let $f$ be convex on a convex set $\mathcal{C}$ and let $\vect{x}^*$ be a local minimizer: $f(\vect{x}^*) \le f(\vect{x})$ for all $\vect{x} \in \mathcal{C}$ within some distance $\eps > 0$ of $\vect{x}^*$. Then $f(\vect{x}^*) \le f(\vect{x})$ for all $\vect{x} \in \mathcal{C}$. If moreover $\mathcal{C} = \R^n$ and $f$ is differentiable, then $\vect{x}^*$ is a global minimizer iff $\nabla f(\vect{x}^*) = \vect{0}$; on a general convex $\mathcal{C}$ the first-order condition is instead $\nabla f(\vect{x}^*)\T(\vect{x} - \vect{x}^*) \ge 0$ for all $\vect{x} \in \mathcal{C}$.
\end{proposition}
\begin{proof}
Suppose $f(\vect{y}) < f(\vect{x}^*)$ for some $\vect{y} \in \mathcal{C}$. The points $\vect{x}_\theta = \theta\vect{y} + (1-\theta)\vect{x}^*$ lie in $\mathcal{C}$ and satisfy $f(\vect{x}_\theta) \le \theta f(\vect{y}) + (1-\theta)f(\vect{x}^*) < f(\vect{x}^*)$ for every $\theta \in (0, 1]$; for $\theta$ small enough $\vect{x}_\theta$ is within $\eps$ of $\vect{x}^*$, contradicting local minimality. For the second claim, a differentiable convex function satisfies $f(\vect{y}) \ge f(\vect{x}) + \nabla f(\vect{x})\T(\vect{y} - \vect{x})$ for all $\vect{x}, \vect{y}$, that is, its graph lies above its tangent planes \cite[Section~3.1]{boyd2004convex}. On $\mathcal{C} = \R^n$ a zero gradient therefore makes $\vect{x}$ a global minimizer, and conversely an unconstrained minimizer of a differentiable function has zero gradient. On a general convex $\mathcal{C}$ the same inequality gives $f(\vect{x}) \ge f(\vect{x}^*) + \nabla f(\vect{x}^*)\T(\vect{x} - \vect{x}^*) \ge f(\vect{x}^*)$ whenever the stated condition holds; and if $\nabla f(\vect{x}^*)\T(\vect{x} - \vect{x}^*) < 0$ for some $\vect{x} \in \mathcal{C}$, then $f$ decreases along the segment from $\vect{x}^*$ toward $\vect{x}$, which lies in $\mathcal{C}$, so $\vect{x}^*$ is not optimal \cite[Section~4.2]{boyd2004convex}. The gradient need not vanish at a constrained optimum: $f(x) = x$ on $\mathcal{C} = [0, 1]$ is minimized at $x^* = 0$ with $\nabla f(x^*) = 1$. \Cref{fig:appB-lp} draws the constrained condition for a linear program: at the optimal vertex $-\vect{c}$ lies in the cone of the normals of the active constraints, so no feasible direction decreases the objective. That example \emph{maximizes}, so the arrow shown inside the cone of the active normals is $+\vect{c}$, which is the same statement with the sign of the objective flipped. \qedhere
\end{proof}
...
\label{thm:appB-kkt}
If $\vect{x}^*$ is a local minimizer of \cref{eq:appB-nlp} and the constraints satisfy a constraint qualification at $\vect{x}^*$ (for instance, the gradients of the active constraints are linearly independent, or all constraints are affine), then there exist multipliers $\vect{\lambda}^* \in \R^m$ and $\vect{\nu}^* \in \R^p$ with\index{KKT conditions}
\begin{align}
...
\end{align}
If $f$ and the $g_i$ are convex and the $h_j$ affine, these conditions are also sufficient: every point that satisfies them is a global minimizer.
\end{theorem}

The proof is in \textcite[Chapter~12]{nocedal2006numerical} and, for the
convex case, in \textcite[Chapter~5]{boyd2004convex}. At the optimum the
negative gradient of the cost is balanced by the normals of the \emph{active}
constraints, those with $g_i(\vect{x}^*) = 0$, which by \cref{eq:appB-kkt-cs}
are the only ones that can carry a positive multiplier; $\lambda_i^*$ measures
how much the optimal cost would drop if constraint $i$ were relaxed by one
unit. This is the theory behind the soft constraints of \cref{ch:ch21}: a slack
variable with a linear penalty reproduces the hard constraint exactly, whenever
it is feasible, as soon as the penalty weight exceeds the multiplier
\cite[Chapter~17]{nocedal2006numerical}. With equality constraints only, the
conditions reduce to the classical Lagrange multipliers.
...
\label{def:appB-lp}
A \textbf{linear program}\index{linear program} (LP) minimizes a linear function over a polyhedron:
\begin{equation}
  ...
  \label{eq:appB-lp}
\end{equation}
Equality constraints, sign constraints $\vect{z} \ge \vect{0}$ and maximization are all reduced to this form by writing $\vect{a}\T\vect{z} = \beta$ as two inequalities, $z_j \ge 0$ as $-z_j \le 0$ and $\max \vect{c}\T\vect{z}$ as $\min (-\vect{c})\T\vect{z}$. Simplex solvers and most textbooks use instead the \textbf{standard form}\index{linear program!standard form} $\min \vect{c}\T\vect{z}$ subject to $\mat{A}\vect{z} = \vect{b}$, $\vect{z} \ge \vect{0}$, reached from \cref{eq:appB-lp} by adding a \emph{slack} variable $s_i \ge 0$ to every inequality, $\vect{a}_i\T\vect{z} + s_i = b_i$, and by splitting a free variable into $z_j = z_j^+ - z_j^-$ with $z_j^\pm \ge 0$; \code{scipy.optimize.linprog} accepts inequalities, equalities and bounds directly.
\end{definition}
...
the level lines are parallel to it (\cref{fig:appB-lp} maximizes, so there the
level line slides in the direction $+\vect{c}$).

\begin{theorem}[Vertex optimality]
\label{thm:appB-vertex}
Every LP of the form \cref{eq:appB-lp} either is infeasible ($P = \emptyset$), or is unbounded ($\vect{c}\T\vect{z} \to -\infty$ on $P$), or attains its optimum. In the last case, if $P$ has at least one vertex---which holds in particular for the standard form with $\vect{z} \ge \vect{0}$---then some vertex of $P$ is optimal.
\end{theorem}

A proof is in \textcite[Chapter~13]{nocedal2006numerical}. The simplex
method walks from vertex to adjacent vertex, never worsening the objective and
improving it away from degenerate pivots, until no neighbor is better, which
by convexity is a global optimum (an anti-cycling rule such as Bland's
...
destroys convexity: the feasible set is a union of polyhedral slices, one for
every assignment of the integer variables---a scatter of isolated points only
when \emph{every} variable is integer---and convex in neither case. The
problem is NP-hard in general. Dropping the integrality
gives the \textbf{LP relaxation}\index{LP relaxation}, whose optimal value is
a bound on the MILP optimum (a lower bound for a minimization, an upper
bound for a maximization), because the relaxation optimizes over a larger
...
\end{enumerate}
The method terminates, for bounded integer variables, because every branch
cuts away a slice of the relaxed region that contains no integer point, and it is exact because a subproblem is
...
condition stays enforced. The value of $M$ matters: too small cuts off
feasible positions, too large makes the LP relaxation so loose that branch and
bound has little to prune, besides the numerical trouble of mixing
...
\label{thm:appB-gd}
Let $f$ be convex and differentiable with an $L$-Lipschitz gradient, $\norm{\nabla f(\vect{x}) - \nabla f(\vect{y})} \le L\norm{\vect{x} - \vect{y}}$, and let $\vect{x}^*$ be a minimizer. With a constant step $\alpha \le 1/L$ the iterates of \cref{eq:appB-gd} satisfy $f(\vect{x}_k) - f(\vect{x}^*) \le \norm{\vect{x}_0 - \vect{x}^*}^2/(2\alpha k)$. If in addition $f$ is $\mu$-strongly convex, $f(\vect{y}) \ge f(\vect{x}) + \nabla f(\vect{x})\T(\vect{y} - \vect{x}) + \tfrac{\mu}{2}\norm{\vect{y} - \vect{x}}^2$ for all $\vect{x}, \vect{y}$ (equivalently $\nabla^2 f \succeq \mu\mat{I}$ when $f$ is twice differentiable), then $\norm{\vect{x}_k - \vect{x}^*} \le (1 - \alpha\mu)^{k}\norm{\vect{x}_0 - \vect{x}^*}$ for $\alpha \le 1/L$: the error shrinks geometrically, at a rate governed by the condition number $L/\mu$.
\end{proposition}
...
$\alpha < 2/\lambda_{\max}$, and the slowest coordinate is the one with the
smallest eigenvalue: a large eigenvalue ratio, a long narrow valley, forces a
small step and a slow crawl along the valley floor.
...
Throughout this section $\mat{A}$ is the adjacency matrix and $\mat{L}$ the
Laplacian of a graph, not the generic matrix of \cref{sec:appB-linalg} or the
Cholesky factor of \cref{thm:appB-psd-tests}, and Laplacian eigenvalues are
numbered in \emph{increasing} order, the reverse of \cref{thm:appB-spectral}.
...
neighbors of $i$ are $N_i = \set{j : \set{i, j} \in E}$.
...
disagreements with its neighbors, which is exactly what a consensus
protocol feeds back.
...
(\mat{I} - \eps\mat{L})\vect{x}_k$ multiplies eigencomponent $i$ by $1 -
...
\label{def:appB-bigO}
For functions $f, g\colon \N \to \R_{\ge 0}$, $f(n) = \bigO{g(n)}$ if there are constants $c > 0$ and $n_0$ with $f(n) \le c\,g(n)$ for all $n \ge n_0$; $f(n) = \Omega(g(n))$ if there are constants $c > 0$ and $n_0$ with $f(n) \ge c\,g(n)$ for all $n \ge n_0$; and $f(n) = \Theta(g(n))$ if there are constants $c_1, c_2 > 0$ and $n_0$ with $c_1\,g(n) \le f(n) \le c_2\,g(n)$ for all $n \ge n_0$, that is, if both bounds hold.\index{big-O notation}
\end{definition}
...
random choices) or \emph{amortized} (average over a sequence of operations),
and it must be read with the right variables: Dijkstra's algorithm with a
binary heap runs in $\bigO{(\abs{V} + \abs{E})\log\abs{V}}$ (\cref{ch:ch03}),
which on a grid with $\abs{E} = \Theta(\abs{V})$ is $\bigO{\abs{V}\log\abs{V}}$;
a space-time search over $T$ steps has $(T+1)\abs{V}$ states; and the
...
$a[0]$ is the minimum. Inserting appends the key and swaps it upward while it
is smaller than its parent (\emph{sift-up}); extracting the minimum moves the
last key to the root and swaps it downward with its smaller child
(\emph{sift-down}); each walks one root-to-leaf path of length $\lfloor \log_2
n \rfloor$, hence $\bigO{\log n}$, and building a heap from an unsorted array
by sifting down from the last internal node costs only $\bigO{n}$
\cite[Chapter~6]{cormen2009clrs}. \Cref{tab:appB-heap} collects the costs of
the operations a search needs, with those of the \python containers used in
the book's code.
...
Keys must be \emph{hashable}, in \python immutable: tuples of integers such as
a cell $(x, y)$ or a space-time state $(x, y, t)$ are ideal, lists and NumPy
arrays are not accepted, and floating-point keys are legal but dangerous
because two values equal on paper may differ in the last bit, which is why the
search code of the book indexes cells by integer tuples.

The \code{heapq} module implements a binary heap on a plain list, but it has
no decrease-key. The standard repair is the lazy-deletion queue that
\cref{ch:ch02} states as \cref{alg:ch02-lazypq} and every planner of the book
calls: a \code{dict} holds the best known key of each node, an improved key is
pushed as a fresh entry $(\fcost, -\gcost, \mathit{counter}, \mathit{node})$
rather than edited in place, and a popped entry whose key is no longer the best
one is discarded. The heap then holds at most one entry per relaxation,
$\bigO{\abs{E}}$ in total, which leaves the asymptotic bounds unchanged.
...
\begin{itemize}
  ...
  \item Linearization keeps the Jacobian and drops an error of second order in the displacement; the double-integrator step $\pos' = \pos + \vel\dt + \tfrac12\acc\dt^2$, $\vel' = \vel + \acc\dt$ is exact for a piecewise-constant input.
  \item Covariances propagate as $\mat{A}\mat{\Sigma}\mat{A}\T$ under any linear map; Gaussians stay Gaussian under affine maps, marginalization and conditioning, and the conditioning formula \cref{eq:appB-conditional} is the Kalman update in disguise.
  ...
  \item A binary heap gives $\bigO{\log n}$ insertion and extraction; \code{heapq} plus a \code{dict}, the lazy deletion of \cref{alg:ch02-lazypq} and a tie-breaking counter is the priority queue of every search in this book.
\end{itemize}
\end{summary}
