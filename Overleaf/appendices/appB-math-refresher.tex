...
Every LP \cref{eq:appB-lp} either is infeasible ($P = \emptyset$), or is unbounded ($\vect{c}\T\vect{z} \to -\infty$ on $P$), or attains its optimum. In the last case, if $P$ has at least one vertex---which holds in particular for the standard form with $\vect{z} \ge \vect{0}$---then some vertex of $P$ is optimal.
\end{theorem}
...
Cholesky factor of \cref{thm:appB-psd-tests}.
