...
\Cref{sec:appB-linalg,sec:appB-calculus} serve the filtering, control and
consensus chapters (\cref{ch:ch18,ch:ch19,ch:ch21,ch:ch23}) and the potential
fields of \cref{ch:ch15}; \cref{sec:appB-probability} serves the tracking and
prediction chapters (\cref{ch:ch18,ch:ch19,ch:ch20});
\cref{sec:appB-optimization} the velocity-space, control and integer-programming
chapters (\cref{ch:ch13,ch:ch15,ch:ch20,ch:ch21,ch:ch22}); and
\cref{sec:appB-graphs,sec:appB-complexity} the search chapters
(\cref{ch:ch03,ch:ch04,ch:ch08,ch:ch09}) and the consensus chapter
(\cref{ch:ch23}). The notation is that of the notation table at the front of
...
Gaussians is Gaussian with block-diagonal covariance, so with $\mat{A} =
[\mat{I}\ \ \mat{I}]$ the sum of independent Gaussians is Gaussian with the
sums of the means and of the covariances, which is how process noise enters
the prediction step of \cref{ch:ch18}; and a linear filter therefore never has
to represent more than a mean and a covariance.
...
\begin{dronebox}
Each layer of the hybrid architecture of \cref{ch:ch24} leans on one part of
this appendix. The global planner (\cbs, \ecbs; \cref{ch:ch09,ch:ch10}) lives
on the graphs, heaps and hash tables of \cref{sec:appB-graphs,sec:appB-complexity}.
The prediction layer (\cref{ch:ch18,ch:ch19,ch:ch20}) is the Gaussian algebra of
\cref{sec:appB-probability}: affine maps for the predict step, conditioning for
the update, chi-square gates for rejecting stray detections, and gradient
descent for its learned models. The local layer (\cref{ch:ch13,ch:ch14}) is the
convex geometry of half-planes and the double-integrator step of
\cref{sec:appB-calculus}. The control and formation layers (\cref{ch:ch21,ch:ch22,ch:ch23})
are quadratic and mixed-integer programs, KKT multipliers and the Laplacian
spectrum. When a chapter's first pages send you here, read only the
section named and return.
\end{dronebox}
