...
Files written (whitespace separated, one header row, ASCII only):
  ...
  figures/data/ch25-study-levels.dat   the two levels the mini-study leaves
                                       at their base value, run on the (4,1)
                                       cell with the same 20 seeds: the
                                       formation-preserving level of the
                                       constraints factor (Eq. 24.6 on or
                                       off) and the learned level of the
                                       prediction factor (the Chapter 20
                                       adapter, with the constant-velocity
                                       stand-in in the book's own runs)
...
from ch25_evaluation import (METRIC_COLUMNS, STRATEGIES, STUDY_CELLS, STUDY_SEEDS,  # noqa: E402
                             mean_ci, paired_compare, paired_table, print_summary,
                             run_mini_study, run_study, summarize, week10_predictor,
                             write_dat)
...
def levels(base_rows):
    """The two factor levels that the mini-study holds at their base value:
    the formation-preserving level of the constraints factor and the learned
    level of the prediction factor, both on the (4, 1) cell with the same
    seeds, so that the numbers quoted in Sections 25.4.1 and 25.11 come from
    the code."""
    cell, strat = (4, 1), "hybrid"
    runs = {
        "formation_off": run_study([cell], (strat,), STUDY_SEEDS, formation=None),
        "formation_on": run_study([cell], (strat,), STUDY_SEEDS, formation="all"),
        "prediction_noisy": [r for r in base_rows
                             if (r["k"], r["m"]) == cell and r["strategy"] == strat],
        "prediction_learned": run_study([cell], (strat,), STUDY_SEEDS,
                                        prediction="learned",
                                        predictor=week10_predictor()),
    }
    out, keys = [], ("ef_mean", "ef_max", "dmin_di", "path_ratio", "makespan",
                     "replans", "collision")
    for name, rr in runs.items():
        row = {"level": name, "n": len(rr)}
        for key in keys:
            row[key] = float(np.mean([r[key] for r in rr]))
        ci = mean_ci([r["ef_max"] for r in rr])
        row["ef_max_lo"], row["ef_max_hi"] = ci["lo"], ci["hi"]
        out.append(row)
    write_dat(os.path.join(DATA, "ch25-study-levels.dat"), out,
              ("level", "n") + keys + ("ef_max_lo", "ef_max_hi"),
              note=("the formation-preserving and learned levels on the (4,1) cell, "
                    "hybrid strategy, seeds 0-19, paired with the mini-study; "
                    "generated by code/figures/gen_ch25_study.py"))
    print("\nThe two levels held at their base value in the mini-study, cell (4,1), "
          "hybrid, 20 seeds:")
    print("%-19s %7s %7s %8s %7s %7s %7s %6s" % (
        "level", "eF mean", "eF max", "dmin_di", "ratio", "mksp", "repl", "coll"))
    for row in out:
        print("%-19s %7.3f %7.3f %8.3f %7.3f %7.2f %7.2f %6.2f" % (
            row["level"], row["ef_mean"], row["ef_max"], row["dmin_di"],
            row["path_ratio"], row["makespan"], row["replans"], row["collision"]))
    for a, b in (("formation_off", "formation_on"),
                 ("prediction_noisy", "prediction_learned")):
        for metric in ("ef_max", "dmin_di", "path_ratio"):
            c = paired_compare([r[metric] for r in runs[b]], [r[metric] for r in runs[a]])
            print("  paired %-10s %s minus %s: %+.3f [%+.3f, %+.3f] d_z %+.2f p_wilcoxon %.4f" % (
                metric, b, a, c["mean_diff"], c["lo"], c["hi"], c["d_z"], c["p_wilcoxon"]))
