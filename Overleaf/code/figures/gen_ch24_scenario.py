...
# frame 1: t = 0
frame(out, 0, 0, Hh + GAP + 1.0, "(a) $t=0$: nominal \\cbs plan")
# frame 2: the trigger of A
ex = prediction_extra(ib)
frame(out, ib, W + GAP, Hh + GAP + 1.0, "(b) $t=%.1f$: prediction, trigger of A" % t_trig, ex)
...
frame(out, i3, 2 * (W + GAP), Hh + GAP + 1.0, "(c) $t=%.1f$: A and C avoiding (\\orca)" % t3, ex)
...
frame(out, i4, 0, 0, "(d) $t=%.1f$: A's reconnection fails" % rc["t"], ex)
...
frame(out, i5, W + GAP, 0, "(e) $t=%d$: A's replanned path" % rp["t_start"], ex)
# frame 6: the end
i6 = len(hist) - 1
frame(out, i6, 2 * (W + GAP), 0, "(f) $t=%.0f$: all drones at their goals" % hist[i6]["t"])
...
def panel(out, x0, paths, title, mark=None, t_from=5, t_to=9):
    ...
    for (n, path, t0) in paths:
        ...
        for c, ts in seen.items():
            dx = -0.45 if n == "B" else 0.0
            dy = 0.5 if n == "B" else -0.15
...
out = ["% The conflict re-check after C's reconnection at t = %.1f s (generated by" % rk["t"],
       "% code/figures/gen_ch24_scenario.py from ch24_hybrid.py).",
       "\\begin{tikzpicture}[x=0.31cm,y=0.31cm,font=\\scriptsize]"]
panel(out, 0, rk["before"], "(a) after C's reconnection: conflict $\\langle B,C,(%d,%d),%d\\rangle$" % (conf.v[0], conf.v[1], conf.t), mark=conf.v)
panel(out, W + GAP + 2.0, rk["after"], "(b) after the local \\cbs on $\\set{B,C}$ from $t=%d$" % rk["t_start"])
out.append("\\end{tikzpicture}")
...
print("formation error: max %.2f at t=%.1f, final %.2f; connected always %s; min mate distance %.2f"
      % (max(h["e_form"] for h in hist), max(hist, key=lambda h: h["e_form"])["t"], hist[-1]["e_form"],
         all(h["connected"] for h in hist), min(r[4] for r in rows) if False else sim.d_min_mates))
...
print("longest inter-drone link: %.2f m (R_comm %.1f, ell_act %.1f)" % (
    max(np.linalg.norm(h["pos"][i] - h["pos"][j]) for h in hist for i in range(3) for j in range(i + 1, 3)
        if True) if False else max(min(max(np.linalg.norm(h["pos"][i] - h["pos"][j]) for j in range(3) if j != i) for i in range(3)) for h in hist), p["r_comm"], p["ell_act"]))
