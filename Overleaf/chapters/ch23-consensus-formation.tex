...
it can hear from its neighbors, that keeps the relative positions, that keeps
the communication graph connected, that yields to an avoidance maneuver and
that pulls the square back together afterward. That controller is the subject
...
moves its state toward the average of its neighbors' states. Consensus is
...
toward the numbers of the drones you can hear (\cref{fig:ch23-idea}). A drone
whose neighbors are all higher climbs, a drone in the middle of its
neighbors stays. Two things follow immediately. Whatever one drone gains, its
neighbor loses, so the \emph{sum} of the numbers never changes; and the
...
\begin{figure}[tb]
  ...
  update uses three neighbors. Right: the consensus rule for one drone. Each
  gray arrow is the difference $\vect{x}_j - \vect{x}_i$ to a neighbor; their
  sum (orange) is $\abs{N_i}$ times the vector from $\vect{x}_i$ to the
  neighbors' average, so drone $i$ moves toward the average of the drones it
  can hear.}
  \label{fig:ch23-idea}
\end{figure}
...
\end{equation}
The \textbf{neighborhood}\index{neighborhood $N_i$} of drone $i$ is
...
The \textbf{adjacency matrix}\index{adjacency matrix} $\mat{W} \in \R^{n
\times n}$ has $w_{ij} = 1$ if $\set{i, j} \in E$ and $w_{ij} = 0$ otherwise
(in particular $w_{ii} = 0$); weighted graphs use $w_{ij} > 0$ for the strength
of the link. The \textbf{degree matrix}\index{degree matrix} is
$\mat{D} = \diag(d_1, \dots, d_n)$ with $d_i = \sum_j w_{ij}$, and the
\textbf{graph Laplacian}\index{graph Laplacian}\index{Laplacian|see{graph Laplacian}} is
\begin{equation}
  \mat{L} = \mat{D} - \mat{W}, \qquad
  \ell_{ij} = \begin{cases} d_i & i = j,\\ -w_{ij} & i \ne j. \end{cases}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item $\mat{L}$ is symmetric and
    $\vect{x}\T \mat{L} \vect{x} = \sum_{\set{i,j} \in E} w_{ij} (x_i - x_j)^2 \ge 0$
  ...
  \item if all weights are $1$ and $G$ is not complete then $\lambda_2 \le d_{\min}$,
    and the complete graph $K_n$ with unit weights has $\lambda_2 = n$.
\end{enumerate}
\end{proposition}
\begin{proof}
(i) The diagonal entry $d_i$ of row $i$ equals the sum of the $w_{ij}$ that are
subtracted in the same row. (ii) Symmetry is inherited from $\mat{W}$. Expand
$\vect{x}\T \mat{L} \vect{x} = \sum_i d_i x_i^2 - \sum_{i \ne j} w_{ij} x_i x_j$
and group the terms of each edge: $w_{ij}(x_i^2 + x_j^2 - 2 x_i x_j) =
w_{ij}(x_i - x_j)^2$. A symmetric matrix with a non-negative quadratic form
...
Gershgorin's theorem every eigenvalue lies in a disk centered at some diagonal
...
hearing only its two neighbors, has $\lambda_2 = 2(1 - \cos(\pi/n))$, which is
...
$\lambda_2$ measures it on a continuous scale from $0$ (disconnected) upward.
...
\begin{equation}
  ...
  \label{eq:ch23-error}
\end{equation}
The \textbf{centered formation error} removes the best translation first:
...
neighbors' messages. The translation $\bar{\vect{y}}$ that $e_c$ removes is
...
\begin{equation}
  ...
  = \sum_{j=1}^{n} w_{ij} (\vect{x}_j - \vect{x}_i), \qquad i = 1, \dots, n.
  \label{eq:ch23-protocol}
\end{equation}
The right-hand side is $-d_i \vect{x}_i + \sum_j w_{ij} \vect{x}_j$, which is
...
$h = k_c\,\dt$:
\begin{equation}
  \vect{x}_{k+1} = (\mat{I} - h\mat{L})\,\vect{x}_k, \qquad\text{that is}\qquad
  \vect{x}_{i,k+1} = \vect{x}_{i,k} + h \sum_{j \in N_i} (\vect{x}_{j,k} - \vect{x}_{i,k}).
  \label{eq:ch23-discrete}
\end{equation}
The matrix $\mat{P} = \mat{I} - h\mat{L}$ is called the Perron matrix of
the graph \cite{olfatisaber2007consensus}. Its rows sum to one, and its entries
$1 - h d_i$ on the diagonal and $h w_{ij}$ off it are all non-negative if
and only if $h \le 1/d_{\max}$. In that case every drone's new state is a
weighted average of its own and its neighbors' states, in which it keeps the
weight $1 - h d_i$ on itself, and the maximum can never grow nor the minimum
...
of the swarm). The exact condition is $h < 2/\lambda_n$, which by
...
\end{equation}
Drone $i$ moves toward the point $\pos_j - \vect{d}_{ij}$ where it should be
according to each neighbor $j$, and it stops when it is there for all of them.
...
two meters to its east but $j$ wants $i$ one meter to its west, the pair
...
positions satisfies all edges, the controller minimizes the residual and a
...
discrete-time protocol \cref{eq:ch23-discrete} with $h = k_c\dt$, so the
...
$i$ broadcasts now is what its neighbors use in their next period. The whole
period costs $\bigO{\abs{N_i}\,m}$ operations, one message in from each
neighbor and one message out.
...
motion of neighbors. The term $-k_d(\vel_i - \dot{\vect{r}})$ is absolute
damping toward the reference velocity (with $\dot{\vect{r}} = \vect{0}$ it
...
Line~\ref{alg:ch23-second:sum} needs the neighbors' velocities as well as
...
$\pos_4 = (3.5, 3)$ (meters) with a radio range $R_{\mathrm{comm}} = 3$~m.
...
(\cref{fig:ch23-square}a, gray), the degrees $(2, 2, 3, 1)$ and
\[
  \mat{W} = \begin{pmatrix} 0&1&1&0\\ 1&0&1&0\\ 1&1&0&1\\ 0&0&1&0 \end{pmatrix}, \qquad
  \mat{L} = \mat{D} - \mat{W} = \begin{pmatrix} 2&-1&-1&0\\ -1&2&-1&0\\ -1&-1&3&-1\\ 0&0&-1&1 \end{pmatrix}.
\]
...
\cref{eq:ch23-discrete} from the same altitudes with $h = 0.25 < 1/3$ and
with $h = 0.6 > 1/2$. The first run is a slowed-down copy of the continuous
one (after the first step drone~3 lands exactly on the average of its three
neighbors, $(7.75 + 6.25 + 2.5)/3 = 5.5$, so its update term is zero and it
never moves again, while the others keep converging toward it); the second run
...
$\norm{(4.5, 1.5)}^2 = 22.5$~m$^2$ to the mean of squares) and the centered error
...
\begin{table}[tb]
  \centering\small
  \caption{Discrete-time consensus $\vect{z}_{k+1} = (\mat{I} - h\mat{L})\vect{z}_k$
  on \cref{ex:ch23-square}. Left: $h = 0.25$ satisfies
  $h < 1/d_{\max} = 1/3$ and converges. Right: $h = 0.6$ exceeds the
  ...
  \toprule
  & \multicolumn{4}{c}{$h = 0.25$} & & \multicolumn{4}{c}{$h = 0.6$}\\
...
\begin{table}[tb]
  ...
  edge RMS error $e_F$, the centered error $e_c$ and the bound
...
\begin{figure}[tb]
  ...
  (dots) along the fixed communication graph (gray) to the final square
...
range each drone hears only its two neighbors, the graph is a path,
...
\begin{figure}[tb]
  ...
  toward the average $7$ (dotted). (b) With $R_{\mathrm{comm}} = 3.8$~m the
...
to zero: every
drone sees its neighbors agreeing with it and is satisfied. The only local
...
Let the graph be undirected and connected and let $0 < h < 2/\lambda_n$.
Then the iteration $\vect{x}_{m+1} = (\mat{I} - h\mat{L})\vect{x}_m$ of
...
\begin{equation}
  ...
  \rho = \max\bigl(\abs{1 - h\lambda_2}, \abs{1 - h\lambda_n}\bigr) < 1 .
  \label{eq:ch23-discreterate}
\end{equation}
In particular the local condition $h < 1/d_{\max}$ of
\cref{eq:ch23-stepsize} suffices, because $\lambda_n \le 2 d_{\max}$.
\end{theorem}
\begin{proof}[Proof sketch]
$\mat{P} = \mat{I} - h\mat{L}$ has the eigenvectors of $\mat{L}$ with the
eigenvalues $\mu_k = 1 - h\lambda_k$; $\mu_1 = 1$ keeps the average, and
...
$0 < h\lambda_k < 2$ for $k \ge 2$, that is, when $h < 2/\lambda_n$
...
$\rho$ is smallest when $1 - h\lambda_2 = -(1 - h\lambda_n)$, that is,
for $h^* = 2/(\lambda_2 + \lambda_n)$ \cite{xiao2004fast}. For the graph of
\cref{ex:ch23-square} this is $h^* = 0.4$ with $\rho = 0.6$ per step, while
the local bound $h = 1/3$ gives $\rho = 2/3$. The optimum needs the spectrum
...
drones it hears, and $\mat{L} = \mat{D} - \mat{W}$ with $d_i = \sum_j w_{ij}$
...
w_{ij}(x_i - x_j)^2 + k_l\sum_{i:\,b_i = 1} x_i^2 \ge 0$. If the form is zero, the
...
Let the communication graph be undirected and connected with unit weights,
and let the displacements come from offsets, $\vect{d}_{ij} = \vect{o}_j - \vect{o}_i$.
...
\end{equation}
the centered formation error satisfies $e_c(t) \le e^{-\lambda_2 t} e_c(0)$,
...
is diagonalized by the eigenvectors of $\mat{L}$, which are shared by
...
move along the line to each neighbor, $\dot{\pos}_i = \sum_j
...
goal. \textcite{reynolds1987flocks} produced this behavior in computer
graphics with three rules for his \emph{boids}\index{boids}: separation
(steer away from crowded neighbors), alignment (steer toward the average
heading of the neighbors) and cohesion (steer toward their average
...
\begin{equation}
  ...
  + \underbrace{\sum_{j \in N_i} w_{ij}(\pos)\,(\vel_j - \vel_i)}_{\text{velocity alignment}}
...
bump function so that neighbors can enter and leave the range without a jump
...
this parameter $\eps$, which this chapter writes $\sigma$. The \emph{alignment term} is
consensus on the velocities with the same distance-dependent weights
$w_{ij}(\pos) \in [0, 1]$, so \cref{thm:ch23-consensus} explains why the
velocities become equal. The \emph{navigation feedback} is a PD term toward a
...
\begin{figure}[tb]
  ...
  the neighbors within the range $r$ produce the gradient term (repel when
...
local safety layer: \orca (\cref{ch:ch13}) takes the velocity computed by the
...
pair of positions (a disk of relative positions), so it can be handed to an
optimization-based controller. In the MPC of \cref{ch:ch21} it becomes one
constraint per kept edge and per step of the horizon, either as a second-order
cone or, for a quadratic program, replaced by a polygon inscribed in the disk:
...
cannot jump across the last few centimeters, and the activation distance
...
the local safety layer is the same construction at the other end of the scale,
...
matrix $s\mat{I} - \mat{L} - \frac{s}{n}\vect{1}\vect{1}\T$ with
$s = 2d_{\max}$ (not the pinning matrix $\mat{B}$ of
...
centralized version and agrees with NumPy's eigenvalues to $10^{-6}$ in the
...
\paragraph{Step size.}
The gain and the control period enter only as the product $h = k_c\dt$, and
\cref{thm:ch23-discrete} needs $h < 1/d_{\max}$ with margin. At $20$~Hz
($\dt = 0.05$~s) with $k_c = 1$~s$^{-1}$ and at most five neighbors,
$h = 0.05$ is far below $0.2$; the bound bites when the gain is raised for
...
With $h$ above $2/\lambda_n$ the swarm does not merely converge slowly: it
diverges, and it does so in the highest-frequency pattern of the graph, so
the symptom is neighboring drones lurching in opposite directions at every
...
$\lambda_4 = 4 = 2d_{\max}$, so $h = 1/d_{\max} = 0.5$ exactly gives the
...
each neighbor is a bounded delay and a lost packet is a switching topology, so
...
\begin{lstlisting}[caption={The formation error of \cref{def:ch23-error} over the edges of the formation graph, and its centered variant (from \code{code/ch23\_consensus.py}).},label={lst:ch23-error}]
...
which need nothing else but their neighbors' positions and the offsets
...
\textbf{To and from the local safety layer.} The velocity of
...
the same edges, and a drop of $\lambda_2$ toward zero, or a kept edge beyond
...
A tempting fix for a high formation error during an avoidance maneuver is to
raise the formation gain until the shape holds. This turns the formation
controller into an adversary of the local safety layer: the consensus term pulls the
avoiding drone back toward the intruder, the safety term pushes it out again,
...
afterward, $3$~s in \cref{fig:ch23-avoidance}. What must be enforced is the
...
\begin{itemize}
  \item The communication graph of a swarm is the radius graph of the drone
    positions; its Laplacian $\mat{L} = \mat{D} - \mat{W}$ is symmetric,
  ...
  \item In discrete time, $\vect{x}_{k+1} = (\mat{I} - h\mat{L})\vect{x}_k$
    converges for $h < 2/\lambda_n$, and $h < 1/d_{\max}$ is the
...
the graph-theoretic textbooks. \textcite{oh2015survey} organizes formation
...
$\mat{W}$, $\mat{D}$ and $\mat{L}$, verify $\mat{L}\vect{1} = \vect{0}$, and
...
average for every $h$, and that the maximum state never grows if
$h \le 1/d_{\max}$. (b) Four drones sit on a line at $x = 0$, $1$, $10$
...
h\mat{L}$ has the eigenvalues $1 - h\lambda_k$ with the eigenvectors of
...
is connected and $h < 2/\lambda_n$. Then prove $\lambda_n \le 2d_{\max}$
with Gershgorin's theorem and conclude that $h < 1/d_{\max}$ suffices.
...
convention that $w_{ij} = 1$ when $i$ hears $j$, check that it has a directed
...
disk, so that a plan that satisfies them keeps the edge in range; how much
...
formation error during an avoidance maneuver. Work in the API of
...
the complete graph during the maneuver; report what happens to $\lambda_2$,
