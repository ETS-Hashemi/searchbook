...
With a gain $k$ in \cref{eq:ch23-protocol} the rate is $k\lambda_2$, and for
...
cluster averages, and the disagreement norm stays at $10$ for ever: every
...
Then the iteration $\vect{x}_{k+1} = (\mat{I} - \eps\mat{L})\vect{x}_k$
conserves the average and converges to it geometrically,
\begin{equation}
  \norm{\vect{\delta}_k} \le \rho^k \norm{\vect{\delta}_0}, \qquad
...
$\vect{\delta}_k = \mat{P}^k\vect{\delta}_0$ expands in the modes $k \ge 2$
with the factors $\mu_k^k$. All of these lie in $(-1, 1)$ exactly when
...
\begin{figure}[tb]
  ...
  $1$~m/s; $R_{\mathrm{comm}} = 3.5$~m, $k = 1$, $k_l = 1.5$, $v_{\max} =
...
integrators under \cref{eq:ch23-second} without pinning. In the eigenbasis of
$\mat{L}$ the shifted dynamics decouple into the modes
\begin{equation}
...
$k_d > 0$; with $k_v = k_d = 0$ every mode oscillates for ever at the angular
frequency $\sqrt{k_p\lambda_k}$. The mode $\lambda_1 = 0$ is the centroid: its
velocity decays to $\dot{\vect{r}}$ if $k_d > 0$ and is otherwise conserved.
\end{proposition}
\begin{proof}[Proof sketch]
...
$\mat{I}$; each mode is a damped oscillator with the characteristic polynomial
...
\begin{equation}
  ...
  - \underbrace{c_1(\pos_i - \pos_\gamma) - c_2(\vel_i - \vel_\gamma)}_{\text{navigation feedback}},
  \label{eq:ch23-flock}
\end{equation}
...
$\norm{\vect{z}}_\sigma = (\sqrt{1 + \eps\norm{\vect{z}}^2} - 1)/\eps$ so that
the potential is differentiable everywhere. The \emph{alignment term} is
...
closest velocity that is collision-free for the horizon $\ttc$, a hard
...
matrix $\mat{B} = s\mat{I} - \mat{L} - \frac{s}{n}\vect{1}\vect{1}\T$ with
$s = 2d_{\max}$, whose largest eigenvalue is $s - \lambda_2$; each iteration
...
\paragraph{Step size.}
The gain and the control period enter only as the product $\eps = k\dt$, and
\cref{thm:ch23-discrete} needs $\eps < 1/d_{\max}$ with margin. At $20$~Hz
($\dt = 0.05$~s) with $k = 1$~s$^{-1}$ and at most five neighbours,
...
Real drones do not update in lockstep. Each drone uses the latest position it
has received from every neighbour, which may be one or several periods old.
For bounded delays the results of \cref{sec:ch23-directed} apply: a uniform
delay below $\pi/(2\lambda_n)$ is harmless, and larger or unequal delays are
absorbed by a smaller gain. A lost packet removes the term of that neighbour
for one period, which is a switching topology; consensus survives as long as
the union of the graphs over bounded windows is connected. What does not
survive asymmetric losses is the exact average: if $i$ hears $j$ but $j$ misses
$i$, the graph is momentarily directed and unbalanced and the agreed value
drifts. For a formation this only moves the template by a few centimetres,
and a pinned leader corrects it; for a swarm that averages sensor readings it
matters, and one then uses protocols that retransmit or that exchange mass
explicitly.
...
a path for the formation as a whole, a virtual leader whose footprint is the
formation's, and MPC (\cref{ch:ch21}) may smooth it. That path is the reference
$\vect{r}(t)$, $\dot{\vect{r}}(t)$ of \cref{alg:ch23-formation}, sampled at the
current time. One drone is pinned to it; the others need nothing but their
neighbours' positions and offsets. The offsets $\vect{o}_i$ are the shape of
the mission (a line for a survey, a square for a camera rig), and the formation
graph is chosen connected with $\lambda_2$ as large as the radio range allows.
...
\begin{figure}[tb]
  ...
  $t = 10.5$~s and is back below $0.05$~m at $t = 13.6$~s; the closest
  approach to the intruder is $0.91$~m. Bottom: the longest edge reaches
...
The survey of \textcite{olfatisaber2007consensus} is the standard entry point
and covers everything in \cref{sec:ch23-properties} with the Perron-matrix
view of discrete time; the original results are \textcite{olfatisaber2004consensus}
for the Laplacian framework, delays and switching, \textcite{ren2005consensus}
for directed graphs and \textcite{jadbabaie2003coordination} for the first
switching-graph proof. The book of \textcite{ren2008distributed} and the
tutorial of \textcite{ren2007information} treat leader--follower and
second-order consensus and formation control for vehicles;
\textcite{mesbahi2010graph} and \textcite{bullo2009distributed} are the graph-theoretic
textbooks. \textcite{oh2015survey} organises formation control into the three
families of \cref{tab:ch23-families}, with \textcite{anderson2008rigid} and
\textcite{krick2009stabilisation} for the rigidity theory of distance-based
formations. Flocking is \textcite{reynolds1987flocks},
\textcite{olfatisaber2006flocking} and \textcite{tanner2007flocking};
connectivity maintenance is \textcite{ji2007distributed},
\textcite{zavlanos2011graph} and, for the distributed estimation of
$\lambda_2$, \textcite{yang2010decentralized}. The optimal step size is
\textcite{xiao2004fast}.
...
$\eps \le 1/d_{\max}$. (b) Two pairs of drones sit $10$~m apart with
$R_{\mathrm{comm}} = 2$~m and the initial states $(1, 3, 10, 14)$; predict the
limit of the consensus protocol and the final disagreement norm without
simulating. (c) Prove the identity $\sum_{i<j}\norm{\vect{y}_i - \vect{y}_j}^2 =
...
is stationary if and only if $\vect{d}_{ji} = -\vect{d}_{ij}$ on every edge;
what happens otherwise? (b) With antisymmetric displacements, show that the
...
$-v\,(k\mat{L} + k_l\mat{B})\inv\vect{1}\otimes\vect{e}$ of
\cref{prop:ch23-leader}. Evaluate it for the graph of \cref{ex:ch23-square}
with drone~1 pinned, $k = k_l = 1$ and $v = 1$~m/s: how far does each drone
