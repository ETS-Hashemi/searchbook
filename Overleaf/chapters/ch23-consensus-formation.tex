...
Gershgorin's theorem every eigenvalue lies in a disk centerd at some diagonal
...
\begin{equation}
  ...
  \label{eq:ch23-error}
\end{equation}
The \textbf{centerd formation error} removes the best translation first:
...
$\norm{(4.5, 1.5)}^2 = 22.5$~m$^2$ to the mean of squares) and the centerd error
...
\begin{table}[tb]
  ...
  edge RMS error $e_F$, the centerd error $e_c$ and the bound
...
\end{equation}
the centerd formation error satisfies $e_c(t) \le e^{-\lambda_2 t} e_c(0)$,
...
\begin{lstlisting}[caption={The formation error of \cref{def:ch23-error} over the edges of the formation graph, and its centerd variant (from \code{code/ch23\_consensus.py}).},label={lst:ch23-error}]
...
\textcite{anderson2008rigid} and \textcite{krick2009stabilization} for the
