...
The single integrator assumes that the drone can change its velocity instantly. That is the model behind velocity obstacles and optimal reciprocal collision avoidance (\orca; \cref{ch:ch12,ch:ch13}), whose output is a velocity, and it is implicitly the model of a grid planner, where a move of one cell per step is a velocity of one cell per $\dt$.
