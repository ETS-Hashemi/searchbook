...
With unit move costs, and with the waits at the goal free as in \cref{def:ch02-space-time-state}, this makes $\cost(\pi)=T(\pi)$: a search that accumulates action costs in $\gcost$ returns exactly this travel time. That is the convention of \textcite{stern2019mapf}, what \cref{ch:ch07} writes as $\cost(\pi_i)$, and what the notation table lists under $\sumcost$.
...
Two remarks close the toolkit. A horizon $\ttc$ turns ``will they ever collide?'' into ``will they collide within the next $\ttc$ seconds?'', the question the safety layer of \cref{ch:ch24} asks; the toolbox clamps $t^*$ to $[0,\ttc]$. And everything here carries over to three dimensions, except that the cross product becomes a vector (use its norm in \cref{eq:ch02-tca}) and the two tangent lines become a cone with the same half-angle $\theta=\arcsin(r/d)$.
