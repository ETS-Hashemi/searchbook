...
\Cref{fig:ch02-minkowski} shows the construction. Inflating a polygon by a disc rounds its corners, and inflating a union of obstacles inflates each of them. The same idea serves twice more: \cref{ch:ch12} turns an encounter between two discs into a point and a disc of radius $r_A+r_B$ (\cref{thm:ch02-disc-minkowski}), and in \cref{ch:ch24} the inflation radius grows with the uncertainty of a predicted position.
...
\Cref{tab:ch02-path-plan-trajectory} summarises the vocabulary; keep the three words apart. The hybrid architecture of \cref{ch:ch24} converts one into the next: the global planner produces a plan, each drone follows its time-indexed path as a trajectory, and the local layer edits that trajectory when an intruder appears.
...
The tracker of \cref{ch:ch18} does not hand the planner a position; it hands over an estimate \emph{and} a covariance that says how much to trust it. This section fixes the few facts about that object which Part~VI builds on and \cref{ch:ch24} uses to inflate constraints.
...
To draw the ellipse, call \code{numpy.linalg.eigh} on $\mat{\Sigma}$ (it returns the eigenvalues in ascending order, so reverse them), take the square roots and evaluate \cref{eq:ch02-ellipse-param} at, say, $72$ angles; the toolbox does this in \code{covariance\_ellipse} and \code{ellipse\_points}.
