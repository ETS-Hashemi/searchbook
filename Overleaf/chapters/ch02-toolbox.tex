...
\Cref{tab:ch02-path-plan-trajectory} summarises the vocabulary; keep the three words apart.
...
\code{0.1 + 0.2 == 0.3} is \code{False} in floating point, so two states that are physically identical can hash to different keys and the closed set stops working: the search revisits states forever or until memory runs out. NumPy arrays are not hashable at all (\code{tuple(a)} is).
\end{pitfall}
