...
The theorem says exactly what the braking test buys: as long as the world
stands still and the free-distance queries are right, a DWA drone can be
stopped at any moment without collision, no matter how the scores are
weighted. Everything that the theorem excludes is a real failure mode.
Obstacles that move towards the drone shrink $d'$ faster than
$\norm{\vel}\dt$ (\cref{sec:ch14-variants} shows how to repair this with the
predicted velocities); free distances that are overestimated, by sensor noise
or by a coarse grid, break the induction; and a velocity controller that
tracks the command with a lag makes the window larger than what the drone
can really do. For the differential drive the same argument works along the
arc provided the robot brakes without changing its curvature (reducing $v$ and
$\omega$ in proportion); Fox et al.\ test $v$ and $\omega$ separately, which
is simpler and, for their robot, conservative enough.
...
The proposition describes the clean form of the failure, a
\textbf{local minimum}\index{local minimum}\index{DWA!local minimum} in
which the drone rests. With a dominant heading term the drone never rests,
because some moving candidate always scores more than $\beta$, and the
failure takes the form of \textbf{oscillation}\index{oscillation}\index{DWA!oscillation}
instead. \Cref{fig:ch14-utrap} shows the classical instance, a U-shaped
obstacle between the drone and the goal, and \cref{sec:ch14-tuning} runs
it: with the default weights the drone flies into the U at full speed,
brakes in front of the back wall (as \cref{thm:ch14-safety} promises), and
then, because every candidate that improves the heading points at the wall
and is inadmissible while the candidates along the wall are admissible,
it flies up and down between the two arms. In $300$ steps ($75$~s) it covers
$80$~m without ever leaving the U, coming within $4$~cm of the arms on each
turn. The self-test of \code{ch14\_dwa.py} reproduces this run and asserts
that the goal is \emph{not} reached; it is the documented failure case of
the chapter, and the Week~7 coding exercise asks you to build your own.
...
A second, subtler oscillation arises without any trap. When two candidates
score almost the same, say the two sides of an obstacle that sits exactly
on the line to the goal, the winner can change from step to step as the
geometry changes by a hair, and the drone zigzags. This is the same
phenomenon as the oscillation of velocity obstacles in \cref{ch:ch12} and
of potential fields in \cref{ch:ch15}, and the remedies are the same in
spirit: add inertia to the decision. \Cref{sec:ch14-variants} lists them.
...
between steps.\index{DWA!complexity} The self-test of \code{ch14\_dwa.py}
performs several hundred control steps, each with $25$ candidates against up
to three obstacles, in about a second of plain \python, which is enough for
the $4$~Hz loop of the examples; compiled implementations run the same
computation at hundreds of hertz.
...
\begin{table}[tb]
  ...
  \toprule
  Scene & $(\alpha, \beta, \gamma)$ & outcome & steps & length [m] & min.\ clear.\ [m] & speed [m/s]\\
  \midrule
  corridor & $(0.6, 0.2, 0.2)$ default & reached & 16 & 5.9 & 0.05 & 1.48\\
  corridor & $(0.8, 0.1, 0.1)$ heading & reached & 16 & 5.8 & 0.05 & 1.46\\
  corridor & $(0.2, 0.6, 0.2)$ clearance & reached, outside the corridor & 21 & 8.7 & 0.09 & 1.66\\
...
allows. In the corridor it squeezes past the pillar with $5$~cm to spare and
arrives in $16$ steps. In the U-trap it drives straight in and oscillates
between the arms for the whole run. Heading-dominant DWA is efficient and
predictable in open space and blind in a cluttered one: it cannot trade a
short detour now for progress later.

\paragraph{Clearance dominates.}
With $(0.2, 0.6, 0.2)$ the drone is timid. The corridor is $2$~m wide, so
every candidate that enters it has a clearance of at most $0.33$ against
$1$ for the open field outside: the drone turns away, flies around the
outside of the lower wall and reaches the goal from below, $47\,\%$ longer
than the direct route. In the U-trap it turns around before the U and flies
away from everything at nearly full speed, ending $141$~m from the start.
Clearance-dominant DWA keeps the largest berth but avoids narrow passages
and, if the free space is large, prefers it to the goal.

\paragraph{Velocity dominates.}
With $(0.2, 0.2, 0.6)$ the drone wants to be fast above all. In the corridor
this is harmless, since the fastest admissible commands also lead to the
goal, and it is the quickest of the four runs. In the U-trap it enters, is
forced by the braking test to turn at the back wall, and then keeps flying
at full speed wherever there is room, never returning: the heading term is
too weak to bring it back. Velocity-dominant DWA wanders.

\paragraph{The lesson.}
The three terms pull in different directions, and no fixed weighting is
right for all scenes: the heading term should be the largest, as in the
original paper (Fox et al.\ report using $\alpha = 2.0$, $\beta = 0.2$,
$\gamma = 0.2$), and the trap must be fixed elsewhere. The last row of
\cref{tab:ch14-weights} shows where: with the default weights and the
heading term aimed at a lookahead point on a global path around the U, the
drone reaches the goal in $25$ steps with a minimum clearance of $0.61$~m.
Weights tune behaviour; the global path fixes topology.
...
DWA evaluates rollouts of a single constant velocity. Simulating a candidate
for a longer horizon $T$, and scoring the whole predicted trajectory rather
than a ray, lets the objective see around the next corner and reduces
oscillation, at the price of more computation and a stronger dependence on
the assumption that the command stays constant. Planners that roll out
short sequences of commands, such as the trajectory rollout of many
navigation stacks and the model predictive controllers of \cref{ch:ch21},
are the natural continuation of this idea; \cref{ch:ch21} shows that MPC is
DWA with the window replaced by a constrained optimisation over the horizon.
...
\"Ogren and Leonard~\cite{ogren2005convergent} showed that a DWA-like
scheme can be made \emph{convergent}, that is, provably reach the goal, by
choosing the objective as a control Lyapunov function derived from a
navigation function and by treating the window as the constraint set of a
receding-horizon controller. This is the bridge between the heuristic DWA of
this chapter and the MPC of \cref{ch:ch21}. Related curvature-space methods
and a survey of local planners are found in the handbook chapters of
\cite{siciliano2016handbook} and in \cite{choset2005principles}.
...
\begin{table}[tb]
  ...
  \midrule
  Searches over & velocities reachable in one interval (the window) & all velocities outside the union of half-planes & nothing: follows the negative gradient of a potential\\
  Assumes & known obstacle geometry near the robot; acceleration and speed limits; obstacles static during the rollout unless predicted & other agents run the same rule and share the avoidance; velocities can change instantly; positions, velocities, radii of the others are known & a potential over positions; obstacles as repulsive sources; the robot follows a force\\
  Guarantees & no collision with static obstacles as long as a braking command is admissible (\cref{thm:ch14-safety}); dynamically feasible commands; not complete & pairwise collision-free for the horizon $\ttc$ if all agents reciprocate and the program is feasible; not complete (deadlocks) & none in general; local minima and oscillation\\
  Cost per step & $\bigO{N_c\,M}$, $N_c = 25$--$1\,331$ & a linear program in $n$ half-planes & $\bigO{M}$ gradient evaluation\\
  Best for & vehicles with tight dynamics, unknown or non-cooperative obstacles, sensor-based avoidance & many cooperative agents of the same kind & very cheap steering, a first cut, or as one term inside another method\\
  \bottomrule
  \end{tabular}
\end{table}

\Cref{tab:ch14-comparison} summarises how DWA relates to its neighbours. In
one sentence: \orca is the right tool when the obstacles are other \orca
agents, DWA when the obstacles are anything else and the vehicle cannot jump
in velocity, and potential fields when you need a steering term in a hurry
and can live without guarantees.
...
The grid spacing $\Delta v$ sets both the cost, $(2n+1)^{\dim}$ candidates,
and the quality: a narrow admissible corridor between two inadmissible
regions can fall between grid points and be missed, so that the drone stops
in front of a gap it could pass. Choose $\Delta v$ so that
$2n+1$ is at least $5$, and refine the grid adaptively near the boundary of
the admissible set if narrow passages matter. Because the grid is centred
on $\vel_a$, the candidates drift with the drone's velocity and are not
aligned with a fixed grid; this is deliberate, ``keep the current velocity''
must always be a candidate. Make sure that $a_{\max}\dt/\Delta v$ is at
least $1$, otherwise the window contains no candidate but $\vel_a$ and the
drone can never change its velocity (\cref{exr:ch14-window}).

\paragraph{Cost of rollouts and free-distance queries.}
For discs and boxes the ray cast of \cref{lst:ch14-dist} is exact and costs
$\bigO{M}$; it is the right choice for the obstacle lists that come out of
the tracking layer. For an occupancy grid or a point cloud, march along the
ray in steps $\Delta s$ smaller than the smallest obstacle feature, or
precompute a distance transform of the map once per scan and read
$\dist$ off it. The rollout of a differential drive is a march along the arc
of \cref{eq:ch14-arc}; \code{diffdrive\_free\_distance()} in the code does
this in steps of $2$~cm and the self-test checks it against the analytic
ray cast for $\omega = 0$. Whatever the method, the query must be
...
Keep the global path as a polyline in the drone's frame of reference,
project the current position onto it, and take the point at arc length $L$
beyond the projection as the target of the heading term
(\code{lookahead\_point()} in the code). When the drone is pushed off the
path by an avoidance manoeuvre the projection follows, and the drone
reconnects further along the path rather than backtracking. If the path is
replanned (\cref{ch:ch05}), just replace the polyline; DWA has no state to
update.
...
The window spans $\pm a_{\max}\dt$ around the current velocity. With a
short control interval or a small acceleration limit this can be less than
one grid step, and the only candidate left is $\vel_a$ itself; the drone then
keeps its velocity whatever happens. Even when the grid is fine enough, a
drone at $v_{\max}$ needs $v_{\max}/(a_{\max}\dt)$ steps to stop, four
steps or one second in the examples, and the braking test must account for
that by looking $v\dt + v^2/(2a_b)$ ahead rather than one step. Check
$a_{\max}\dt \ge \Delta v$ at start-up, and size $a_{\max}$ by what the
velocity controller really delivers, not by the datasheet.
\end{pitfall}
