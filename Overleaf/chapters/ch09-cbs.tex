...
\begin{itemize}
  ...
  \item Recognize the classic implementation mistakes, and know the improvements (cardinal conflicts, bypass, high-level heuristics, symmetry breaking) that turn textbook \cbs into a practical solver.
\end{itemize}
\end{objectives}
...
\emph{global planner}: before take-off it computes the nominal
time-indexed route of every drone, and these routes are what the prediction,
local safety and replanning layers later protect and repair. Whenever a
...
such constraints can serve as the low level, which is why \cbs generalizes
...
minimizes $\sumcost$, and $C^*$ denotes its cost.
...
no conflict. Every non-goal node has \emph{at most} two
\textbf{children}, one per constraint of the split of one of its conflicts;
a child whose low-level problem is infeasible does not exist
(\cref{alg:ch09-cbs} discards it at line~\ref{alg:ch09-cbs:replan}). The
...
\cref{alg:ch09-lowlevel} states the differences line by line. Two of its
ingredients are worth naming before that. The heuristic is the static
...
test}\index{goal-stay test} of \cref{def:ch04-goalstay}, also called the
...
\Cref{alg:ch09-cbs} is the high level. The low level is not a new search: it is the
space-time \astar of \cref{ch:ch04}, printed in full as \cref{alg:ch04-spacetime},
reading its filter as the constraint set of one agent, and \cref{alg:ch09-lowlevel}
lists only the lines in which the two differ. Together the three are a complete
description of \cbs as published by
...
\begin{algorithm}[htb]
\caption{The low level as a delta on space-time \astar: the four lines by which
\cref{alg:ch04-spacetime} becomes a search for one agent under its constraints}
...
\Fn{\CbsLowLevel{$a_i$, $\mathcal{C}_i$}}{
  run \FuncSty{SpaceTimeAStar}$(G, s_i, \gamma_i, \mathcal{C}_i, T_{\max})$ of \cref{alg:ch04-spacetime} --- states $(v, t)$, wait action first, $\gcost(v,t) = t$, key $(\fcost, -t)$ with $\fcost(v,t) = t + d_i(v)$ --- with the following changes\;
  \emph{filter:} the forbidden set is $\mathcal{C}_i$, the constraints of \emph{this} agent; skip the successor $v'$ if $\langle a_i, v', t+1\rangle \in \mathcal{C}_i$ \KwOr $\langle a_i, v, v', t\rangle \in \mathcal{C}_i$\label{alg:ch09-lowlevel:forbid}\;
  $t_\gamma \gets$ largest $t$ with $\langle a_i, \gamma_i, t\rangle \in \mathcal{C}_i$, or $-1$; a constraint set, unlike a reservation table (\cref{def:ch08-reservation}), never parks an agent, so $t_\gamma < \infty$ always\label{alg:ch09-lowlevel:tgamma}\;
  \emph{goal test:} return the path as soon as a popped state has $v = \gamma_i$ \KwAnd $t > t_\gamma$\label{alg:ch09-lowlevel:goal}\;
  \emph{horizon:} $T_{\max} \gets H + 1 + D$, with $H$ the largest time in $\mathcal{C}_i$ (or $-1$; $t+1$ for an edge constraint $\langle a_i, u, v, t\rangle$) and $D = \max_v d_i(v)$\label{alg:ch09-lowlevel:horizon}\;
}
\end{algorithm}
...
\Cref{alg:ch09-lowlevel} solves the low-level problem of
\cref{def:ch09-lowlevel}: it is \cref{sec:ch04-spacetime}'s space-time \astar
written against a constraint set.\index{CBS!low level} The search itself is
unchanged, and it is worth saying which parts that covers. The state is a pair
$(v, t)$; the wait action is generated before the moves; the key is
$\fcost = t + d_i(v)$ with ties broken toward the larger $t$ (the ``deeper''
state); and relaxation is replaced by duplicate detection, because
$\gcost(v, t) = t$ for every route into a state, so the first time a state is
generated it is already the cheapest. Because $d_i$ is consistent, no state is
expanded twice.

Four lines are new. Line~\ref{alg:ch09-lowlevel:forbid} is the filter: it drops
the successors that a vertex or edge constraint of \cref{def:ch09-constraint}
forbids, and the agent index is what selects $\mathcal{C}_i$ out of the node's
constraints. Line~\ref{alg:ch09-lowlevel:tgamma} computes $t_\gamma$ of
\cref{eq:ch09-tgamma}, the last time at which the goal is forbidden; a
constraint set is finite and forbids single time steps, so, unlike a
reservation table, it can never make $t_\gamma$ infinite.
Line~\ref{alg:ch09-lowlevel:goal} is the goal test with the goal-stay rule; a
path that reaches $\gamma_i$ at a time $t \le t_\gamma$ is not accepted, and the
search continues, typically by waiting somewhere and re-entering the goal later,
or by leaving the goal and coming back. Line~\ref{alg:ch09-lowlevel:horizon}
fixes the time horizon: after the last constrained time $H$ the search is
unconstrained, and from any cell the goal is then at most $D$ steps away, so an
optimal path never needs more than $H + 1 + D$ steps. The horizon keeps the
search finite when no path exists, for instance when the constraints seal the
agent in. With these four lines the first path that passes the goal test is a
minimum-cost path among those that respect $\mathcal{C}_i$.
...
$(x, y)$ with $x$ the column and $y$ the row, $y$ growing upward.
\end{example}
...
well. This node is worse than it looks: between $t = 1$ and $t = 2$, $a_2$ moves
up into $(2,2)$ while $a_3$ moves down into $(2,1)$, an edge conflict
...
\begin{figure}[tb]
  ...
  \caption{The constraint tree of \cref{ex:ch09-crossing}. Every child adds
    ...
    Costs never decrease downward. Nodes $N_4$, $N_5$ and $N_6$ were
...
solution cannot make both moves, because together they are a swapping
conflict (the \emph{edge conflict} of \cref{sec:ch09-problem}), so it
...
\Cref{tab:ch09-compare} summarizes the comparison; the improvements of
...
to split at all, how to \emph{guide} the high level, and how to recognize
...
both children, so the search makes progress toward $C^*$ with every split.
...
summarizes the three cases; in \cref{ex:ch09-crossing} every split was
cardinal, which is why ICBS builds the same tree there.

\begin{table}[tb]
  \centering\small
  \caption{The three conflict types of \cref{def:ch09-cardinal} and what a
  split achieves. ICBS works down the last column: split a cardinal conflict
  if there is one, otherwise a semi-cardinal one.}
...
Recognizing the type without generating the children uses the
...
search forward and one backward in space-time. A vertex conflict at
...
a poor way to coordinate them. Meta-agent \cbs (MA-CBS)\index{meta-agent CBS}\index{CBS!meta-agent} by Sharon et al.~\cite{sharon2015cbs} counts, for
every pair of agents, the conflicts split between them in the search so far
(some implementations count only along the current branch); when the count
exceeds a bound $B$, the two agents are
...
toward fewer conflicts then pops $N_3$ before $N_2$, and CBSH expands three
...
monotonically toward its goal. \Cref{exr:ch09-rectangle} asks you to prove
...
conflicts) and \cbs with cardinal conflicts first and the bypass, labeled
...
\begin{figure}[tb]
  ...
  \caption{\cbs on random $8 \times 8$ grids ($15\,\%$ obstacles, $12$
    ...
    count by two to seven and raise the success rate.}
...
bypasses cuts the mean node count to $15$, $114$ and $92$ for $k = 8, 10,
12$ and the maxima to $66$, $771$ and $308$; it solves all instances with
$k = 8$ and $92\,\%$ and $75\,\%$ for $k = 10$ and $12$. (Its mean at
$k = 10$ is larger than at $k = 12$ because at $k = 10$ it still solves an
instance that needs $771$ nodes -- one of those that plain \cbs abandons --
whereas at $k = 12$ the three hardest instances time out and drop out of the
mean altogether: means taken over the solved instances only are not
comparable when two solvers, or one solver at two team sizes, solve
different sets.) Its price is
...
setting make the success rates coarse (one instance is $8\,\%$), the
$5$~s cap makes them depend a little on the machine, and the
...
is another common choice: it dives toward a goal node instead of sweeping
a level.
...
\begin{dronebox}
\cbs is the global planner, the first of the four layers of the
...
watches for intruders that were not in the plan, the local safety layer
