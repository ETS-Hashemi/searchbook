...
contested cell at the contested time, and it continues with whichever
alternative is cheaper. The result is a plan that is provably optimal and a
...
\begin{figure}[tb]
  ...
  \caption{The two-level idea. Left: planned alone, two shortest paths meet
    ...
    constrained agent with the low level. The high level expands the cheaper
    child first.}
  \label{fig:ch09-idea}
\end{figure}
...
$\pi_i[t] = \pi_j[t] = v$, and an \textbf{edge conflict} (a swap)
...
This is the constrained space-time \astar of \cref{ch:ch04} with the
constraint set in the role of the reservation table. Three details of that
...
\gamma_i$ and $t > t_\gamma$'', where\index{goal-occupied-later test}
\begin{equation}
...
\Cref{alg:ch09-lowlevel} is \cref{ch:ch04}'s space-time \astar written
...
is the goal test with the goal-occupied-later rule; a path that reaches
...
the drones take
\[
R.\pi_1 = (0,1),(1,1),(2,1),(3,1),(4,1), \quad
R.\pi_2 = (2,0),(2,1),(2,2), \quad
...
conflict $\langle a_2, a_3, (2,1), 1\rangle$. (Agent $a_3$ follows one step
behind $a_1$ from $t=2$ on, but following is not a conflict in our model.)
...
\begin{table}[tb]
  ...
  \caption{The constraint-tree nodes of \cref{ex:ch09-crossing} in the order
    ...
    listed. ``Conflicts'' is the number of conflicts among the node's paths;
    the last column names the conflict that is split when the node is expanded.}
  \label{tab:ch09-ct}
  \begin{tabular}{@{}llccccl@{}}
  \toprule
  Node & Constraint added & $\cost(\pi_1)$ & $\cost(\pi_2)$ & $\cost(\pi_3)$ & Total / conflicts & Expansion \\
  \midrule
  $N_0$ (root) & none & 4 & 2 & 4 & 10 / 1 & 1st: split $\langle a_2,a_3,(2,1),1\rangle$\\
  $N_1$ ($\leftarrow N_0$) & $\langle a_2,(2,1),1\rangle$ & 4 & 3 & 4 & 11 / 1 & 2nd: split $\langle a_1,a_2,(2,1),2\rangle$\\
  $N_2$ ($\leftarrow N_0$) & $\langle a_3,(2,1),1\rangle$ & 4 & 2 & 5 & 11 / 4 & 3rd: split $\langle a_2,a_3,(2,1)\!\to\!(2,2),1\rangle$\\
  $N_3$ ($\leftarrow N_1$) & $\langle a_1,(2,1),2\rangle$ & 5 & 3 & 4 & 12 / 0 & 4th: goal node, returned\\
  $N_4$ ($\leftarrow N_1$) & $\langle a_2,(2,1),2\rangle$ & 4 & 4 & 4 & 12 / 0 & left in \Open (also optimal)\\
  $N_5$ ($\leftarrow N_2$) & $\langle a_2,(2,1)\!\to\!(2,2),1\rangle$ & 4 & 3 & 5 & 12 / 5 & left in \Open\\
  $N_6$ ($\leftarrow N_2$) & $\langle a_3,(2,2)\!\to\!(2,1),1\rangle$ & 4 & 2 & 6 & 12 / 2 & left in \Open\\
  \bottomrule
  \end{tabular}
\end{table}
...
finds it in $0.01$~s, while \cbs had expanded $60\,000$ constraint-tree
nodes after $13$~s without finding a conflict-free node, because every extra
...
\begin{table}[tb]
  ...
  Prioritized planning (\cref{ch:ch08}) & no & no & nothing exponential & many agents, sparse interaction, no guarantees\\
  \cbs (this chapter) & solvable & \checkmark & depth of the constraint tree & many agents with few, local conflicts\\
  \ecbs (\cref{ch:ch10}) & solvable & $w$-bounded & depth, but far fewer nodes & the same, at larger scale\\
  \bottomrule
  \end{tabular}
\end{table}
...
Their algorithm, ICBS, therefore splits a cardinal conflict if the node has
...
MDDs, semi-cardinal when it is the only cell in one of them, and
...
minimum cost under it; hence \cref{thm:ch09-split,thm:ch09-lowerbound} and
...
everything is merged at once and MA-CBS is joint-space \astar; with $B =
\infty$ it is plain \cbs. The bound therefore interpolates between the two
...
CBSH)\index{CBSH}\index{CBS!high-level heuristic}. The quantity to estimate
...
cardinal means, in unit-cost grids). A set of agents that covers every edge
of the conflict graph must therefore pay at least its size, so
\begin{equation}
\label{eq:ch09-hcg}
h_{\mathrm{CG}}(N) = \text{size of a minimum vertex cover of the conflict graph of } N
\end{equation}
is admissible, and the high level can expand nodes in order of
...
time order until it finds a cardinal one, so it performs more low-level
expansions per constraint-tree node ($23\,600$ against $17\,700$ on average
at $k = 12$); an MDD-based classifier avoids this. Twelve instances per
...
$400$ lines: the grid and the backward breadth-first distances, the low
...
possible orders. Sharon et al.\ describe duplicate detection by hashing the
constraint sets, and report that it rarely pays because such coincidences
are uncommon; the implementation here does not detect duplicates, and the
...
\begin{lstlisting}[caption={The low level: space-time \astar under constraints, with the goal-occupied-later test and the horizon $T_{\max} = H + 1 + D$ (from \code{code/ch09\_cbs.py}).},label={lst:ch09-lowlevel}]
...
And the low level must apply the goal-occupied-later test
...
\begin{itemize}
  ...
  \item A conflict $\langle a_i, a_j, v, t\rangle$ is split into two children, one adding $\langle a_i, v, t\rangle$ and one adding $\langle a_j, v, t\rangle$; an edge conflict is split into the two opposite edge constraints. Only the constrained agent is replanned; constraints accumulate down the tree.
  ...
  \item The low level must obey the constraints \emph{and} the goal-occupied-later test, and needs the horizon $T_{\max} = H + 1 + D$; a parked agent still conflicts.
...
Classify every conflict split in \cref{tab:ch09-ct} as cardinal,
semi-cardinal or non-cardinal, using the costs of the children. Then draw
...
$h_{\mathrm{CG}}$ of \cref{eq:ch09-hcg} for $N_0$, $N_1$ and $N_2$.
\end{exercise}
...
goal-occupied-later test and the horizon; conflict detection with parked
