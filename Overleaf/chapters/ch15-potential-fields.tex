...
any repulsion -- while the conic force is bounded but discontinuous at the
goal, where the unit vector flips, so a drone under it chatters across the
goal. The hybrid form takes the bounded force far away and the smooth bowl
near the goal; the constant $-\tfrac12 k_{\mathrm{att}} (d^*)^2$ makes
$U_{\mathrm{att}}$ continuous at $d = d^*$, and both branches give a force of
magnitude $k_{\mathrm{att}} d^*$ there (\cref{exr:ch15-conic}); it is the form
recommended by Choset et al.~\cite{choset2005principles}. Clipping the
quadratic force at a speed $v_{\max}$, as the controller of
...
tolerance $\eps_{\mathrm{g}}$, needed because the quadratic bowl makes the
drone approach the goal exponentially and never arrive exactly
(\cref{exr:ch15-conic}). Line~\ref{alg:ch15-descend:hit} reports a
collision, which cannot happen in continuous time (\cref{thm:ch15-descent})
but can in discrete time (\cref{sec:ch15-oscillation}).
...
the force there is zero, so the drone simply stops -- and it also fires when
the drone chatters across a point.
...
\vect{F} - b\dot{\pos}$, the marble of \cref{sec:ch15-intuition}, while
almost every mobile-robot implementation uses the first-order form $\vel =
\vect{F}$ and sends $\vel$ to the velocity controller of the vehicle. The magnitude of $\vect{F}$ is then arbitrary and can be enormous,
...
otherwise. Once the speed is clipped only the ratio of the two gains shapes
the trajectory; $\rho_0$ decides how early an obstacle is felt, and $\dt$ and
$v_{\max}$ set the step length.
...
\begin{itemize}
  ...
  the failures around; they do not remove them from all scenes.
  ...
  of a robot driven by raw sonar readings, and says nothing against
  potential terms inside a larger system (\cref{sec:ch15-drone}) or against
  navigation functions, which answer the trap problem for a class of worlds.
\end{itemize}
...
Its discrete cousin, the wavefront planner of Choset et
...
positions, not velocities: an intruder approaching head-on is repelled only
once it is within $\rho_0$, whereas the velocity obstacles of \cref{ch:ch12}
and \orca (\cref{ch:ch13}) reason about relative velocity and time to
collision, and yield earlier and reciprocally. It has no notion of the
drone's dynamics, whereas the dynamic window approach of \cref{ch:ch14}
evaluates only commands the drone can execute. And it has no guarantee in
discrete time and the four failure modes of \cref{sec:ch15-failures}, which
no tuning removes from every scene.
...
every local layer of \cref{ch:ch24} follows: a global plan supplies short
legs, the local layer executes them reactively, and a stalled leg triggers
replanning with \dstarlite or \astar (\cref{ch:ch05,ch:ch04}). Week~7 of the
study plan (\cref{ch:appA}) covers this chapter together with \cref{ch:ch14};
its coding exercise is \cref{exr:ch15-coding}.
\end{dronebox}
...
\begin{summary}
\begin{itemize}
  \item A potential field adds a bowl at the goal, $U_{\mathrm{att}} = \tfrac12 k_{\mathrm{att}} \norm{\pos - \pos_{\mathrm{g}}}^2$ (conic beyond $d^*$ in the hybrid form), to a wall around every obstacle, $U_{\mathrm{rep}} = \tfrac12 k_{\mathrm{rep}} (1/\rho - 1/\rho_0)^2$ for $\rho \le \rho_0$, and commands the negative gradient,
  \[ \vect{F}_{\mathrm{att}} = -k_{\mathrm{att}} (\pos - \pos_{\mathrm{g}}), \qquad \vect{F}_{\mathrm{rep}} = k_{\mathrm{rep}} \Bigl( \frac{1}{\rho} - \frac{1}{\rho_0} \Bigr) \frac{\nabla\rho}{\rho^{2}}. \]
