...
convoys through a corridor and the plan of \cref{sec:ch07-example}; a
rotation on a fully occupied cycle keeps the same distance, so cycle
...
and $(1,3)$, and $(1,3)$ is a dead end because $a_1$ parks at $(0,3)$; so the
pocket $(1,1)$ must be used, and $a_3$ has to clear it first. The brute-force
...
yardstick of \ecbs (\cref{ch:ch10}); the gap between them and the optimal
...
\begin{figure}[tb]
  \centering
  \resizebox{\textwidth}{!}{\inputfigure{ch07/solver-families}}
...
\paragraph{What the families do.}
The optimal search-based solvers pay for optimality with exponential
worst-case time, as \cref{thm:ch07-nphard} demands, and differ in what they
search: \cbs~\cite{sharon2015cbs}\index{CBS!as solver family} a tree of
constraints, branching on the first conflict that the detector of
\cref{sec:ch07-detection} reports; \mstar the joint space, coupling only the
agents that actually conflict; ICTS\index{ICTS} the vectors of path costs in
increasing order. \ecbs~\cite{barer2014ecbs} relaxes \cbs with focal search
and returns a solution of cost at most $w$ times the optimum for a chosen
$w \ge 1$, typically $1.1$ to $1.5$; for a drone swarm this is the family to
start from, because a few per cent of extra flight time buys an order of
magnitude in the number of agents. Prioritized
planning\index{prioritized planning}~\cite{silver2005cooperative} fixes an
order of the agents and plans each with space-time \astar around the paths
of the earlier ones; it is fast and simple, but it can fail on solvable
instances and its plans can be far from optimal, as \cref{ch:ch08} shows
with counterexamples. Push-and-Swap and
Push-and-Rotate~\cite{luna2011push,dewilde2014push}\index{Push-and-Rotate}
are rule-based: they move the agents to their goals one by one and resolve
blockages with local manoeuvres, and Push-and-Rotate is the complete
algorithm behind \cref{thm:ch07-feasible}.

\paragraph{Reductions and learning.}
Reduction-based solvers fix a makespan bound $B$, encode ``is there a
solution with $\makespan \le B$?'' as a Boolean formula or an integer
program with one variable per agent, vertex and time step, hand it to a SAT
or ILP solver, and increase $B$ until the answer is yes; the first yes is
makespan-optimal by construction, and \cref{ch:ch22} shows the
integer-programming encoding~\cite{yu2016optimal}. Learned solvers such as
PRIMAL~\cite{sartoretti2019primal} train a decentralised policy by imitation
of an optimal solver and by reinforcement learning, so that each agent
chooses its next move from a local observation. Such policies scale and
react, but they guarantee neither completeness nor collision freedom, so a
plan they produce must pass the validator of this chapter and a classical
solver must stand by as a fallback.
...
trees on the game maps). The older single-agent maps also contain \code{G}
and \code{S} for passable ground and swamp and \code{O} and \code{W} for
out-of-bounds and water; our parser accepts \code{.}, \code{G} and \code{S}
as free and treats every other character as blocked. The cell in row $y$ and
...
treat as informative only. The coordinates are $x$ (column) first and $y$
...
and, for the random and maze maps, the obstacle density or the corridor
width. The set of maps has grown since the benchmark was published; take the
current list from the website, and name the exact maps and scenarios in any
result you report.
...
and unit speed, and its time steps are synchronous. It is the right tool for
comparing MAPF solvers with each other, which is how \cref{ch:ch08} to
\cref{ch:ch11} use it, and the wrong tool for predicting how a swarm of
drones will behave: it says nothing about altitude, kinematics, sensing or
intruders, and a success rate quoted without the map, $k$ and the time
limit is meaningless. \Cref{ch:ch25} builds the drone-specific scenarios on
top of these files rather than instead of them.
...
infinity''. The same convention makes the horizon $T$ of
\cref{alg:ch07-pairwise} sufficient: beyond $T$ nothing moves, and two
parked agents never share a cell because goals are distinct.
\end{pitfall}
...
(\cref{exr:ch07-separation}). The conflict types of \cref{def:ch07-conflicts}
are the classical ones; which pairs of moves are physically safe is a
...
Given all this, why not plan drones directly in continuous space and time?
Because classical MAPF isolates the part of the problem that is
combinatorial, who passes first at each shared cell, from the part that is
continuous, how exactly a drone flies from one cell to the next; and it is
the combinatorial part that is NP-hard (\cref{thm:ch07-nphard}) and needs a
complete, optimal or bounded-suboptimal search. The discrete plan fixes an
order of visits at every cell, and this order is what MAPF-POST and the
local layers of \cref{ch:ch24} preserve while they adapt speeds and
trajectories. The guarantees, completeness and optimality within a factor
$w$, are proved at the discrete level and survive as long as the cell side,
the time step and the safety margins are chosen so that the drone model of
\cref{ch:ch02} can execute every move and every hover
(\cref{exr:ch07-separation} is the kind of check this requires). The
discrete plan is also what the detector re-checks after the local layer has
changed a route, and what makes results comparable on the benchmark.
...
\cref{exr:ch07-conflict-cases,exr:ch07-coding} test.
\end{dronebox}
...
\begin{itemize}
  ...
  \item Classical MAPF forbids vertex and swapping conflicts (edge conflicts are vertex conflicts in disguise) and allows following and cycle conflicts.
...
\cref{thm:ch07-bounds} can hold with equality, by giving a plan for each
...
and $\makespan = 4$. Use \cref{thm:ch07-bounds} and the fact that the shortest paths of $a_1$
and $a_2$ are unique to explain, without any search, why no solution can
...
(a) Prove the claim of the walkthrough of \cref{alg:ch07-pairwise}: if a
plan has a vertex or swapping conflict at some time $t > T$, then it has a
vertex conflict at time $T$. (b) Under disappear-at-target an agent is
...
(a) Without looking at \code{ch07\_mapf.py}, implement a plan validator and
both conflict detectors of \cref{sec:ch07-detection}; test them on the cases
of \cref{exr:ch07-conflict-cases} and on \cref{ex:ch07-grid}, and check
that the two detectors agree on random plans. (b) Write the \code{.map} and
...
grids with a test for crossing diagonals, and show a plan that the
4-connected detector accepts although two agents collide physically.
(d) The coding exercise of Week~3 (\cref{ch:appA}): write a first
