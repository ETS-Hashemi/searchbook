...
After the line \code{version 1}, every line describes one agent with the nine
fields of \cref{tab:ch07-scen}. The coordinates are given as $x$ (column)
first and $y$ (row) second, the opposite of our $(r, c)$; transposing them is
the most common bug in home-made parsers, and it goes unnoticed on square
maps until an agent starts inside a wall. The last field is the length of
the agent's shortest path as computed by the benchmark's own tools; treat it
as informative only and recompute distances with your own search. An
\emph{instance} is made from a scenario by taking its first $k$ agents. The
...
\begin{table}[tb]
  \centering\small
  \caption{The fields of a \code{.scen} line, with the values of the first
  agent of \cref{lst:ch07-files}, whose start $(x, y) = (0, 0)$ is the cell
  $(0, 0)$ and whose goal $(3, 2)$ is the cell $(r, c) = (2, 3)$.}
  \label{tab:ch07-scen}
  \begin{tabular}{@{}lll@{}}
  \toprule
  Field & Meaning & Example\\
  \midrule
  bucket & distance group of the single-agent benchmark; ignored by MAPF solvers & \code{0}\\
  map & name of the \code{.map} file & \code{small.map}\\
  width, height & size of the map; must match the header of the map file & \code{4}, \code{3}\\
  $s_x$, $s_y$ & start column and start row & \code{0}, \code{0}\\
  $g_x$, $g_y$ & goal column and goal row & \code{3}, \code{2}\\
  length & shortest single-agent path length reported by the benchmark; recompute it & \code{5}\\
  \bottomrule
  \end{tabular}
\end{table}
...
intruders. Results also depend strongly on the family, on $k$ and on the
time limit, so a success rate quoted without all three is meaningless.
\Cref{ch:ch25} builds the drone-specific scenarios on top of these files
...
has a bug, however plausible its plan looks. \Cref{lst:ch07-scen} is the
scenario parser; note the swap of $x$ and $y$ in its last line. The
...
\begin{lstlisting}[caption={Parsing a \code{.scen} file into $(r,c)$ cells (from \code{code/ch07\_mapf.py}).},label={lst:ch07-scen}]
...
swaps only for $t = 0, \dots, T-1$, since no move starts at $T$. Finally, a
conflict at $t = 0$ is impossible in a legal plan because starts are
distinct, yet a validator must check it anyway: a solver that returns a
path starting at the wrong cell is exactly the kind of bug it exists to
catch.
\end{pitfall}
...
introduced for warehouse pick-up and delivery, replaces the single goal of
\cref{def:ch07-instance} by a stream of goals and re-plans in a rolling
window, so the stay-at-target convention is relaxed to ``stay until the next
task''. \cref{ch:ch08} touches windowed planning, and the replanning loop of
\cref{ch:ch24} is a lifelong problem in disguise.
...
\cref{ch:ch02} can execute every move and every hover; the geometry of
\cref{exr:ch07-separation} is the kind of check this requires. The discrete
plan is also what the detector re-checks after the local layer has changed a
route, and it is what makes results comparable on the benchmark of
\cref{sec:ch07-benchmarks}. Continuous-time solvers push the boundary, but
they keep the structure of \cbs and the conflict types of this chapter; they
change what a conflict is geometrically, not what a solver does with it.
...
\begin{itemize}
  ...
  \item Stay-at-target: an agent that has arrived keeps its goal cell for ever and blocks it; the cost of a path is the time of its final arrival, and trailing waits at the goal are free.
  \item Of the five conflict types of Stern et al., classical MAPF forbids vertex and swapping conflicts (edge conflicts are vertex conflicts in disguise) and allows following and cycle conflicts.
  \item $\sumcost(\Pi) = \sum_i \cost(\pi_i)$ and $\makespan(\Pi) = \max_i \cost(\pi_i)$; both are bounded below by the naive plan of independent shortest paths, and $\makespan \le \sumcost \le k\,\makespan$. The two objectives can disagree already for three agents.
  \item Conflict detection over the padded paths costs $\bigO{k^2 T}$ pairwise and $\bigO{kT}$ expected with hash maps; scanning up to the horizon $T$ suffices.
  \item Feasibility is polynomial when the graph is connected and two vertices are empty; optimality is NP-hard for both objectives. Solver families trade this hardness for guarantees: optimal (\cbs, \mstar), bounded-suboptimal (\ecbs), unbounded (prioritized planning, Push-and-Rotate), reduction-based (SAT, ILP) and learned.
  \item Instances come as \code{.map} files (\code{.} free, \code{@} and \code{T} blocked) and \code{.scen} files ($x$ = column, $y$ = row); the first $k$ agents of a scenario form an instance, and success rate against $k$ is the standard measure.
  \item Drones fly on 3D lattices with kinematic post-processing or continuous-time variants, but the classical problem remains the right abstraction: it holds the combinatorial core and the guarantees.
\end{itemize}
\end{summary}

\paragraph{Further reading.}
The definitions of this chapter follow Stern et al.~\cite{stern2019mapf},
which also introduces the benchmark and surveys the variants; read it in
full. Felner et al.~\cite{felner2017search} survey the search-based optimal
solvers and their open problems, and Sharon et al.~\cite{sharon2015cbs} is
the reference for \cbs, the solver of \cref{ch:ch09}. Complexity: Surynek
\cite{surynek2010optimization} and Yu and LaValle~\cite{yu2013structure} for
hardness, Kornhauser et al.~\cite{kornhauser1984pebble} for pebble motion
and de Wilde et al.~\cite{dewilde2014push} for the complete polynomial
algorithm. Silver~\cite{silver2005cooperative} is the origin of cooperative
\astar and of the reservation table of \cref{ch:ch08}. For drones,
H\"onig et al.~\cite{honig2016mapf,honig2018trajectory} on kinematic
constraints, Andreychuk et al.~\cite{andreychuk2019ccbs} on continuous time
and Ma et al.~\cite{ma2017lifelong} on lifelong MAPF.
...
and $\makespan = 4$. Use \cref{thm:ch07-bounds} to explain, without any
search, why no solution can have $\sumcost = 9$ and why $\makespan = 4$ forces
at least two agents to be delayed.
\end{exercise}
...
\code{random-1}, build instances with the first $k = 10, 20, \dots, 100$
agents, plan every agent alone, count the conflicts of the naive plan and
time both detectors as $k$ grows. (c) Extend the detector to 8-connected
...
corridor with one pocket of \cref{exr:ch07-corridor} the planner fails for
one order of the two agents and succeeds for the other. \cref{ch:ch08}
develops this planner properly.
\end{exercise}
