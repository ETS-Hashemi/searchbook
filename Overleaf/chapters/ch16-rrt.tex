...
$r$, a configuration is the position of its centre, $\state\in\R^{d}$ with $d=2$
...
$\tau\colon[0,1]\to\Xfree$ with $\tau(0)=\state_{\mathrm{init}}$ and
...
\begin{itemize}
  ...
  \item $\Steer(\state_{\mathrm{near}},\state_{\mathrm{rand}})$\index{RRT!Steer} moves from
        $\state_{\mathrm{near}}$ towards $\state_{\mathrm{rand}}$ by at most $\eta$:
        \begin{equation}
...
\begin{figure}[tb]
  ...
  $\eta$ towards the sample and proposes $\state_{\mathrm{new}}$;
  ...
  $\state_{\mathrm{new}}$ towards $\state'_{\mathrm{rand}}$, is rejected because its
...
$\state_0=\state_{\mathrm{init}},\state_1,\dots,\state_m=\state_{\mathrm{goal}}$ along $\tau$ with
...
$3\nu+\nu=4\nu\le\delta_{c}$ of $\state_{k+1}\in\tau$, and a ball is convex, so the
...
Line~\ref{alg:ch16-connect:extend} extends the current tree $\mathcal{T}_a$ towards a
...
\KwIn{a collision-free polygonal path $\tau=(\state_0,\dots,\state_k)$, number of attempts $K$}
\KwOut{a collision-free path with the same endpoints that is no longer than $\tau$}
\Fn{\RrtShortcutPath{$\tau$, $K$}}{
  \For{$i\gets1$ \KwTo $K$}{
    draw $s_1<s_2$ uniformly from $[0,\mathrm{length}(\tau)]$ and let $\vect{a}=\tau(s_1)$, $\vect{b}=\tau(s_2)$ be the points at these arc lengths\;
    \If{$\vect{a}$ and $\vect{b}$ lie on different segments of $\tau$ \KwAnd \RrtSegmentFree{$\vect{a}$, $\vect{b}$}}{
      replace the part of $\tau$ between $\vect{a}$ and $\vect{b}$ by the segment $[\vect{a},\vect{b}]$\label{alg:ch16-shortcut:replace}\;
    }
  }
  \KwRet{$\tau$}\;
}
\Fn{\RrtPrunePath{$\tau$}}{
  $\tau'\gets(\state_0)$; $i\gets0$\;
  \While{$i<k$}{
    $j\gets$ the largest index $>i$ with \RrtSegmentFree{$\state_i$, $\state_j$}\label{alg:ch16-shortcut:jump}\;
    append $\state_j$ to $\tau'$; $i\gets j$\;
  }
  \KwRet{$\tau'$}\;
}
\end{algorithm}
