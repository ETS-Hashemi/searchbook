...
\Fn{\RrtShortcutPath{$\tau$, $K$}}{
  \For{$i\gets1$ \KwTo $K$}{
    draw $s_1<s_2$ uniformly from $[0,\mathrm{length}(\tau)]$ and let $a=\tau(s_1)$, $b=\tau(s_2)$ be the points at these arc lengths\;
...
\rrt cannot recognise an infeasible query and failure means only ``no path
...
that each one is chosen as $x_{\mathrm{near}}$ by the next sample, using
\cref{eq:ch16-voronoi}. Then show in general that for a tree that is a single
straight chain of $k+1$ vertices spaced $\eta$ apart, centred in a large
