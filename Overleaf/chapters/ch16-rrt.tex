...
darts the probability that $v$ is \emph{selected} equals the area of its
...
tree one step of length $\eta$ towards it. A vertex is selected with
...
$r_{\mathrm{goal}}>0$. Suppose there is a path $\tau$ of finite length from
$x_{\mathrm{init}}$ to $x_{\mathrm{goal}}$ with \textbf{clearance}\index{clearance}
...
compact, so finitely many of the balls of radius $\delta_{c}/2$ centred on
its points cover it, and joining the centres of consecutive overlapping
...
$65\,\%$ above the $17.20$ reference and $70\,\%$ above the zero-clearance
optimum $16.72$. Over the $100$ runs the lengths range from $20.55$ to
$28.35$, with the mean and spread of \cref{tab:ch16-connect}; not one run
comes within $15\,\%$ of the reference. Two features of \cref{alg:ch16-rrt}
...
\Fn{\RrtKinoGrow{$\state_{\mathrm{init}}$, $\pos_{\mathrm{goal}}$, $N$}}{
  ...
  \For{$i\gets1$ \KwTo $N$}{
    ...
    $\mathrm{best}\gets\KwNil$; $\acc_{\mathrm{best}}\gets\KwNil$\tcp*{convention $\rho(\KwNil,\cdot)=+\infty$}
    \For{$j\gets1$ \KwTo $m$}{
...
cutting a corner moves the path towards the inside of the turn, which is
exactly where the obstacle that caused the corner sits. \Cref{ch:ch21} takes
the better route of optimising the trajectory subject to the dynamics and
the obstacles instead of repairing it afterwards.
...
a small constant, about $0.05$~ms for $n=10^{3}$ vertices, $0.3$~ms for
$10^{4}$ and $2.9$~ms for $10^{5}$ in three dimensions, means of $200$
queries timed and printed by the self-test of \code{ch16\_rrt.py} on the
machine that produced this book. For the trees of this chapter the scan is not the bottleneck, and it
...
seconds (it prints its own runtime, $4.5$~s here) and re-checks in one place
...
and the nearest-neighbour timings quoted above.
...
wall of thickness $0.02$ with $\delta=0.05$, and the inflated test rejects
every one of the forty directions and forty crossings it tries.
\end{pitfall}
...
grid lets many agents share one discretisation, which the multi-agent
