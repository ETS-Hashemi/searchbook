...
nearest-neighbour search and parameters, and \cref{sec:ch16-choice} the
...
the dart, moves from that vertex a fixed distance $\eta$ towards the dart,
...
region of vertex $v$ makes $v$ the parent of the new vertex, so with uniform
darts the probability that $v$ is extended equals the area of its region
divided by the area of the map. The tips of branches that point into empty
space own the largest regions, so they are extended most often, and each
...
the tree is pulled towards the largest unexplored regions until they have
been filled.
...
$r$, a configuration is the position of its centre, $x\in\R^{d}$ with $d=2$
...
$x_{\mathrm{init}}$ and one directed edge $(x_{\mathrm{parent}},x)$ per
...
how to do this without ever missing an obstacle. Write $\dist(x,\mathcal{O})$
for the Euclidean distance from $x$ to the workspace obstacles $\mathcal{O}$;
...
$\dist(x,\mathcal{O})>r$, and let $\delta>0$ be the \textbf{check
...
for every $i$, then every point $p$ of $[a,b]$ has $\dist(p,\mathcal{O})>r$, that is,
the segment is collision-free.
\end{proposition}
\begin{proof}
Take any $p\in[a,b]$. It lies between two consecutive test points, so its
distance to the nearer of them, $p_i$ say, is at most $\delta/2$. The function
$x\mapsto\dist(x,\mathcal{O})$ is $1$-Lipschitz (an infimum of distances,
each of which changes by at most $\norm{p-p_i}$ when $p_i$ is replaced by
$p$), so
...
an obstacle, and needs $\lceil\norm{b-a}/\delta\rceil+1$ point tests. It
...
towards the goal once in a while instead of waiting for a uniform sample to
...
segment is free, the new vertex joins the tree with $x_{\mathrm{near}}$ as
...
vertex lies in the goal ball: exact equality with $x_{\mathrm{goal}}$ never
...
finally appends $x_{\mathrm{goal}}$ itself when the segment from the entry
...
$\textsc{Extend}(T,x)$ with three outcomes: \textsc{Trapped} if the segment
was blocked, \textsc{Advanced} if a new vertex was added short of $x$, and
\textsc{Reached} if the new vertex is $x$ itself, which happens exactly when
$\norm{x-x_{\mathrm{near}}}\le\eta$. \Cref{alg:ch16-connect} is written in
these terms.

\paragraph{The Voronoi bias, quantified.}
With $p_{\mathrm{goal}}=0$ the sample is uniform on $\mathcal{X}$, so the
probability that vertex $v$ is chosen by line~\ref{alg:ch16-rrt:nearest} is
\begin{equation}
...
\end{equation}
the relative volume of its Voronoi region. A vertex whose region is large is
far from the other vertices in some direction, and extending it enlarges the
tree in exactly that direction. LaValle \cite{lavalle1998rrt} observed that
...
always extend the vertex nearest to the goal, which helps when that vertex
has a clear view of the goal and hurts when it sits against an obstacle
(\cref{sec:ch16-pitfalls}).
...
left of the wall $[3,5]\times[0,6]$, and every step towards them crosses the
wall and is rejected: the tree does not grow in these iterations.}
...
from the start. The next samples lie towards the north-east, and the tree
...
at $x=3$ blocks every step towards the right half of the map. The tree is now
...
of \cref{fig:ch16-snapshots} shows only $17$ vertices after $50$ iterations:
...
$x_{\mathrm{goal}}$ is free and is appended, which gives a path of $50$
vertices and length $23.85$. Of the $402$ iterations, $158$ were rejected by
the collision check, and the checks tested $3\,965$ points, about ten per
...
$(8,4)$ and has length $16.72$ (in the limit of zero clearance; with a
clearance of $0.075$ it has length $17.20$), so the \rrt path is $43\,\%$
longer. \Cref{sec:ch16-properties} explains why this is not bad luck, and
\cref{sec:ch16-smoothing} how most of the excess is removed afterwards.
...
\emph{Claim: if the tree has a vertex $v\in B_k$ and the current sample
satisfies $x_{\mathrm{rand}}\in B_{k+1}$, then the iteration adds a vertex in
$B_{k+1}$.} Let $v'=\Nearest(V,x_{\mathrm{rand}})$. Then
...
\begin{itemize}
  \item \emph{The role of $\eta$.} The step size only enters through $\nu\le\eta/3$:
        ...
        not hurt, because \textsc{Steer} moves \emph{at most} $\eta$ and
  ...
  \item \emph{Narrow passages.} The ball volume is proportional to $\nu^{d}$, so
        ...
        \cref{sec:ch16-connect}. In \cref{ex:ch16-map} the gap between the
        walls is one unit wide on a ten-unit map, wide enough to be found
...
$65\,\%$ above the optimum of $17.20$ (for the clearance $0.075$ of the
reference computation). Over the $100$ runs the length averages $23.87$ with
standard deviation $1.84$, minimum $20.55$ and maximum $28.35$; not one run
comes within $15\,\%$ of the optimum. Two features of \cref{alg:ch16-rrt}
...
each edge points towards a random sample, not towards the goal.
...
Let $n$ be the number of vertices. One iteration costs one nearest-neighbour
...
\textsc{Connect} returns \textsc{Reached}, $x_{\mathrm{new}}$ belongs to both
...
($518\to425$ median iterations), a large one hurts badly ($745$), because
half of the samples then select the vertex nearest to the goal, which for
most of the run sits against a wall, and the extension is rejected. The tree
...
planner cannot move ``towards'' a sampled state along a straight line,
...
candidate vertex, far too expensive inside a nearest-neighbour query. The
...
line~\ref{alg:ch16-kino:nearest} is a design decision that strongly affects
the growth of the tree. Kinodynamic planning has a large literature
...
waypoints, which must then be collision-checked again.
...
the roadmap. PRM is a \emph{multi-query} method that spends its effort once
per map; \rrt is a \emph{single-query} method that spends it once per query
and never builds a graph of the whole space. For a drone whose map changes
and whose queries differ every time, the single-query tree is the natural
choice, and it is the only one that extends to dynamics without an exact
steering function. \rrtstar and \irrtstar
...
a small constant, about $0.02$~ms for $n=10^{3}$ vertices, $0.2$~ms for
$10^{4}$ and $2.3$~ms for $10^{5}$ in two or three dimensions on an ordinary
laptop. For the trees of this chapter the scan is not the bottleneck, and it
needs no rebuilding when a vertex is added. Beyond $10^{4}$ vertices, or when
the planner is called many times per second, use a
...
iteration and insertion into a balanced kd-tree is awkward, implementations
rebuild the tree every few hundred insertions and scan the vertices added
since, or use a bucket grid over $\mathcal{X}$;
...
\Cref{tab:ch16-parameters} lists the parameters. Two of them are most often
mishandled. The \emph{seed}: \rrt is randomised, and both debugging and
reporting need reproducibility, so seed the generator
(\code{numpy.random.default\_rng(seed)}) and report statistics over many
seeds, as in \cref{tab:ch16-connect}, never a single run. The \emph{budget}:
because \rrt cannot recognise an infeasible query, choose $N$ from the time
you can afford and treat failure as ``no path found within the budget''.
...
\code{extend} and \code{connect} of \cref{alg:ch16-connect} are in the same
file, and \code{rrt\_connect} is the loop of the algorithm with the swap
\code{t\_a, t\_b = t\_b, t\_a} at the end of each iteration. The self-test
runs in about two seconds and checks the collision tests on thin walls,
spheres and inflated boxes; that every returned path is collision-free at
ten times finer resolution, starts at the start, ends at the goal and has
edges no longer than $\eta$; that the worked example is reproducible; that
shortcutting and pruning never increase the length; that \rrt-Connect needs
fewer iterations than \rrt in median and mean over $40$ seeds; that the 3D
scene is solved by both planners; and that the kinodynamic trajectory obeys
the double integrator and its limits.
...
segment. The self-test of \code{ch16\_rrt.py} contains exactly this trap, a
wall of thickness $0.02$ with $\delta=0.05$, and the inflated test catches it
at every angle.
\end{pitfall}
...
Use a grid and \astar when the space is two-dimensional or a coarse 3D
lattice is accurate enough, when many agents must share one discretisation
(the multi-agent planners of \cref{ch:ch07,ch:ch08,ch:ch09,ch:ch10,ch:ch11}
all need a graph), or when an exact optimal answer on the graph matters. Use
\rrt when the space is large or high-dimensional, when a feasible path is
needed quickly and its quality can be repaired afterwards, or when the
dynamics must be respected during planning. Use \rrtstar when path quality
matters and the time is available. \Cref{exr:ch16-coding} asks you to
measure these trade-offs yourself.
...
share the map but work at different scales. When a segment between two
consecutive waypoints of the grid path becomes infeasible, the local planner
runs \cref{alg:ch16-rrt} or \cref{alg:ch16-connect} from the current
position to the next reachable waypoint, with a goal region of about one
cell and with the obstacles inflated by the drone radius, the tracking
error and the prediction uncertainty of \cref{ch:ch19,ch:ch20}. The result
is shortcut (\cref{alg:ch16-shortcut}), converted into a trajectory by the
...
budget, the event is escalated to a global replan, exactly the decision
logic of \cref{ch:ch24}. In the other direction, the waypoints of the grid
path can seed the goal bias of the tree, which turns the global plan into a
heuristic for the local sampler.
...
failure within a known time. It guarantees nothing about optimality
...
study plan (\cref{ch:appA}) covers this chapter and \cref{ch:ch17}, and its
...
\begin{itemize}
  ...
  \item The Voronoi bias explains the fast exploration: a vertex is
        extended with probability equal to the relative volume of its
        Voronoi region, so the tree is pulled into the largest unexplored
        regions.
