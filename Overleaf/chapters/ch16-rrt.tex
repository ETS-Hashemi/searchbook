...
\Fn{\RrtExtendTree{$T$, $x$}}{
  ...
  \If{\RrtSegmentFree{$x_{\mathrm{near}}$, $x_{\mathrm{new}}$}}{
    ...
    \lIf{$x_{\mathrm{new}}=x$}{\KwRet{\textup{\textsc{Reached}}}}
    \KwRet{\textup{\textsc{Advanced}}}\;
  }
  \KwRet{\textup{\textsc{Trapped}}}\;
}
\Fn{\RrtConnectGreedy{$T$, $x$}}{
  \Repeat{$s\neq\textup{\textsc{Advanced}}$}{$s\gets$ \RrtExtendTree{$T$, $x$}\label{alg:ch16-connect:greedy}\;}
  \KwRet{$s$}\;
}
\Fn{\RrtConnectPlan{$x_{\mathrm{init}}$, $x_{\mathrm{goal}}$, $N$}}{
  ...
  \For{$i\gets1$ \KwTo $N$}{
    ...
    \If{\RrtExtendTree{$T_a$, $x_{\mathrm{rand}}$} $\neq\textup{\textsc{Trapped}}$\label{alg:ch16-connect:extend}}{
      ...
      \If{\RrtConnectGreedy{$T_b$, $x_{\mathrm{new}}$} $=\textup{\textsc{Reached}}$\label{alg:ch16-connect:test}}{
...
nearest neighbours by collision-checked segments, and answer any number of
...
\cref{alg:ch16-connect} in the same file. The self-test runs in about two
seconds and checks the collision tests on thin walls, spheres and inflated
boxes; that every returned path is collision-free at ten times finer
resolution, joins start and goal and has edges no longer than $\eta$; that
the worked example is reproducible; that
...
(c)~Let $d_0=\dist(a,\mathcal{O})-r$ be the free distance of the first
endpoint. Explain why no point of the segment within $d_0$ of $a$ needs to
