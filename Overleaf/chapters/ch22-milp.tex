...
drone swarm needs. A \emph{moving} obstacle whose predicted position at
step $k$ comes from \cref{ch:ch20} is a different rectangle $O_k$ per step,
inflated by the prediction uncertainty; the encoding does not change. A
\emph{three-dimensional} box needs six binaries per step, and a convex
polygon or polyhedron with $r$ faces needs $r$; a Euclidean separation
disc is approximated from inside by a regular polygon with $r$ sides at the
cost of $r$ binaries per pair and step. The \emph{minimum-time} objective
\cref{eq:ch22-time} and the time-window constraints of the exercises turn
the MILP into a scheduler. Run in a \emph{receding horizon}, the MILP
...
of the bound, at $N = 24$ the gap exceeds $50$\,\%. GAPFOURSIX The number
of binaries, \cref{tab:ch22-binaries}, is not the whole story; what the
...
almost unchanged ($12.04$~m/s), but the solve takes about $20$~s and
$4\,000$ nodes instead of $1.8$~s and $366$. A coarse step with the margins
