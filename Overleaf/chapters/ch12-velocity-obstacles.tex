...
argument of \cref{thm:ch02-disc-minkowski}, two discs of radii $r_A$ and $r_B$ overlap exactly
...
\cref{def:ch02-single-integrator}): its centre obeys $\pos(t) = \pos + t\,\vel$ as long as the
...
argument for two discs (\cref{thm:ch02-disc-minkowski}), the discs overlap at time $t$ if
...
centre to the line; \cref{def:ch02-point-segment-distance}). If
...
grazing: the discs touch at a single instant. The worked example of
...
every scaled disc. It remains to identify the boundary, and for that we use
...
is the part of the cone lying at or beyond that disc, and its boundary is the
near arc of the disc together with the two legs beyond their points of
...
is in $\VO^{\ttc}_{A|B}$, and without avoidance the discs would overlap from
...
a miss because the relative velocity $(1.2,-0.9)$ passes above the disc
...
small northward component, $\vel_A = (1.046, 0.092)$, at $t = 5.0\,\mathrm{s}$ the centres are
...
because the centre distance on $[k\dt, (k+1)\dt]$ depends only on the
...
\begin{table}[tb]
  ...
  centre distance over the run, ``pairs $<R$'' counts the pairs of agents whose
  centre distance dropped below $R$ at some step, and ``empty'' the number of
...
manoeuvre is mostly ``speed up and pass in front''. Among agents that all
...
oscillate around the crowded centre. None of these are head-on
...
replaces the cone by the union of the discs swept along a predicted
trajectory, is due to Large, Sekhavat, Shiller and
Laugier~\cite{large2002nonlinear}. Implementations that face many neighbours
