...
\begin{objectives}
\begin{itemize}
  \item Explain why local collision avoidance is a choice of \emph{velocity} rather than a choice of path, and set up an encounter between two disks in relative coordinates.
  \item Construct the collision cone and the velocity obstacle of a moving disk by hand: apex, tangent rays and the half-angle $\theta=\arcsin\bigl((r_A+r_B)/\norm{\pos_B-\pos_A}\bigr)$.
  ...
  \item Implement a velocity-obstacle simulator in \python, measure minimum separation and path length, and recognize the oscillation that appears when two agents avoid each other at the same time.
\end{itemize}
\end{objectives}
...
did not foresee. A delivery drone follows its conflict-based-search (\cbs) route across a park at
...
point of view, in which the intruder is a disk that moves with the
...
to the velocity the drone wanted to fly anyway makes the maneuver small. The
...
\emph{local safety layer}. The global planner (\cbs or \ecbs, enhanced
\cbs, \cref{ch:ch09,ch:ch10}) produces nominal routes; the prediction layer
...
once the danger has passed. The reciprocal variants of \cref{ch:ch13} ---
...
Two disks move through the plane with constant velocities. Whether they will
...
$\pos_B - \pos_A$ that drifts at the rate $\vel_B - \vel_A$, the negative of
the relative velocity of \cref{def:ch12-relative}.
Two more simplifications make the picture crisp. First, by the Minkowski-sum
argument of \cref{thm:ch02-disc-minkowski}, two disks of radii $r_A$ and $r_B$ overlap exactly
when their centers are closer than $r_A + r_B$; so we may shrink $A$ to a point
and fatten $B$ to a disk of radius $r_A + r_B$. Second, instead of letting the
fat disk drift toward the point, let the point move toward the fixed disk
...
relative velocity. It hits the disk if and only if the ray intersects the disk
(\cref{fig:ch12-idea}). The directions whose rays hit the disk form a wedge
between the two tangent lines from the origin to the disk. This wedge is the
...
\begin{figure}[tb]
  ...
  agent $B$ is the disk of radius $r_A+r_B$ (its own radius is drawn inside)
  ...
  disk: the red ray hits, the gray ray grazes the disk along a tangent, the
...
\begin{keyidea}
Look at an encounter from the agent's own frame: the other agent is a disk of
radius $r_A + r_B$, and the relative velocity $\vel_A - \vel_B$ is dangerous
exactly when its ray hits that disk. The dangerous relative velocities form a
...
Throughout, an \textbf{agent}\index{agent!disk model} is a disk that moves as
a point mass with a commanded velocity (the point-mass drone model of
\cref{def:ch02-single-integrator}): its center obeys $\pos(t) = \pos + t\,\vel$ as long as the
velocity $\vel$ is held. Agent $A$ has center $\pos_A$, velocity $\vel_A$ and
...
agents keep their velocities, the center of $B$ seen from the center of $A$ is
\begin{equation}
  \pos_B(t) - \pos_A(t)
...
\end{equation}
which is the position of a \emph{fixed} disk center $\pos_{\mathrm{rel}}$ seen
from a point that has moved to $t\,\vel_{\mathrm{rel}}$. By the Minkowski-sum
argument for two disks (\cref{thm:ch02-disc-minkowski}), the disks overlap at time $t$ if
...
\end{equation}
where $\disc{\vect{c}}{r}$ is the closed disk of radius $r$ around $\vect{c}$.
The disk $\disc{\pos_{\mathrm{rel}}}{R} = \disc{\pos_{\mathrm{rel}}}{r_B}
\oplus \disc{\vect{0}}{r_A}$ is the Minkowski sum\index{Minkowski sum!in velocity obstacles} of $B$'s disk with $A$'s disk placed at the origin: $A$ has
...
$\set{t\,\vel_{\mathrm{rel}} : t > 0}$ from the origin intersects the disk
...
ray in direction $\vel$ hits the disk, so does the ray in direction $2\vel$,
...
$\set{t\vect{u} : t \ge 0}$ meets the disk if and only if the distance from the
center $\pos_{\mathrm{rel}}$ to the ray is at most $R$. If $\varphi \le \pi/2$,
the point of the line $\set{t\vect{u} : t \in \R}$ closest to the center is at
$t^* = \pos_{\mathrm{rel}}\cdot\vect{u} = d\cos\varphi \ge 0$, hence on the ray,
and its distance to the center is $d\sin\varphi$ (the perpendicular from the
center to the line; \cref{def:ch02-point-segment-distance}). If
$\varphi > \pi/2$, the closest point of the ray is the origin itself, at
distance $d > R$, and the ray misses. Therefore the ray meets the disk if and
...
$\varphi = \theta$ has distance exactly $R$ to the center, so it touches the
...
are the tangent points of \cref{thm:ch02-tangents}. \qedhere
\end{proof}
...
toward $B$ is dangerous. Forgetting to add $r_A$ to $r_B$ is the most common
...
$A$ must not command. It is a wedge with its \textbf{apex}\index{velocity obstacle!apex} at $\vel_B$, its axis parallel to $\pos_{\mathrm{rel}}$ and the
...
relative directions sits at $A$ and the inflated disk around $B$; in the
...
\begin{figure}[tb]
  ...
  inflated disk of radius $R=1$; the tangents from the center of $A$ enclose
  ...
  circle is the truncation disk for $\ttc=5\,\mathrm{s}$
...
velocity $\vel_{\mathrm{rel}}$ is the first time at which the disks touch,
\begin{equation}
...
\end{equation}
This is the ray--circle intersection of \cref{thm:ch02-ray-circle} with the ray starting at
the origin in the direction $\vel$ and the circle centered at $\pos$.
...
\begin{equation}
  ...
  \label{eq:ch12-ttc}
\end{equation}
the smaller root of \cref{eq:ch12-quadratic}. The disks overlap exactly during
...
upward with the value $d^2 - R^2 > 0$ at $t = 0$. It is non-positive exactly
between its two real roots, if they exist, so the disks overlap exactly on that
...
directions that point away from the disk. A velocity with $\Delta = 0$ is
grazing: the disks touch at a single instant. The worked example of
\cref{sec:ch12-example} evaluates \cref{eq:ch12-ttc} on the encounter of
\cref{fig:ch12-geometry}.
...
In relative-velocity coordinates the truncated cone is a union of disks,
\begin{equation}
  \VO^{\ttc}_{A|B}(\vel_B) - \vel_B
  = \bigcup_{0 < t \le \ttc} \disc{\pos_{\mathrm{rel}}/t}{R/t} .
  \label{eq:ch12-union}
\end{equation}
Every disk $\disc{\pos_{\mathrm{rel}}/t}{R/t}$ is inscribed in the collision
cone (tangent to both legs), and the disks shrink and move toward the apex as
$t$ grows. Consequently $\VO^{\ttc}_{A|B}$ is the cone with its tip cut off by
the disk $\disc{\vel_B + \pos_{\mathrm{rel}}/\ttc}{R/\ttc}$: its boundary consists
of the two legs beyond their points of tangency with this disk and of the arc
of the disk that faces the apex.
\end{proposition}
\begin{proof}
...
$\vel$ collides at some $t \le \ttc$, \cref{eq:ch12-union} follows. The disk for
...
every scaled disk. It remains to identify the boundary, and for that we use
the scaling identity that \cref{eq:ch12-collision} gives at once: for every
$s > 0$ the point $t(s\vel)$ is the point $(st)\vel$, so
\begin{equation}
  t_c(s\vel) = t_c(\vel)/s .
  \label{eq:ch12-scaling}
\end{equation}
Fix a unit vector $\vect{u}$ pointing into the cone, so that
$t_c(\vect{u}) < \infty$. By \cref{eq:ch12-scaling},
$t_c(s\vect{u}) \le \ttc$ holds if and only if $s \ge t_c(\vect{u})/\ttc$, so
the truncated obstacle meets that ray exactly in the half-line
$\set{s\vect{u} : s \ge t_c(\vect{u})/\ttc}$. Its endpoint
$(t_c(\vect{u})/\ttc)\,\vect{u}$ is the point at which the ray enters
$\disc{\pos_{\mathrm{rel}}/\ttc}{R/\ttc}$, because
$\norm{\pos_{\mathrm{rel}}/\ttc - s\vect{u}} \le R/\ttc$ is the same
inequality as $\norm{\pos_{\mathrm{rel}} - (\ttc s)\vect{u}} \le R$, whose
smallest solution is $\ttc s = t_c(\vect{u})$. Hence the truncated obstacle
is the part of the cone lying at or beyond that disk, and its boundary is the
near arc of the disk together with the two legs beyond their points of
tangency with it. \qedhere
\end{proof}
...
\begin{figure}[tb]
  ...
  velocity $\vel$ collides at time $t$ exactly when $\vel$ lies in the disk
  $\disc{\pos_{\mathrm{rel}}/t}{R/t}$; the disks for $t=1$, $2$ and $4$ are
  drawn, each inscribed in the cone. With the horizon $\ttc=2$ only the dark
  region beyond the disk for $t=2$ is forbidden: the velocity $\vel$, which
...
its instance in the worked example, where the truncation disk for
$\ttc = 5\,\mathrm{s}$ has center $\vel_B + \pos_{\mathrm{rel}}/5 = (1,-0.1)$
...
$\VO^{\ttc}$ collides in $\ttc + \eps$ seconds, so the horizon must be long
enough for $A$ to make the maneuver that the next decision will demand.
...
disk $\mathcal{V}_A = \disc{\vect{0}}{v_{\max}}$ of speeds up to the limit
$v_{\max}$ of \cref{def:ch02-single-integrator}. The \textbf{preferred
...
$\pos_A$ toward the goal at the nominal speed. Given the obstacles
...
\begin{figure}[tb]
  ...
  \caption{Choosing a velocity with two obstacles. The blue disk is the
  ...
  The feasible set is what remains of the disk. The preferred velocity
...
sampling algorithm below, for example one that penalizes changes from the
\emph{current} velocity to reduce chattering, or one that maximizes the speed
toward the goal, which is the heuristic of the original
...
least dangerous velocity, for example the one that maximizes $t_c$ or the one
that minimizes $\norm{\vel - \vel^{\mathrm{pref}}} + \kappa/t_c(\vel)$ for a
weight $\kappa > 0$. And the minimizer may be hard to compute exactly when
...
\Fn{\VoTimeToCollision{$\pos$, $\vel$, $R$}}{
  ...
  \If{$\Delta' < 0$}{\KwRet{$\infty$}\tcp*{the ray misses the disk}}
  $t \gets (b - \sqrt{\Delta'})/a$\label{alg:ch12-ttc:root}\;
  \lIf{$t < 0$}{\KwRet{$\infty$}\tcp*[f]{the disk lies behind $A$}}
  \KwRet{$t$}\;
}
\Fn{\VoInside{$\pos_A$, $r_A$, $\pos_B$, $\vel_B$, $r_B$, $\vel_A$, $\ttc$}}{
  ...
  \KwRet{$t < \infty$ \textnormal{and} $t \le \ttc$}\;
}
\end{algorithm}
...
disks already overlap; then the collision is happening now and the function
...
the disk and the velocity is safe for ever. Line~\ref{alg:ch12-ttc:root} takes
...
horizon, which is exactly \cref{def:ch12-truncated}. The guard $t < \infty$
is what makes the limit case work: without it the convention
$\infty \le \infty$ would report every collision-free velocity as dangerous,
and it is precisely this guard that makes $\ttc = \infty$ test membership in
the untruncated $\VO_{A|B}$. The test is the same in three
...
\Fn{\VoChoose{$A$, $\set{B_j}$, $\ttc$}}{
  $\vel^{\mathrm{pref}} \gets$ \VoPreferred{$A$}\tcp*{toward the goal at nominal speed}
  ...
  $F \gets \set{\vel \in S : t_{\min}(\vel) > \ttc \ \text{or}\ t_{\min}(\vel) = \infty}$\label{alg:ch12-choose:feasible}\;
  \If{$F \ne \emptyset$}{
...
velocities that covers the reachable disk, in the book's code ten concentric
...
line~\ref{alg:ch12-choose:feasible}, where $t_{\min} = \infty$ (no collision
at all) counts as safe, is membership in the union of
...
each; with NumPy the loop over $S$ is one vectorized expression.
...
line~\ref{alg:ch12-simulate:move} is exact for a point mass with a commanded velocity,
...
$\vel_B = (0, 0.9)$ toward $(0, 5)$ and does not react to $A$. Both have radius
...
point at $-36.87^\circ$ and $-53.13^\circ$: the leg directions are the
$3$--$4$--$5$ unit vectors $(0.8,-0.6)$ and $(0.6,-0.8)$, and the tangent
points of the circle of radius $1$ around $(5,-5)$, seen from the origin, lie
at distance $\sqrt{d^2-R^2} = 7$ along them, at $(5.6,-4.2)$ and
$(4.2,-5.6)$. The apex is
$\vel_B = (0, 0.9)$. The preferred velocity of $A$ is
$\vel^{\mathrm{pref}} = (1, 0)$, so the relative velocity is
$\vel = (1,0)-(0,0.9) = (1,-0.9)$. Then $\vel\cdot\vel = 1.81$,
$\pos\cdot\vel = 9.5$ and $\pos\cdot\pos - R^2 = 49$, so
$\Delta/4 = 90.25 - 88.69 = 1.56$ and, by \cref{eq:ch12-ttc},
$t_c = (9.5 - 1.249)/1.81 = 4.559\,\mathrm{s} < \ttc$: the preferred velocity
is in $\VO^{\ttc}_{A|B}$, and without avoidance the disks would overlap from
$t = 4.56$ to $t = 5.94\,\mathrm{s}$.
Indeed, in \cref{fig:ch12-geometry}(b) the point $(1,0)$ lies inside the
truncation disk of center $(1,-0.1)$ and radius $0.2$, at distance $0.1$ from
its center.
...
$0.1$; its time to collision is exactly $\ttc = 5\,\mathrm{s}$ (the method
\code{VelocityObstacle.closest\_boundary\_point} computes it). The sampled algorithm cannot
...
\begin{figure}[tb]
  ...
  without avoidance, and the two circles are the disks at $t = 5\,\mathrm{s}$,
  ...
  $t=0.9$ and $t=3.6\,\mathrm{s}$ and returns to its preferred velocity once
  $B$ has passed. (c)~Center distance over time with and without avoidance;
  the dashed line is $R = 1$. Without avoidance the disks overlap from
...
a miss because the relative velocity $(1.2,-0.9)$ passes above the disk
instead of through it (its discriminant is negative); in the workspace this is
$A$ crossing in front of $B$, as \cref{fig:ch12-example}(a) shows. From $t=3.7\,\mathrm{s}$ the agent adds a
small northward component, $\vel_A = (1.046, 0.092)$, at $t = 5.0\,\mathrm{s}$ the centers are
$1.001\,\mathrm{m}$ apart, and from $t=5.1\,\mathrm{s}$ the preferred velocity is
feasible again ($t_c = \infty$): $A$ heads for its goal, arriving after a path
of length $10.027\,\mathrm{m}$ instead of $10$. Without avoidance the centers
...
\begin{table}[tb]
  ...
  velocity ($\infty$: the ray misses the disk).}
...
preferred velocity has $t_c = 0.013\,\mathrm{s}$: the disks are about to touch,
...
half-angle is almost $90^\circ$ ($d = 1.0012\,\mathrm{m} \approx R$), so the
wedge is nearly a half-plane, yet more than a hundred of the samples still lie
outside it. Second, the velocity at $t = 6\,\mathrm{s}$
is $(0.998,-0.064)$, which is not on the sample grid: it is the preferred
velocity itself, now pointing slightly south from $(1.64, 0.22)$ toward the
...
\le R$, and by \cref{eq:ch12-relmotion} this norm is the center distance at time
...
decision is made. The theorem may be applied on each interval separately
because the center distance on $[k\dt, (k+1)\dt]$ depends only on the
velocities held during that interval, and both are constant there. \qedhere
\end{proof}
...
origin, on the side away from $\pos_{\mathrm{rel}}$, so the reachable disk
$\disc{\vect{0}}{v_{\max}}$ is centered on the axis at distance $s > v_{\max}$
...
The disk lies inside the wedge of half-angle $\theta = \arcsin(R/d)$ if and
only if $\arcsin(v_{\max}/s) \le \theta$, i.e., $v_{\max}/s \le R/d$.
\qedhere
\end{proof}

The hypothesis $s > v_{\max}$ is not an extra assumption: $d > R$ makes
$v_{\max}d/R > v_{\max}$, so the conclusion already forces it. The
proposition is therefore a threshold statement about the single quantity $s$.
...
costs $\bigO{n^2 m}$; for $m = 722$ this is $n(n-1)m \approx 4 \cdot 10^4$
evaluations per step for $n = 8$, which NumPy handles in a few milliseconds. Restricting
each agent to its $k$ nearest neighbors, as every real implementation does,
...
\begin{itemize}
  ...
  maneuvers~\cite{fiorini1998motion}; the one-step rule is what practical
  systems use, backed by a global planner (\cref{ch:ch24}).
  \item \textbf{No progress guarantee.} Nothing forces the agent toward its
  ...
  \item \textbf{Sampling error.} The sampled minimizer can be off the exact
...
Let two identical agents fly head-on along parallel lines offset laterally
by $0.2$ --- $A$ from $(-5,0)$ to $(5,0)$ and $B$ from $(5,0.2)$ to
$(-5,0.2)$ --- both at speed $1$ with $R = 1$, and let \emph{both} apply
\cref{alg:ch12-choose} at every step. At $t = 0$ each
...
velocity $(1,0)$ now clears the disk, so $A$ returns to $(1,0)$, and $B$, by
...
so it crosses zero at almost every decision. A real
drone cannot follow such a command, and the maneuver it executes is not the
one that was analyzed.

\begin{figure}[tb]
  \centering
  \inputfigure{ch12/oscillation}
  \caption{Oscillation. Two agents pass each other head-on along lines
  offset by $0.2$. (a)~Trajectories when both apply the velocity-obstacle rule
  ...
  the other's sidestep by canceling its own; with one avoider it changes
...
other takes the other half; the reciprocal constructions that do this are
listed in \cref{sec:ch12-variants} and derived from this chapter's geometry in
\cref{ch:ch13}. Note that reciprocity is a property of the \emph{pair}: against
...
every pair is on a crossing course through the center, and \emph{all} of them
...
center distance over the run and the path length relative to the straight
line.
...
\begin{figure}[tb]
  ...
  crossing agents. (a)~Minimum center distance; the dashed line is $R = 1$.
  Without avoidance every scenario collides. One VO agent among
  non-cooperative intruders (stream) keeps the distance at the guaranteed
  minimum $R$ ($1.0003$) for every $k$, as \cref{cor:ch12-intruder} promises. When all agents apply VO
  simultaneously (circle) the distance dips below $R$ already for $n=3$ and
  stays below it from $n=5$ on.
  (b)~Path length relative to the straight line: the price of avoidance is
  $1.4$--$2.3\,\%$ in the stream and up to $14\,\%$ in the crowded circle.}
...
\begin{table}[tb]
  ...
  \caption{The experiment of \cref{fig:ch12-crossing}. $d_{\min}$ is the minimum
  center distance over the run, ``pairs $<R$'' counts the pairs of agents whose
  center distance dropped below $R$ at some step, and ``empty'' the number of
  decisions at which no sampled velocity was feasible.}
  ...
  $n$ & $d_{\min}$ none & $d_{\min}$ VO & path ratio & $d_{\min}$ none & $d_{\min}$ VO & path ratio & pairs $<R$ & empty\\
  \midrule
...
and pays $1.4$--$2.3\,\%$ extra path, arriving in $11.3$--$11.5\,\mathrm{s}$,
earlier than the $12\,\mathrm{s}$ of the straight flight, because its
maneuver is mostly ``speed up and pass in front''. Among agents that all
apply the rule simultaneously the picture degrades with density: one pair
grazes at $n = 3$ ($0.990$), and from $n = 5$ on the minimum distance falls
below $R$ at every density (to $0.914$ at $n = 6$), decisions with an empty
feasible set appear, and the paths lengthen by up to $14\,\%$ as agents
oscillate around the crowded center. None of these are head-on
...
is the cone beyond the disk $\disc{\pos_{\mathrm{rel}}/\ttc}{R/\ttc}$. An obstacle
that is not a disk, such as a building or a wall, is first inflated by $r_A$
(\cref{thm:ch02-inflation}; the distance to a wall segment is
\cref{def:ch02-point-segment-distance}) and then treated point by point: a velocity is dangerous if
...
long wall forbids every velocity with any component toward it, however far
away the wall is.
...
instead, \cref{thm:ch12-truncated} generalizes without change of argument: $A$
...
the union of these disks over $t \le \ttc$. It is no longer a cone, but the
...
step form the disk $\disc{\vel_A}{a_{\max}\dt}$ intersected with
...
while the agent accelerates toward its new velocity it flies neither the old
...
With a single obstacle, the minimizer of \cref{eq:ch12-cost} is the projection
...
whichever is closest. The method \code{closest\_boundary\_point} of the class
\code{VelocityObstacle} does this in a few lines and returned
$\vel^* = (1, 0.1)$ in \cref{ex:ch12-crossing}. With several obstacles the projection onto one wedge
...
way out of a conflict: climbing over an intruder can be a smaller maneuver
...
distance from the center to the ray, $d\sin\varphi$, is at most $R$, exactly as
...
The proof is the one of \cref{thm:ch12-cone} and \cref{thm:ch12-truncated}
read with 3-vectors; the only new fact is that the set of directions at angle
$\le \theta$ from a fixed axis in $\R^3$ is a circular cone rather than a
wedge. \Cref{exr:ch12-3d} asks for the details. \qedhere
\end{proof}
...
\begin{figure}[tb]
  ...
  with apex at $A$, axis toward the center of the sphere of radius
...
\Cref{fig:ch12-cone3d} shows the cone. The tangent points form
...
chapter in about 650 lines: the plane-geometry helpers copied from
...
the tangent directions, the truncation disk and the exact projection onto the
...
Three situations need explicit handling. When $d \le R$ the disks already
...
$t_c > \ttc + \eps$. Finally, $\vel_{\mathrm{rel}} = \vect{0}$ (two
...
and long enough for the maneuver that a velocity just outside
...
limit. Too short a horizon produces late, sharp maneuvers and empty feasible
sets; too long a horizon produces the frozen behavior of the pitfall below.
...
$v_{\max} = 1.5$. In your own implementation, always add the preferred
velocity and, to reduce chattering, the current velocity to the candidates
(the code of \cref{lst:ch12-choose} adds only $\vel^{\mathrm{pref}}$), and consider a finer ring around
$\vel^{\mathrm{pref}}$. With a single obstacle, prefer the exact projection of
\cref{sec:ch12-variants}; with many obstacles, sampling is robust and accepts
other cost functions and reachable sets without any change to the rule (the
dynamic window of \cref{ch:ch14}). The test of all
candidates against one obstacle is one vectorized NumPy expression
(\code{ttc\_batch}), which is why the self-test, including a sample of the
experiments of \cref{sec:ch12-experiment}, finishes in a fraction of a second;
the full sweep of \cref{tab:ch12-crossing} is produced by
\code{code/figures/gen\_ch12\_sim.py}.
...
vectorized \code{ttc\_batch}, and the running minimum over obstacles is the
...
The disk in \cref{def:ch12-collision-cone} has radius $r_A + r_B$, not $r_B$.
...
component toward it, and a slow drone a hundred meters away blocks a whole
...
\begin{pitfall}[Discretization of the velocity samples]
...
chatters. Sample finely near the preferred and the current velocity, and
always include both as candidates.\index{pitfall!velocity sampling}
\end{pitfall}
...
if the maneuver has pushed the drone too far off its route the replanning
...
up is a real error: a reciprocal maneuver against an intruder avoids only half
of the collision, and a full-responsibility maneuver among swarm members
...
\begin{itemize}
  ...
  tip cut off by the inscribed disk $\disc{\vel_B + \pos_{\mathrm{rel}}/\ttc}{R/\ttc}$,
  ...
  sampling the velocity disk is the simple and robust way to do so. Static
...
avoidance maneuvers; the geometric background (Minkowski sums,
...
test. The non-linear velocity obstacle of \cref{sec:ch12-variants}, which
replaces the cone by the union of the disks swept along a predicted
trajectory, is due to Large, Sekhavat, Shiller and
Laugier~\cite{large2002nonlinear}. Implementations that face many neighbors
restrict the union of \cref{eq:ch12-feasible} to the $k$ nearest ones, the
choice made by the \orca library~\cite{vandenberg2011orca}.
...
$\theta = \arcsin(R/d)$ with the direction to the center, and that they touch
...
that each of these disks is tangent to both legs of the cone, and that the
point of the truncated obstacle closest to the apex lies on the axis at
distance $(d - R)/\ttc$, while the tangency points of the truncation disk lie
...
collision and the time at which the disks separate again, and compute the
...
half-angle of the reachable disk seen from the apex, and evaluate it for
...
collision cone and that the truncation disk has center $\pos_{\mathrm{rel}}/\ttc$.
...
straight wall at distance $2\,\mathrm{m}$ from its center, with $r_A = 0.5$.
...
(\cref{ch:appA}). Build a 2D simulation of $n$ disk agents on crossing courses
...
with velocity obstacles and report, per run, the minimum center distance, the
...
disks and check the frozen behavior without truncation. Keep the harness: in
...
center and radius in terms of $d$ and $R$. (c)~Extend \code{velocity\_samples}
