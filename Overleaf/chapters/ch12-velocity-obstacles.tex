...
$\vel^{\mathrm{pref}}$. With a single obstacle the exact projection computed by the
method \code{closest\_boundary\_point} of \code{VelocityObstacle} is both
cheaper and better; with many
obstacles, sampling is robust and accepts other cost functions and reachable
sets without any change to the rule (the dynamic window of
\cref{ch:ch14}), while the exact
