...
conflict-based search (\cbs) or its bounded-suboptimal variant \ecbs
...
\begin{figure}[tb]
  ...
  $(3, 4)$, where the heuristic cannot see the wall at $x = 4$. (b) Dijkstra's algorithm ($\hcost = 0$) expands 26 cells,
...
distance cannot foresee. The cell $(3, 1)$ is reached one pop after $(0, 4)$,
because both carry $\fcost = 13$ and $11$ respectively; the expansion order is
$\dots, (3, 4), (3, 3), (3, 2), (0, 4), (3, 1), (3, 5), \dots$. Only after these does the frontier reach $\fcost = 13$, and the search
...
\bigO{\log \hcost^*(n)}$; a constant \emph{relative} error is not enough.
...
formulation is the one of Stern et al.~\cite{stern2019mapf}: agents move on a
shared graph in discrete time steps, each agent may move to a neighbour or wait,
and no two agents may occupy the same vertex at the same time step or swap
places along an edge. \Cref{ch:ch08,ch:ch09,ch:ch10} build three planners on top
of that formulation. All three share one component, and it is an \astar search:
...
Let $G = (V, E)$ be the grid graph of \cref{def:ch02-grid}. A \textbf{space-time
state}\index{state!space-time} is a pair $(v, t)$ with $v \in V$ and
...
\pi_{t+1}, t\rangle$ is in none of them for $0 \le t < T$.
\end{definition}
...
demands of a MAPF solution. The value $t_\gamma$ is computed once, before the
...
compute. Write $H$ for the largest time step that appears in any constraint
($H = -1$ if there is none) and $D$ for the largest number of moves from any
cell to $\gamma$ in the grid in which the parked cells of the table are treated
as walls.
...
the only forbidden cells are the parked ones, which are forbidden for ever. Let
...
generated. The consistency of $\hcost$ means the search inherits
...
\begin{figure}[tb]
  ...
  path (solid) waits once and costs~3.}
...
\begin{table}[tb]
  ...
  3 & $((1,1),2)$ & 2 & 1 & 3 & $((2,1),3){:}3$, $((1,1),3){:}4$, and the six entries of step~2 that are still open\\
...
$t = 4$. Agent~2 has to fly from $(3,0)$ to $(0,0)$, which alone takes 3 steps.
Under the table, space-time \astar returns
\[
...
\cref{thm:ch04-horizon}. That failure is not a bug in the search but the known
...
\begin{lstlisting}[caption={The expansion step of space-time \astar (from \code{code/ch04\_astar.py}): the goal-stay test, the wait action, the constraint filter and duplicate detection. \code{h} is the static distance table and \code{table} the reservation table.},label={lst:ch04-spacetime}]
    while heap:
...
step~19 with $\gcost = 13$, which happens to be optimal, but that is luck: on
the 8-connected version of the same grid a first path of cost $\approx 12.24$ is
generated before the optimal one of cost $9 + 2\sqrt{2} \approx 11.83$ is. The
...
\cref{ch:ch24} (\orca, \cref{ch:ch13}; DWA, \cref{ch:ch14}) handles the
