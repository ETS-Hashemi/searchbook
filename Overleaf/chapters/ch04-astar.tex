...
\begin{itemize}
  ...
  \item Implement grid \astar and space-time \astar in \python, visualize the open and closed sets, and recognize the failure modes that matter in a drone planner.
\end{itemize}
\end{objectives}
...
\cref{ch:ch24} calls \astar again whenever a local avoidance maneuver has
...
anytime planner of \cref{ch:ch06} (anytime repairing \astar, \arastar) are best
understood as \astar with extra bookkeeping. This chapter is
...
start. Its frontier grows like a ripple in a pond: a disk around the start that
...
expanded late or not at all. The frontier stops being a disk and becomes an
...
\begin{figure}[tb]
  ...
  is a disk around the start $s$ that reaches the goal $\gamma$ only when it has
...
with its effort focused toward the goal.
\end{keyidea}
...
only when both orthogonal neighbors it passes are free, so a drone of positive
...
Manhattan distance assigns $2 > \sqrt{2}$. \Cref{tab:ch04-heuristics} summarizes
...
chapter.\index{A*!as generalization of Dijkstra}
...
broken toward the larger $\gcost$, and among equal keys the entry pushed first
...
\begin{figure}[tb]
  ...
  \caption{\Cref{ex:ch04-grid} after termination. Gray cells are \Closed
  (expanded), blue cells are still in \Open; the bold number is $\fcost$ at the
  time of expansion (for Dijkstra $\fcost = \gcost$) and the small gray number
...
$(0, 0)$; steps~2--4 then walk up the column $x = 1$: the three cells expanded
there have $\fcost = 7$, because moving toward the
...
expansions for the same instance, and \astar with ties broken toward the
...
practical. Pearl~\cite{pearl1984heuristics} makes this precise: \astar
...
\bigO{\log \hcost^*(n)}$ --- which is why \cref{sec:ch04-variants} buys speed
by giving up optimality rather than by sharpening $\hcost$.
...
Martelli~\cite{martelli1977complexity} builds graphs on which \astar re-expands
nodes exponentially often in the number of nodes, so the $\bigO{\abs{V}}$
expansion count above needs \cref{thm:ch04-consistent}(i).
...
shows why the maximum of several admissible heuristics
(\cref{exr:ch04-max}) is never worse than any of them, and it gives the rule of thumb for designing
...
\begin{figure}[tb]
  ...
  heuristic. Every cell has $\fcost = 38 = C^*$. (a) Breaking ties toward the
  larger $\gcost$, \astar expands only the 39 cells of one optimal path.
  (b) Breaking ties toward the smaller $\gcost$, it expands all 400 cells.
...
seconds, when a neighbor flies through it; a corridor may be usable in one
...
may move to a neighbor or wait, and no two agents may occupy the same vertex at
...
maneuver. If you need geometric length as well, carry it as a second number and
optimize time first; \cref{ch:ch09} discusses the trade-off, and safe interval
path planning~\cite{phillips2011sipp} avoids the discretization altogether.
...
\subsection{Vertex and edge constraints}
...
the agents that were planned earlier, collected in a reservation table --- the
data structure Silver introduced for cooperative
pathfinding~\cite{silver2005cooperative} and defined formally in
\cref{def:ch08-reservation}. Either way the search sees nothing but a set of
forbidden pairs $(v, t)$ and triples $(u, v, t)$, and the class
\code{ReservationTable} in \code{ch04\_astar.py} answers the only two questions
it asks: is $v$ blocked at time $t$, and is the move $u \to v$ blocked at
time $t$.
...
table is empty). This is what \code{ReservationTable.horizon()} computes. (The letter $H$
is the horizon of a stored time-indexed path in
\cref{def:ch02-time-indexed-path}; here it is a property of the constraint
table, and the horizon of the \emph{search} is $T_{\max}$.) Write
...
\Fn{\AstarSpaceTime{$G$, $s$, $\gamma$, $R$, $T_{\max}$}}{
  ...
  \While{\Open $\ne \emptyset$}{
    ...
    \ForEach{$v' \in \{v\} \cup \{v'' : (v'', c) \in \Succ(v)\}$}{\tcp*[h]{wait first, then the moves}\label{alg:ch04-spacetime:succ}\;
...
work is done. It does not test the start state itself: both the algorithm and
\code{space\_time\_astar()} assume that the caller hands over a start that is
free at time~$0$, as the planners of \cref{ch:ch08,ch:ch09} do; if an earlier
agent may park on this agent's start from $t = 0$, add
\emph{or $(s, 0) \in R$} to the same line. Line~\ref{alg:ch04-spacetime:succ} generates the wait action
...
\cref{alg:ch04-astar}. Note that the search minimizes the \emph{arrival time}
...
\begin{figure}[tb]
  ...
  cost~2, which is therefore no longer available. Gray states are the four the
...
first component orders by $\fcost$, the second breaks ties toward the larger
...
\textbf{2. The fallback of the replanning layer.} The local safety layer of
...
seconds-scale avoidance of an intruder locally. When a maneuver has pushed a
...
\cref{ch:ch08} formalizes, and Stern et al.~\cite{stern2019mapf} fix the MAPF
