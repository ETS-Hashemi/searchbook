...
\begin{itemize}
  ...
  \item Implement \orca in \python, reproduce the worked example, and compare no avoidance, VO and \orca in a simulation.
\end{itemize}
\end{objectives}
...
Eight drones of a survey swarm fly nominal paths planned by \cbs
...
Velocity obstacles (\cref{ch:ch12}) give such a rule for one agent among
...
\end{equation}
for the relative position, the relative velocity and the combined radius.
...
$t \mapsto t\,\vel$ enters $\disc{\pos_{\mathrm{rel}}}{r}$ at a time smaller
than $\ttc$.\index{time to collision}
...
velocity obstacle rule alternates between $0$ and about $-0.2$\,m/s for the
whole approach, and its velocity change reverses direction 49~times in 100
steps. For a drone this is a stream of contradictory commands to the
...
\label{def:ch13-rvo}
The \textbf{reciprocal velocity obstacle}\index{reciprocal velocity obstacle}
...
\textbf{time horizon}\index{time horizon $\tau$}: \orca promises nothing beyond
...
half-plane\index{ORCA!half-plane}
\begin{equation}
...
that it uses the same rule. It is called \emph{optimal} because, among all
pairs of half-planes that split the responsibility equally and are safe,
this pair leaves the most permitted velocities near the current ones, in
the sense that for every radius the permitted fraction of the disc around
$\vel_A$ (and around $\vel_B$) is as large as possible; van den Berg et al.\
prove this and note that the equal split can be replaced by any fixed split
that sums to one~\cite{vandenberg2011orca}. \Cref{alg:ch13-halfplane}
...
\Fn{\OrcaClosest{$\pos$, $\vel$, $r$, $\ttc$, $\dt$}}{
  \If{$\norm{\pos} \le r$}{
    $\vect{c} \gets \pos/\dt$; $\rho \gets r/\dt$; $\vect{w} \gets \vel - \vect{c}$; $\vect{n} \gets \vect{w}/\norm{\vect{w}}$\label{alg:ch13-halfplane:overlap}\tcp*{already overlapping: separate within one step}
...
responsibility}\index{ORCA!full responsibility}\index{non-cooperative obstacle}:
\begin{equation}
...
boundary point to $\vel_{\mathrm{rel}}$ lies in the plane spanned by
$\pos_{\mathrm{rel}}$ and $\vel_{\mathrm{rel}}$ (reflecting through that plane
leaves the body unchanged, and the closest point of a convex body is
unique), and in that plane the body is exactly the two-dimensional
truncated cone. Hence $\vect{u}$ and $\vect{n}$ are computed by
...
this, and its self-test checks that the result is the two-dimensional
...
is what the \textbf{incremental linear program}\index{linear program!incremental}
...
\Fn{\OrcaLPLine{$i$, $H_1, \dots, H_{i-1}$, $v_{\max}$, $\vel^{\mathrm{pref}}$}}{
  ...
  \For{$j \gets 1$ \KwTo $i-1$}{
    ...
    \If{$\abs{\mathit{den}} \le \eps$}{\lIf{$\mathit{num} < 0$}{\KwRet{\KwNil} (parallel, $H_j$ excludes the whole line)} \lElse{\KwContinue}}
...
\begin{equation}
  ...
  \label{eq:ch13-dense}
\end{equation}
while the half-planes of static obstacles remain hard. Pushing a half-plane
inward by the same amount $\delta$ for all agents is equivalent to
shrinking the horizon for all of them at once, so the velocity found is the
one that postpones the first collision as long as possible; it ignores
$\vel^{\mathrm{pref}}$ entirely, because safety comes first.
\Cref{eq:ch13-dense} is a linear program in three variables $(v_x, v_y, \delta)$,
...
\Cref{alg:ch13-step} puts the pieces together for $n$ agents. All agents
build their half-planes from the same snapshot of positions and velocities,
solve their programs independently, and only then move. The order of the
half-planes matters in two places: the hard half-planes of static obstacles
come first, so that \cref{alg:ch13-dense} can keep them hard, and within
the agent half-planes a random order gives the expected linear running time.

\begin{algorithm}[htb]
\caption{One \orca time step for $n$ agents}
\label{alg:ch13-step}
\KwIn{agents with positions, velocities, radii, preferred velocities and speed limits; static obstacles; horizons $\ttc$ (agents) and $\ttc_{\mathrm{obst}}$ (obstacles); time step $\dt$}
\KwOut{new velocities and positions of all agents}
\Fn{\OrcaStep{agents, obstacles, $\ttc$, $\ttc_{\mathrm{obst}}$, $\dt$}}{
  \ForEach{agent $A$}{
    $\mathcal{H} \gets$ empty list\;
    \ForEach{obstacle $O$ within sensing range}{append \OrcaHalfPlane{$A$, $O$, $\ttc_{\mathrm{obst}}$, $\dt$, $1$} to $\mathcal{H}$\label{alg:ch13-step:obst}\tcp*{full responsibility, hard}}
    \ForEach{neighbour $B$ within sensing range}{\label{alg:ch13-step:neigh}
      $\alpha \gets \tfrac12$ if $B$ runs \orca, else $1$\label{alg:ch13-step:share}\;
      append \OrcaHalfPlane{$A$, $B$, $\ttc$, $\dt$, $\alpha$} to $\mathcal{H}$\;
    }
    $(\vel, f) \gets$ \OrcaLP{$\mathcal{H}$, $v_{\max}$, $\vel_A^{\mathrm{pref}}$}\label{alg:ch13-step:lp}\;
    \lIf{$f \le \abs{\mathcal{H}}$}{$\vel \gets$ \OrcaDense{$\mathcal{H}$, number of obstacles, $f$, $\vel$, $v_{\max}$}\label{alg:ch13-step:dense}}
    $\vel_A^{\mathrm{new}} \gets \vel$\;
  }
  \ForEach{agent $A$}{$\vel_A \gets \vel_A^{\mathrm{new}}$; $\pos_A \gets \pos_A + \dt\,\vel_A$\label{alg:ch13-step:move}\;}
}
\end{algorithm}
...
\begin{table}[tb]
  ...
  LP & violation of $\vel_A^{\mathrm{pref}}$ & $0.1512$\\
    ...
    & new $\vel_{\mathrm{rel}}$, its angle from $\pos_{\mathrm{rel}}$ & $(2.3618, -0.3000)$, $-14.36^\circ = -\theta$\\
...
\arcsin(1/4.0311) = 14.36^\circ$, so the legs point at $21.49^\circ$ (left)
and $-7.24^\circ$ (right). The truncating disc has centre $(1, 0.125)$ and
radius $0.25$. \emph{Step 2.} $\vel_{\mathrm{rel}} = (2, 0)$ points at
$0^\circ$, between the legs, and at distance $2$ it is far beyond the
...
by symmetry B returns $(-1.1809, 0.1500)$. Each agent has changed its
velocity by $0.1512$\,m/s, and the new relative velocity
$(2.3618, -0.3000)$ makes the angle $-14.36^\circ = -\theta$ with
$\pos_{\mathrm{rel}}$: it lies exactly on the right leg, so the discs will
graze and not penetrate. This is the pairwise guarantee of
...
would have been $(2.1809, -0.1500)$, at $-3.93^\circ$, inside the cone.
...
\begin{figure}[tb]
  ...
  distance over all pairs against time; the dashed line is the collision
...
Let $n$ agents run \cref{alg:ch13-step} with the same horizon $\ttc$ from
...
With $k$ neighbours per agent, one step of \cref{alg:ch13-step} costs
...
\emph{safety filter} for the next $\ttc$ seconds and nothing more.
\begin{itemize}
  \item \emph{Feasibility.} The half-planes may not intersect. The fallback
  of \cref{alg:ch13-dense} then minimises the largest violation, which
  postpones the first collision but does not exclude it.
  \item \emph{Exact, shared information.} Both agents must use the same
  $\pos_{\mathrm{rel}}$, $r$, $\ttc$ and current velocities. Sensing noise,
  latency and different horizons make the two half-planes slightly
  inconsistent; the radius margin of \cref{sec:ch13-implementation} absorbs
  small inconsistencies, and the intruder case absorbs the large ones by
  taking full responsibility.
  \item \emph{Instantaneous velocity changes.} The agents are assumed to fly
  the new velocity immediately. A drone tracks it with a delay and an error
  (\cref{ch:ch02,ch:ch14}); again the radius margin pays for it.
  \item \emph{No goal-reaching, no completeness.} \orca chooses the closest
  permitted velocity to the preferred one, step by step, with no memory and
  no lookahead beyond $\ttc$. It can stop an agent forever.
\end{itemize}

\begin{proposition}[\orca is not a global planner]
\label{thm:ch13-incomplete}
There are instances with a collision-free solution in which agents running
\cref{alg:ch13-step} never reach their goals.
\end{proposition}
...
corridor. The wall half-planes forbid any lateral velocity that would reach
a wall within $\ttc_{\mathrm{obst}}$, so the only permitted velocities lie
along the corridor. As the agents approach, the cone half-angle $\theta$
grows towards $90^\circ$ and the half-planes of the pair forbid every
relative velocity that closes the gap, so the programs return velocities of
decreasing speed and finally $\vect{0}$ for both: the closest velocity to
$\vel^{\mathrm{pref}}$ among the permitted ones is to stand still, because
backing up into the bay is further from $\vel^{\mathrm{pref}}$ than stopping,
and no half-plane ever asks for it. The state repeats forever
(\cref{exr:ch13-corridor}).
\end{proof}
...
bounded error $\eps$ over the next interval, a non-convex set that they
approximate by convex polygons, while inflating the radius by
$\eps$~\cite{alonsomora2013optimal}. The same recipe applies to a
...
\paragraph{Responsibility shares and priorities.}
Any split $\alpha_A + \alpha_B = 1$ keeps the guarantee. A share of
$(1, 0)$ gives $B$ the right of way, which a swarm can use to let a drone
with a critical battery through, or to let a formation leader keep its
course while the followers avoid; the shares must be agreed on both sides,
otherwise \cref{thm:ch13-pairwise} fails (see the first pitfall below).
...
plain tuples and a few NumPy calls; \cref{lst:ch13-cases,lst:ch13-lp,lst:ch13-line}
show the three functions that carry the mathematics. The geometry helpers
...
\begin{lstlisting}[caption={The one-dimensional program on the boundary line of \code{lines[i]} (the line program of \cref{alg:ch13-lp}); with \code{direction\_opt} it maximises along \code{v\_opt} instead, as the dense fallback needs.},label={lst:ch13-line}]
...
Obstacles get a shorter $\ttc_{\mathrm{obst}}$, because a wall is not going
...
into the cone; the symptom is agents that avoid correctly in most
encounters and collide in a few. Test every case against
...
receives from the global planner (\cbs or \ecbs, \cref{ch:ch09,ch:ch10}) the
...
radius inflated by the predicted position uncertainty at the horizon, so
that the drone alone does all the avoiding and the constraint stays
conservative when the prediction is poor. The layer delivers one velocity
...
Manocha~\cite{vandenberg2008rvo}, and \orca, with the optimality proof, the
...
$(1, 0)$ at $y = 0$ with $\ttc_{\mathrm{obst}} = 2$ and show that they
forbid every velocity with $\abs{v_y}$ above a threshold that you compute.
(b) Argue that as the agents approach, the program of each returns
velocities of decreasing speed and finally $\vect{0}$, and that the state is
then a fixed point of \cref{alg:ch13-step}. (c) Propose two remedies, one
...
the three-dimensional program either by implementing the recursive
incremental algorithm (when a half-space is violated, solve the
two-dimensional program of \cref{alg:ch13-lp} on its boundary plane) or,
to start with, by sampling velocities in the ball $\norm{\vel} \le v_{\max}$
and choosing the closest feasible sample to $\vel^{\mathrm{pref}}$. Verify
that the smallest centre distance never drops below $r_A + r_B$ and compare
the running times of the two solvers.
\end{exercise}
