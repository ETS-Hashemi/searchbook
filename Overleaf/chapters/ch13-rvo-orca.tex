...
\begin{itemize}
  ...
  \item Solve the per-agent program, the velocity closest to the preferred one inside all half-planes and the speed disk, with the incremental two-dimensional linear program, and handle the dense case in which the half-planes have an empty intersection.
...
neighbors. The rule must return a velocity that is safe for a short horizon,
...
permitted velocities per neighbor, so that the velocity choice among any
number of neighbors becomes a tiny linear program with a provable pairwise
...
Two people walk toward each other in a corridor. If both step to their left
...
\begin{figure}[tb]
  ...
  reacts to a neighbor who has already reacted. Fourth row: with reciprocity
...
makes the choice among several neighbors a combinatorial problem. Instead it
...
neighbors A intersects all its half-planes and picks the velocity closest to
...
Agents are disks in the plane, as in \cref{ch:ch02}. Agent $A$ has center
$\pos_A \in \R^2$, radius $r_A$, current velocity $\vel_A$, a
\textbf{preferred velocity}\index{preferred velocity} $\vel_A^{\mathrm{pref}}$
(the velocity it would fly if it were alone, for instance toward the next
...
velocity $-\vel_{\mathrm{rel}}$, and the two disks overlap at time $t \ge 0$
exactly when $\norm{\pos_{\mathrm{rel}} - t\,\vel_{\mathrm{rel}}} < R$,
that is, when $t\,\vel_{\mathrm{rel}}$ lies in the open disk
$\disc{\pos_{\mathrm{rel}}}{R}$ of the Minkowski sum
(\cref{ch:ch02}).\index{Minkowski sum} We use open disks throughout: a relative
velocity along which the disks merely touch counts as safe, and the safety
...
the \emph{legs}, are tangent to the disk $\disc{\pos_{\mathrm{rel}}}{R}$; its
...
\end{equation}
The truncated set $\VO^{\ttc}_{A|B}$ is the union of the disks
$\disc{\pos_{\mathrm{rel}}/t}{R/t}$, which shrink and move toward the apex as
...
therefore consists of three pieces: the arc of the smallest disk that faces
the apex, between the two points where the legs touch it, and the two legs
from those tangent points outward. Membership is a ray--disc test
...
obstacles induced by the \emph{current} velocities of the neighbors. When
the neighbors are also agents running this rule, the current velocities they
...
often, because two drones that fly toward each other along a shared corridor
are in exactly this situation.

\begin{proposition}[The reciprocal dance of velocity obstacles]
\label{thm:ch13-dance}
Let two agents of equal radius fly toward each other with
...
$\tfrac12\VO^{\ttc}_{A|B} = \VO^{2\ttc}_{A|B}$, because halving the disk
...
\begin{figure}[tb]
  ...
  velocity. The dotted gray curve is RVO in an almost symmetric instance
...
root: the permitted set per neighbor is a half-plane, which is convex, and
...
\begin{figure}[tb]
  ...
  $\pos_{\mathrm{rel}}$, cut off by the disk of center $\pos_{\mathrm{rel}}/\ttc$
...
which the construction is centered the \emph{optimization velocities}; using
...
toward the cone, against $\vect{n}$, and $\norm{\vect{u}}$ is the margin by
which the pair may drift toward danger. The boundary has three pieces, and
...
and $\rho = R/\ttc$ be the center and radius of the truncating disk, and
...
\end{enumerate}
If $\norm{\pos_{\mathrm{rel}}} \le R$ the disks already overlap, the cone is
the whole plane, and the construction is applied instead to the disk
$\disc{\pos_{\mathrm{rel}}/\dt}{R/\dt}$ of relative velocities that leave the
disks overlapping after one time step: $\vect{w} = \vel_{\mathrm{rel}} -
\pos_{\mathrm{rel}}/\dt$, $\vect{n} = \vect{w}/\norm{\vect{w}}$ and
$\vect{u} = (R/\dt - \norm{\vect{w}})\,\vect{n}$.
\end{lemma}
\begin{proof}[Proof sketch]
The two legs are tangent to the truncating disk at the points where the
radii make the angle $\pi/2 - \theta$ with the direction from the center
toward the apex, so the front arc spans exactly the directions $\vect{w}$
...
away from the center of the truncating disk, which is outward for the arc
whether $\vel_{\mathrm{rel}}$ is inside or outside the disk. In case (ii),
...
near the optimization velocities; they argue this geometrically and note that
...
\begin{algorithm}[htb]
\caption{The \orca half-plane of agent $A$ induced by neighbor $B$}
...
\Fn{\OrcaClosest{$\pos$, $\vel$, $R$, $\ttc$, $\dt$}}{
  ...
  $\vect{c} \gets \pos/\ttc$; $\rho \gets R/\ttc$; $\vect{w} \gets \vel - \vect{c}$\label{alg:ch13-halfplane:disc}\tcp*{the disk that truncates the cone}
...
\cref{thm:ch13-cases}: the disks overlap, the horizon is replaced by one time
step, and the half-plane pushes the agents apart. Line~\ref{alg:ch13-halfplane:disc}
forms the truncating disk and the vector $\vect{w}$ from its center to the
...
the axis $\pos_{\mathrm{rel}}$, truncated by the ball of center
...
Agent $A$ with neighbors $B_1, \dots, B_m$ has $m$ half-planes
...
\end{equation}
The feasible set is an intersection of half-planes and a disk, hence
...
the disk, or it lies on the boundary line of one of the half-planes (or at
...
specialized to two dimensions as in the RVO2 library.
(\cref{sec:appB-lp} reviews linear programs in two variables.)

\begin{figure}[tb]
  \centering
  \inputfigure{ch13/lp}
  \caption{The per-agent program in velocity space: the speed disk and three
  ...
  $H_2$, $H_3$ and the disk. Had the projection violated $H_2$, the
  solution would sit at the intersection of the two boundary lines.}
  \label{fig:ch13-lp}
\end{figure}

\begin{algorithm}[htb]
\caption{Incremental two-dimensional linear program over half-planes and the speed disk}
\label{alg:ch13-lp}
...
\Fn{\OrcaLPLine{$i$, $H_1, \dots, H_{i-1}$, $v_{\max}$, $\vel^{\mathrm{pref}}$}}{
  ...
  \lIf{$\Delta < 0$}{\KwRet{\KwNil} (the line misses the disk)}
  $t_- \gets -b - \sqrt{\Delta}$; $t_+ \gets -b + \sqrt{\Delta}$\label{alg:ch13-lp:chord}\tcp*{chord of the line inside the disk}
...
velocity, clipped to the disk. If the next half-plane $H_i$ is satisfied by
...
So the problem drops to one dimension: \OrcaLPLine parameterizes the line as
$\vect{q}_i + t\,\vect{d}$, finds the interval $[t_-, t_+]$ of the line inside
the disk (line~\ref{alg:ch13-lp:chord}, from the quadratic
...
empty exactly when $H_1, \dots, H_i$ and the disk have no common point, and
...
velocity in the disk violates at least one of them, which means that no
velocity is safe for the full horizon $\ttc$ with respect to every
neighbor. This is not a failure of the algorithm but a fact about the
...
half-planes uniformly and choose the velocity that \textbf{minimizes the
...
heuristic: it minimizes a penetration, not the time to the first collision.
...
$(v_x, v_y, \delta)$ together with the speed disk, the same
...
finds the velocity that minimizes the violation of that half-plane subject
...
Minimizing $\mathrm{viol}_i$ means moving as far as possible in direction
...
\begin{algorithm}[htb]
\caption{Dense fallback: minimize the largest violation when the half-planes do not intersect}
...
\KwOut{a velocity in the disk that satisfies the hard half-planes and minimizes $\max_{i > k} \mathrm{viol}_i(\vel)$}
...
$\vel$ attains it. If even the hard half-planes and the disk have no common
...
per neighbor, with share $\tfrac12$ if the neighbor runs \orca and share
...
\begin{table}[tb]
  ...
  1 & $\pos_{\mathrm{rel}}$, $R$, $\ttc$ & $(4, 0.5)$, $1$, $4$\\
    ...
    & truncating disk: center $\pos_{\mathrm{rel}}/\ttc$, radius $R/\ttc$ & $(1, 0.125)$, $0.25$\\
...
signed angle from $\pos_{\mathrm{rel}}$ to $\vel$, positive counterclockwise,
so that the legs are at $\pm\theta$ and the cone is the set of directions with
$\abs{\angle(\vel, \pos_{\mathrm{rel}})} < \theta$. The truncating disk has center $(1, 0.125)$ and
...
truncating disk: the agents collide in about $1.6$\,s, well within
...
the cone is for the relative velocity to turn slightly downward,
...
half-plane by $0.1512$: it is inside the disk, so \cref{alg:ch13-lp} projects
...
it lies exactly on the right leg, so the disks will graze and not
...
center at the same time. \Cref{fig:ch13-circle} shows eight agents of radius
...
collision with every neighbor is at least $\ttc$); sampled RVO (the same
...
\begin{figure}[tb]
  ...
  velocity obstacle rule and with \orca. Bottom: the smallest center
  ...
  center; the VO agents zig-zag and four pairs still collide; the \orca
  agents spiral gently around the center and the smallest distance touches
...
\begin{table}[tb]
  ...
  ($30$\,s, $300$ steps). ``Colliding pairs'' counts the pairs whose disks
...
Three things to notice. Without avoidance every pair collides at the center
at $t \approx 10$\,s, as it must. The velocity obstacle rule avoids most of
that, but the dance of \cref{thm:ch13-dance} now involves seven neighbors
...
agents slow down near the center and the last one arrives at $26.2$\,s
...
partly backward while the cluster passes. That slowing-down is the
behavior the local safety layer of \cref{ch:ch24} wants: it buys time for
...
\end{lemma}
\begin{proof}
Openness follows from \cref{eq:ch13-vo-tau}, a union of open disks. For
...
\]
is a convex combination of two points of the disk, because the two
coefficients are non-negative and sum to one. Hence $t\vel$ lies in the
disk and $\vel \in \VO^{\ttc}_{A|B}$. The boundary of a convex set with a
...
flying these velocities, the disks of $A$ and $B$ do not overlap during the
next $\ttc$ seconds.
\end{theorem}
\begin{proof}
...
\cref{def:ch13-vo} the disks do not overlap for $t \in (0, \ttc]$.
\end{proof}
...
the same snapshot, with share $\tfrac12$ toward each other and share $1$
toward obstacles and intruders that keep their assumed velocities. If every
...
one-dimensional minimization over an interval. For the running time,
iteration $i$ costs $\bigO{1}$ if $H_i$ is satisfied and $\bigO{i}$
otherwise. Backward analysis~\cite{seidel1991small,deberg2008computational}:
...
With $k$ neighbors per agent, one step of \cref{sec:ch13-step} costs
$\bigO{k}$ per agent for the half-planes and the program, plus the
neighbor query, $\bigO{\log n + k}$ with a kd-tree, so $\bigO{n(\log n + k)}$
...
once two disks already overlap, the overlap branch of \cref{thm:ch13-cases}
...
A static disk is an agent with $\vel_B = \vect{0}$ and share $1$. For
...
tangent to the disks around the endpoints and, after truncation, of a
...
that controller, and the linearized chance constraint has exactly the shape
...
\paragraph{Neighbor queries.}
Every agent needs its neighbors within the sensing range, and a linear
...
nearest-neighbor searches of \cref{ch:ch16}. Cap the number of neighbors
...
toward the next waypoint of the nominal path at cruise speed, scaled down
...
the localization error, half of the position uncertainty of the neighbor
...
neighbor ($\alpha = \tfrac12$ on one side, $0$ on the other), after which the
...
treating cooperative neighbors with full responsibility on both sides
...
the \emph{type} of the neighbor (swarm mate, intruder, obstacle), never
from its current behavior.
\end{pitfall}

\begin{pitfall}[The normal points the wrong way, or the disk is forgotten]
$\vect{n}$ must be the \emph{outward} normal of $\VO^{\ttc}$ at the closest
point in every case: away from the truncating disk's center on the arc,
...
inside. The second bug is to solve the program without the disk: the line
...
guarantee is about is never realized.
\end{pitfall}
...
needs to cross the sensing range, every visible neighbor produces an
...
velocity: toward the next waypoint at cruise speed. While no predicted
...
command per step to the flight controller and reports two things upward:
...
\begin{itemize}
  ...
  \item \orca builds, for each pair and horizon $\ttc$, the truncated cone
        ...
        or the one-step disk when already overlapping) with the outward normal
  ...
  \item Each agent chooses the velocity closest to its preferred one inside
        all its half-planes and the disk $\norm{\vel} \le v_{\max}$: a
        ...
        the dense fallback minimizes the largest violation.
...
Alonso-Mora~\cite{zhu2019chance}. The randomized incremental linear program
is Seidel's algorithm~\cite{seidel1991small}; the textbook treatment,
including the three-dimensional version and the backward analysis, is
...
the disks would touch. (b) Apply \cref{thm:ch13-cases}: which case applies,
...
$\VO^{\ttc}_{A|B}$ and not on the extension of the leg toward the apex.
\end{exercise}
...
and show where the disk enters the computation.
\end{exercise}

\begin{exercise}[\difficulty{2}]
\label{exr:ch13-infeasible}
Construct three half-planes whose intersection with the disk
$\norm{\vel} \le 1.5$ is empty, for instance three lines forming a triangle
that lies outside the disk with the permitted sides facing away from it.
(a) Solve \cref{eq:ch13-dense} for your instance geometrically: the solution
equalizes the violations of the active half-planes. (b) Walk through
...
along the $x$-axis, walls at $y = \pm 0.75$ represented by disks of radius
$0.5$ spaced $0.2$ apart with their centers at $y = \pm 1.25$. (a) Write down
the half-planes that the two nearest wall disks induce on an agent flying
...
verify that the smallest center distance never drops below $r_A + r_B$.
\end{exercise}
