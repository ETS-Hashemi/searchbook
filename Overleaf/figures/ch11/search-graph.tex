...
\begin{tikzpicture}[x=1.86cm,y=1cm,font=\footnotesize,
  ...
  step/.style={font=\tiny,text=sbGray},
...
\node[step,anchor=south] at (v0.north) {steps 1, 4};
\node[step,anchor=south] at (v1.north) {steps 2, 3};
% backpropagation
...
\node[step,anchor=south] at (w4.north) {step 29};
\node[step,anchor=south] at (w5.north) {step 30};
\node[step,anchor=south] at (w6.north) {step 31};
\node[step,anchor=south] at (w7.north) {popped};
% time axis
