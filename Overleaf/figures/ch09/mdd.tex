...
\begin{scope}
  ...
  \node[m,fill=sbAgentC!25] (c0) at (2.4,0) {$(2,2)$};
  \node[mc,fill=sbAgentC!25] (c1) at (2.4,-0.85) {$(2,1)$};
  \node[m,fill=sbAgentC!25] (c2) at (2.4,-1.7) {$(3,1)$};
  \node[m,fill=sbAgentC!25] (c3) at (2.4,-2.55) {$(4,1)$};
  \node[m,fill=sbAgentC!25] (c4) at (2.4,-3.4) {$(4,0)$};
  \draw[e] (c0) -- (c1); \draw[e] (c1) -- (c2); \draw[e] (c2) -- (c3); \draw[e] (c3) -- (c4);
  \node[sbannot,text=sbAgentB] at (0,0.55) {MDD of $a_2$};
  \node[sbannot,text=sbAgentC] at (2.4,0.55) {MDD of $a_3$};
  \node[sbannot,text=black,align=center] at (1.2,-4.1) {one cell each at level 1:\\ $\langle a_2,a_3,(2,1),1\rangle$ is cardinal};
\end{scope}
% ---------------- right: open 3 x 3 room (non-cardinal)
\begin{scope}[xshift=6.5cm]
  \foreach \t in {0,...,4} \node[sbannot,anchor=east] at (-2.45,-0.85*\t) {$t{=}\t$};
  % a1: (0,0) -> (2,2)
